...
function flattenVersions(_all) {
  ...
  return all.map(({
    path,
    version
  }) => "\t".concat(version.padEnd(verLength), "\t").concat(expandPath(path))).join('\n');
}
/**
 * @name detectPackage
...
function detectPackage({
  ...
  (0, _assert.default)(name.startsWith('@polkadot'), "Invalid package descriptor ".concat(name));
  ...
  if (_global.__polkadotjs[name].length !== 1) {
    ...
    console.warn("Multiple instances of ".concat(name, " detected, ensure that there is only one package in your dependency tree.\n").concat(versions));
  }
}