...
function fromMetadata(registry, metadata) {
  return metadata.asLatest.modules.reduce((result, moduleMetadata) => {
    ...
    result[(0, _util.stringCamelCase)(name.toString())] = moduleMetadata.constants.reduce((newModule, meta) => {
      ...
      const codec = (0, _create.createTypeUnsafe)(registry, type, [AS_STRIPPED.includes(type) ? meta.value.toU8a(true) : meta.value]); // This is not a perfect idea, however as it stands with number-only constants on the metadata
