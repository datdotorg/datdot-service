...
function createKeyDoubleMap(registry, itemFn, stringKey, args, [hasher1, hasher2], metaVersion) {
  ...
  const val1 = (0, _codec.createTypeUnsafe)(registry, map.key1.toString(), [key1]).toU8a();
  const val2 = (0, _codec.createTypeUnsafe)(registry, map.key2.toString(), [key2]).toU8a(); // as per createKey, always add the length prefix (underlying it is Bytes)
...
function extendPrefixedMap(registry, itemFn, storageFn) {
  storageFn.iterKey = extendHeadMeta(registry, itemFn, storageFn, () => new _codec.Raw(registry, createPrefixedKey(itemFn)));
  return storageFn;
}
/**
