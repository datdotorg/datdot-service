...
function toLatest(registry, version, rpcData, withThrow = true) {
  it("converts v".concat(version, " to latest"), () => {
  ...
  describe('storage with default values', () => {
    ...
    metadata.asLatest.modules.filter(({
      ...
      mod.storage.unwrap().items.forEach(({
        ...
        const location = "".concat(mod.name, ".").concat(name, ": ").concat(inner);
        it("creates default types for ".concat(location), () => {
          expect(() => {
            try {
              (0, _create.createTypeUnsafe)(registry, inner, [fallback]);
            } catch (error) {
              const message = "".concat(location, ":: ").concat(error.message);
