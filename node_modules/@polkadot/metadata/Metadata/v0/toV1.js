...
function toV1Calls(registry, {
  ...
  return functions.length ? new _codec.Option(registry, _codec.Vec.with('FunctionMetadataV1'), functions) : new _codec.Option(registry, _codec.Vec.with('FunctionMetadataV1'));
}

function toV1Events(registry, metadataV0, prefix) {
  const events = metadataV0.outerEvent.events.find(event => event[0].eq(prefix));
  return events ? new _codec.Option(registry, _codec.Vec.with('EventMetadataV1'), events[1]) : new _codec.Option(registry, _codec.Vec.with('EventMetadataV1'));
}

function toV1Storage(registry, {
  storage
}) {
  return storage.isSome ? new _codec.Option(registry, 'Vec<StorageFunctionMetadataV1>', storage.unwrap().functions) : new _codec.Option(registry, 'Vec<StorageFunctionMetadataV1>');
}
/**
...
function toV1(registry, metadataV0) {
  return (0, _codec.createType)(registry, 'MetadataV1', {
    modules: metadataV0.modules.map(mod => {
      ...
      return (0, _codec.createType)(registry, 'ModuleMetadataV1', {
        name: mod.prefix,
        // Not capitalized
        prefix,
        // Capitalized
