...
function createStorageType(registry, entryType) {
  ...
  return [_objectSpread({}, map, {
    kind: (0, _types.createType)(registry, 'MapKindV11', null, map.linked.isTrue ? 1 : 0)
  }), 1];
}

function convertModule(registry, mod) {
  const storage = mod.storage.unwrapOr(null);
  return (0, _types.createType)(registry, 'ModuleMetadataV11', _objectSpread({}, mod, {
    storage: storage ? _objectSpread({}, storage, {
      items: storage.items.map(item => _objectSpread({}, item, {
        type: (0, _types.createType)(registry, 'StorageEntryTypeV11', ...createStorageType(registry, item.type))
      }))
    }) : null
  }));
}
...
function toV11(registry, _ref) {
  ...
  return (0, _types.createType)(registry, 'MetadataV11', {
