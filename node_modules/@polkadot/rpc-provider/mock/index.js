...
const INTERVAL = 1000;
const SUBSCRIPTIONS = Array.prototype.concat.apply([], Object.values(_jsonrpc.default).map(area => Object.values(area.methods).filter(method => method.isSubscription).map(({
  method,
  section
}) => "".concat(section, "_").concat(method)).concat('chain_subscribeNewHead')));
