...
function isSr25519(type) {
  return type === 'sr25519';
}

function fromSeed(type, seed) {
  return isSr25519(type) ? (0, _utilCrypto.schnorrkelKeypairFromSeed)(seed) : (0, _utilCrypto.naclKeypairFromSeed)(seed);
}
...
function sign(type, message, pair, {
  ...
  withType ? multiSignaturePrefix(type) : SIG_TYPE_NONE, isSr25519(type) ? (0, _utilCrypto.schnorrkelSign)(message, pair) : (0, _utilCrypto.naclSign)(message, pair));
}

function verify(type, message, signature, publicKey) {
  return isSr25519(type) ? (0, _utilCrypto.schnorrkelVerify)(message, signature, publicKey) : (0, _utilCrypto.naclVerify)(message, signature, publicKey);
}
/**
 * @name pair
...
function createPair({
  ...
  return {
    ...
    sign: (message, options) => sign(type, message, {
      publicKey,
      secretKey
    }, options),
