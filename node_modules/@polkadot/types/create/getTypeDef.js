...
function _decodeFixedVec(value, type, _) {
  ...
  (0, _util.assert)(length <= 256, "".concat(type, ": Only support for [Type; <length>], where length <= 256"));
...
function _decodeInt(value, type, _, clazz = 'Int') {
  ...
  (0, _util.assert)(length <= 8192 && length % 8 === 0, "".concat(type, ": Only support for ").concat(clazz, "<bitLength>, where length <= 8192 and a power of 8, found ").concat(length));
  value.displayName = displayName;
  value.length = length;
  return value;
}
...
function hasWrapper(type, [start, end]) {
  ...
  (0, _util.assert)(type.endsWith(end), "Expected '".concat(start, "' closing with '").concat(end, "' on ").concat(type));
  return true;
}
...
['BTreeMap<', '>', _types.TypeDefInfo.BTreeMap, _decodeTuple], ['HashMap<', '>', _types.TypeDefInfo.HashMap, _decodeTuple], ['Int<', '>', _types.TypeDefInfo.Int, _decodeInt], ['Result<', '>', _types.TypeDefInfo.Result, _decodeTuple], ['UInt<', '>', _types.TypeDefInfo.UInt, _decodeUInt]];
