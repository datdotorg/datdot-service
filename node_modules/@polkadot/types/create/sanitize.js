...
exports.removeColonPrefix = removeColonPrefix;
...
alias(['exec::StorageKey'], 'ContractStorageKey'), // alias for internal module mappings
alias(['exec', 'grandpa', 'marker', 'session', 'slashing'].map(s => "".concat(s, "::")), ''), // flattens tuples with one value, `(AccountId)` -> `AccountId`
flattenSingleTuple(), // converts ::Type to Type, <T as Trait<I>>::Proposal -> Proposal
removeColonPrefix()]; // given a starting index, find the closing >
...
function alias(src, dest, withChecks = true) {
  return value => {
    return src.reduce((value, src) => {
      return value.replace(new RegExp("(^".concat(src, "|").concat(BOX_PRECEDING.map(box => "\\".concat(box).concat(src)).join('|'), ")"), 'g'), src => withChecks && BOX_PRECEDING.includes(src[0]) ? "".concat(src[0]).concat(dest) : dest);
    }, value);
  };
}
...
function removeTraits() {
  return value => {
    ...
    .replace(/(T|Self|wasm)::/g, '') // replace `<T as Trait>::` (whitespaces were removed above)
