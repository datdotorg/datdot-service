...
describe('Digest', () => {
  ...
  it('has the correct JSON representation', () => {
    const digest = new _Digest.default(registry, _Header.default.result.digest);
    expect(digest.toJSON()).toEqual(_Header.default.result.digest);
  });
  ...
  it('filters logs, excluding items', () => {
    const logs = new _Digest.default(registry, _SignedBlock.default.result.block.header.digest).logsWith('PreRuntime');
    expect(logs.length).toEqual(1);
    expect(logs[0].type).toEqual('PreRuntime');
  });
  it('filters logs, including items', () => {
    const logs = new _Digest.default(registry, _SignedBlock.default.result.block.header.digest).logsWithout('PreRuntime');
    expect(logs.length).toEqual(1);
