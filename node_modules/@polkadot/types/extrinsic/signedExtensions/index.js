...
const allExtensions = _objectSpread({}, _substrate.default, {}, _polkadot.default); // the v4 signed extensions (the order is important here, as applied by default)
...
function expandExtensionTypes(extensions, type) {
  return extensions.map(key => allExtensions[key]).filter(info => !!info).reduce((result, info) => _objectSpread({}, result, {}, info[type]), {});
}