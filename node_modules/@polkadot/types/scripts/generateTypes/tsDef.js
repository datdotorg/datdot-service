...
function createGetter(definitions, name = '', type, imports, doc) {
...
function tsResultGetter(definitions, resultName = '', getter, {
  info,
  name = '',
  type
}, imports) {
...
function tsSet(definitions, {
  ...
  const types = sub.map(({
    name
  }) => createGetter(definitions, "is".concat(name), 'boolean', imports));
...
function generateTsDefFor(importDefinitions, defName, {
  ...
  const sortedDefs = interfaces.sort((a, b) => a[0].localeCompare(b[0])).map(([, definition]) => definition).join('\n\n');
  ...
  Object.entries(imports.localTypes).forEach(([moduleName, typeMap]) => {
