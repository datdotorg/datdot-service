...
class Rpc {
  ...
  _createInterfaces(userRpc) {
    ...
    this.sections.forEach(sectionName => {
      this[sectionName] = _objectSpread({}, this._createInterface(sectionName, _jsonrpc.default[sectionName] || {}), {}, this._createInterface(sectionName, userRpc[sectionName] || {}));
    });
  }
  ...
  _createInterface(section, methods) {
    return Object.keys(methods).reduce((exposed, method) => {
      ...
      this.mapping.set("".concat(section, "_").concat(method), _objectSpread({}, def, {
        isSubscription,
  ...
  _createMethodSend(section, method, def) {
    const rpcName = "".concat(section, "_").concat(method);
  ...
  _formatInputs(def, inputs) {
    ...
    const optText = reqArgCount === def.params.length ? '' : " (".concat(def.params.length - reqArgCount, " optional)");
  ...
  _formatOutput(method, rpc, params, result) {
    if (rpc.type === 'StorageData') {
      ...
      return this._formatStorageSet(params[0], result.changes);
