...
function createErrorMessage({
  ...
  const inputs = params.map(({
    ...
    name,
    type
  }) => "".concat(name).concat(isOptional ? '?' : '', ": ").concat(type)).join(', ');
...
class Rpc {
  ...
  createInterfaces(interfaces, userBare) {
    ...
    const user = Object.entries(userBare).reduce((user, [sectionName, methods]) => {
  ...
  createMethodSend(method) {
    ...
    const call = (...values) => {
      ...
      return (0, _rxjs.of)(1).pipe((0, _operators.map)(() => this.formatInputs(method, values)), (0, _operators.switchMap)(params => (0, _rxjs.combineLatest)([(0, _rxjs.of)(params), (0, _rxjs.from)(this.provider.send(rpcName, params.map(param => param.toJSON())))])), (0, _operators.map)(([params, result]) => this.formatOutput(method, params, result)), (0, _operators.catchError)(error => {
    ...
    const call = (...values) => {
      return new _rxjs.Observable(observer => {
        ...
        try {
          ...
          subscriptionPromise = this.createSubscriber({
            subType,
            subName,
            paramsJson,
            update
        ...
        return () => {
          ...
          subscriptionPromise.then(subscriptionId => (0, _util.isNumber)(subscriptionId) ? this.provider.unsubscribe(subType, unsubName, subscriptionId) : Promise.resolve(false)).catch(error => l.error(createErrorMessage(method, error)));
        };
      }).pipe((0, _rxjs2.drr)());
    };
  ...
  formatInputs(method, inputs) {
    const reqArgCount = method.params.filter(({
      isOptional
    }) => !isOptional).length;
    ...
    return inputs.map((input, index) => (0, _types.createTypeUnsafe)(this.registry, method.params[index].type, [input]));
  }
  ...
  formatOutput(method, params, result) {
    ...
    return (0, _types.createTypeUnsafe)(this.registry, method.type, [result]);
  }
  ...
  formatStorageData(key, value) {
    ...
    return (0, _types.createTypeUnsafe)(this.registry, type, [isEmpty ? meta.fallback : input], true);
  }
  ...
  formatStorageSet(key, changes, witCache) {
    ...
    const input = isEmpty || this.treatAsHex(key) ? value : (0, _util.u8aToU8a)(value); // store the retrieved result - the only issue with this cache is that there is no
