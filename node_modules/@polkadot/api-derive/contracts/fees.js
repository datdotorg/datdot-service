...
function parseResult([callBaseFee, contractFee, createBaseFee, creationFee, rentByteFee, rentDepositOffset, tombstoneDeposit, transactionBaseFee, transactionByteFee, transferFee]) {
  return {
    callBaseFee,
    contractFee,
    createBaseFee,
...
function fees(api) {
  return (0, _util.memo)(() => {
    return (api.consts.contracts ? queryConstants(api) : (0, _rxjs.of)([ZERO, ZERO, ZERO, ZERO, ZERO, ZERO, ZERO, ZERO, ZERO, ZERO])).pipe((0, _operators.map)(([callBaseFee, contractFee, createBaseFee, creationFee, rentByteFee, rentDepositOffset, tombstoneDeposit, transactionBaseFee, transactionByteFee, transferFee]) => // We've done this on purpose, i.e. so we can  just copy the name/order from the parse above and see gaps
