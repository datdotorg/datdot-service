...
function queryEnumSet(api) {
  ...
  (0, _operators.switchMap)(next => api.query.indices.enumSet.multi([...Array(next.toNumber() + 1).keys()])), (0, _operators.map)(all => all.reduce((result, list, outerIndex) => {
    (list || []).forEach((accountId, innerIndex) => {
      ...
      result[accountId.toString()] = (0, _types.createType)(api.registry, 'AccountIndex', index);
    });
    return result;
  }, {})));
...
function indexes(api) {
  return (0, _util.memo)(() => api.query.indices ? api.query.indices.accounts ? query() : queryEnumSet(api) : (0, _rxjs.of)({}));
}