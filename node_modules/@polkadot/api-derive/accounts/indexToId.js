...
function queryEnumSet(api, _accountIndex) {
  const accountIndex = _accountIndex instanceof api.registry.createClass('AccountIndex') ? _accountIndex : (0, _types.createType)(api.registry, 'AccountIndex', _accountIndex);
...
function indexToId(api) {
  return (0, _util.memo)(accountIndex => api.query.indices ? api.query.indices.accounts ? query(api, accountIndex) : queryEnumSet(api, accountIndex) : (0, _rxjs.of)(undefined));
}