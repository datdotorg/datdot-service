...
function retrieveProposals(api, section, hashes) {
  ...
  (0, _rxjs.combineLatest)(hashes.map(hash => api.query[section].proposalOf(hash))), api.query[section].voting.multi(hashes)]).pipe((0, _operators.map)((_ref) => {
    let [proposals, votes] = _ref;
    const result = [];
