...
function queryWithQueued(api) {
  return (0, _util.memo)((accountId, queuedKeys) => {
    const stashId = api.registry.createType('AccountId', accountId);
    return (api.query.staking.erasStakers ? retrieveCurr(api, stashId) : retrievePrev(api, stashId)).pipe((0, _operators.switchMap)(result => retrieveController(api, stashId, queuedKeys, result)));
  });
}
/**
...
function query(api) {
  return (0, _util.memo)(accountId => api.query.session.queuedKeys().pipe((0, _operators.switchMap)(queuedKeys => api.derive.staking.queryWithQueued(accountId, queuedKeys))));
}

function queryMulti(api) {
  return (0, _util.memo)(accountIds => accountIds.length ? api.query.session.queuedKeys().pipe((0, _operators.switchMap)(queuedKeys => (0, _rxjs.combineLatest)(accountIds.map(acc => api.derive.staking.queryWithQueued(acc, queuedKeys))))) : (0, _rxjs.of)([]));
}