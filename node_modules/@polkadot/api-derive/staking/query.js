...
function parseResult(api, _ref2) {
  let {
    accountId,
    controllerId,
    stashId,
...
function retrieveInfoV1(api, accountId, stashId, controllerId) {
  return api.queryMulti([[api.query.session.nextKeyFor, controllerId], [api.query.staking.ledger, controllerId], [api.query.staking.nominators, stashId], [api.query.staking.payee, stashId], [api.query.staking.stakers, stashId], [api.query.staking.validators, stashId]]).pipe((0, _operators.map)((_ref3) => {
    let [nextKeyFor, stakingLedger, [nominators], rewardDestination, stakers, [validatorPrefs]] = _ref3;
...
function retrieveInfoV2(api, accountId, stashId, controllerId) {
  return (0, _rxjs.combineLatest)([api.query.session.queuedKeys(), api.queryMulti([[api.query.staking.nominators, stashId], [api.query.staking.payee, stashId], [api.query.staking.stakers, stashId], [api.query.staking.validators, stashId], [api.query.session.nextKeys, [api.consts.session.dedupKeyPrefix, stashId]], [api.query.staking.ledger, controllerId]])]).pipe((0, _operators.map)((_ref4) => {
    let [queuedKeys, [_nominators, rewardDestination, stakers, [validatorPrefs], nextKeys, stakingLedger]] = _ref4;
...
function retrieveV1(api, accountId) {
  ...
  return (0, _rxjs.combineLatest)([api.query.staking.bonded(accountId), api.query.staking.ledger(accountId)]).pipe((0, _operators.switchMap)((_ref5) => {
    let [bonded, stakingLedger] = _ref5;
