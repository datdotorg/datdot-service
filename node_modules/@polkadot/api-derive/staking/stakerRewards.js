...
function filterRewards(api, rewards, stakingLedger, withActive) {
  if (withActive) {
    return (0, _rxjs.of)(rewards);
  }

  const validators = uniqValidators(rewards);
  return (0, _rxjs.combineLatest)([api.query.staking.migrateEra ? api.query.staking.migrateEra() : (0, _rxjs.of)({
    unwrapOr: () => api.tx.staking.payoutStakers ? MIN_ONE : MAX_ERAS
  }), api.tx.staking.payoutStakers ? api.derive.staking.queryMulti(validators) : (0, _rxjs.of)([])]).pipe((0, _operators.map)(([optMigrate, queryValidators]) => {
    const migrateEra = optMigrate.unwrapOr(MIN_ONE);
    return rewards.filter(({
      ...
      validators
    }) => Object.keys(validators).length !== 0).map(reward => _objectSpread({}, reward, {
...
function _stakerRewards(api) {
  return (0, _util.memo)((accountId, _eras, withActive) => api.derive.staking.query(accountId).pipe((0, _operators.switchMap)(({
    stakingLedger,
    stashId
  }) => {
    ...
    const eras = withActive ? _eras : filterEras(_eras, stakingLedger);
    return (0, _rxjs.combineLatest)([api.derive.staking._erasPoints(eras), api.derive.staking._erasPrefs(eras), api.derive.staking._erasRewards(eras), api.derive.staking._stakerExposure(stashId, eras)]).pipe((0, _operators.switchMap)(result => filterRewards(api, parseRewards(api, stashId, result), stakingLedger, withActive)));
  })));
}

function stakerRewards(api) {
  return (0, _util.memo)((accountId, withActive) => api.derive.staking.erasHistoric(withActive).pipe((0, _operators.switchMap)(eras => api.derive.staking._stakerRewards(accountId, eras, withActive))));
}

function stakerRewardsMulti(api) {
  return (0, _util.memo)((accountIds, withActive) => accountIds.length ? api.derive.staking.erasHistoric(withActive).pipe((0, _operators.switchMap)(eras => (0, _rxjs.combineLatest)(accountIds.map(acc => api.derive.staking._stakerRewards(acc, eras, withActive))))) : (0, _rxjs.of)([]));
}