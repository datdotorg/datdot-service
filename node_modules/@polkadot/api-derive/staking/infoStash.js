...
function parse(stashId, [_controllerId, [nominators], rewardDestination, stakers, validatorPrefs, _nextKeys]) {
  return {
...
function infoStash(api) {
  ...
  return (0, _util.memo)(stashId => query(api, stashId).pipe((0, _operators.map)(result => parse(stashId, result))));
}