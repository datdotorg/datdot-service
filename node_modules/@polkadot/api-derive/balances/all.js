...
function calcBalances(api, [{
  ...
  if (Array.isArray(locks)) {
    ...
    if (notAll.length) {
      lockedBalance = (0, _types.createType)(api.registry, 'Balance', (0, _util.bnMax)(...notAll.map(({
        amount
      }) => amount)));
    }
  ...
  const {
    ...
    perBlock,
    startingBlock
  } = vesting || (0, _types.createType)(api.registry, 'VestingInfo');
  const isStarted = bestNumber.gt(startingBlock);
  const vestedBalance = (0, _types.createType)(api.registry, 'Balance', isStarted ? perBlock.mul(bestNumber.sub(startingBlock)) : 0);
...
function queryOld(api, accountId) {
  return api.queryMulti([[api.query.balances.locks, accountId], [api.query.balances.vesting, accountId]]).pipe((0, _operators.map)(([locks, optVesting]) => {
    ...
    if (optVesting.isSome) {
      ...
      vestingNew = (0, _types.createType)(api.registry, 'VestingInfo', {
        locked,
        perBlock,
        startingBlock
      });
...
function queryCurrent(api, accountId) {
  return (api.query.vesting.vesting ? api.queryMulti([[api.query.balances.locks, accountId], [api.query.vesting.vesting, accountId]]) : api.query.balances.locks(accountId).pipe((0, _operators.map)(locks => [locks, (0, _types.createType)(api.registry, 'Option<VestingInfo>')]))).pipe((0, _operators.map)(([locks, optVesting]) => [optVesting.unwrapOr(null), locks]));
}
/**
 * @name all
...
function all(api) {
  return (0, _util2.memo)(address => api.derive.balances.account(address).pipe((0, _operators.switchMap)(account => !account.accountId.isEmpty ? (0, _rxjs.combineLatest)([(0, _rxjs.of)(account), api.derive.chain.bestNumber(), api.query.balances.account ? queryCurrent(api, account.accountId) : queryOld(api, account.accountId)]) : (0, _rxjs.of)([account, (0, _types.createType)(api.registry, 'BlockNumber'), [null, (0, _types.createType)(api.registry, 'Vec<BalanceLock>')]])), (0, _operators.map)(result => calcBalances(api, result))));
}