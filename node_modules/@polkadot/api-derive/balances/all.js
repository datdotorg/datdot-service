...
function calcBalances(api, [accountId, bestNumber, [freeBalance, reservedBalance, locks, vesting], accountNonce]) {
  ...
  if (Array.isArray(locks)) {
    ...
    lockedBreakdown = locks.filter(({
      until
    }) => bestNumber && until.gt(bestNumber)); // get the maximum of the locks according to https://github.com/paritytech/substrate/blob/master/srml/balances/src/lib.rs#L699

    if (lockedBreakdown.length) {
      lockedBalance = (0, _types.createType)(api.registry, 'Balance', (0, _util.bnMax)(...lockedBreakdown.map(({
        amount
      }) => amount)));
    }
...
function all(api) {
  return (0, _util2.memo)(address => api.derive.accounts.info(address).pipe((0, _operators.switchMap)(({
    accountId
  }) => accountId ? (0, _rxjs.combineLatest)([(0, _rxjs.of)(accountId), api.derive.chain.bestNumber(), queryBalances(api, accountId), // FIXME This is having issues with Kusama, only use accountNonce atm
