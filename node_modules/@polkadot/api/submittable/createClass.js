...
function createClass({
  ...
  return _temp = class Submittable extends Extrinsic {
    constructor(registry, extrinsic) {
      ...
      this._sendObservable = (updateId = -1) => {
        return this._api.rpc.author.submitExtrinsic(this).pipe((0, _operators.tap)(hash => {
          this._updateSigner(updateId, hash);
        }));
      };

      this._subscribeObservable = (updateId = -1) => {
        return this._api.rpc.author.submitAndWatchExtrinsic(this).pipe((0, _operators.switchMap)(status => this._statusObservable(status)), (0, _operators.tap)(status => {
    ...
    signAsync(account, optionsOrNonce) {
      return this._decorateMethod(() => {
        ...
        return (0, _rxjs.from)(this._api.signer ? this._signViaSigner(account.address, this._makeSignOptions(optionsWithNonce, {}), null) : Promise.resolve(this.sign(account, optionsWithNonce))).pipe((0, _operators.mapTo)(this));
      })();
    ...
    signAndSend(account, optionsOrStatus, optionalStatusCb) {
      ...
      let updateId;
      return this._decorateMethod(() => this._getPrelimState(address, options).pipe((0, _operators.first)(), (0, _operators.mergeMap)(async ([nonce, header]) => {
    ...
    _statusObservable(status) {
      ...
      return (0, _rxjs.combineLatest)([this._api.rpc.chain.getBlock(blockHash), this._api.query.system.events.at(blockHash)]).pipe((0, _operators.map)(([signedBlock, allEvents]) => new _Result.default({
