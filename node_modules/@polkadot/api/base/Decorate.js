...
class Decorate extends _Events.default {
  ...
  filterRpcMethods(methods) {
    ...
    [...this._rpcCore.mapping.entries()].filter(([key, {
      isOptional
  ...
  decorateStorageEntry(creator, decorateMethod) {
    ...
    const getArgs = (...args) => creator.meta.type.isDoubleMap ? [creator, args] : [creator, ...args]; // FIXME We probably want to be able to query the full list with non-subs as well


    const decorated = this.hasSubscriptions && creator.iterKey && (creator.meta.type.asMap.kind.isLinkedMap || creator.meta.type.asMap.kind.isPrefixedMap) ? creator.meta.type.asMap.kind.isLinkedMap ? this.decorateStorageLinked(creator, decorateMethod) : this.decorateStoragePrefixed(creator, decorateMethod) : decorateMethod((...args) => this.hasSubscriptions ? this._rpcCore.state // Unfortunately for one-shot calls we also use .subscribeStorage here
    ...
    decorated.entries = decorateMethod(() => this.retrieveMapEntries(creator));
  ...
  decorateStorageLinked(creator, decorateMethod) {
    ...
    const getNext = key => this._rpcCore.state.subscribeStorage([[creator, key]]).pipe((0, _operators.switchMap)(([_data]) => {
    ...
    return decorateMethod((...args) => args.length ? this._rpcCore.state.subscribeStorage([[creator, ...args]]).pipe((0, _operators.map)(([data]) => data)) : this._rpcCore.state.subscribeStorage([creator.iterKey]).pipe((0, _operators.switchMap)(([key]) => getNext(head = key))));
  }

  retrieveMapData(creator) {
    (0, _util.assert)(creator.meta.type.isMap, 'entries can only be retrieved on maps');
    const outputType = creator.meta.type.asMap.value.toString();
    return this._rpcCore.state // FIXME This should really be some sort of subscription, so we can get stuff as
    // it changes (as of now it is a one-shot query). Not sure how to do this though...
    .getKeys(creator.iterKey).pipe((0, _operators.switchMap)(keys => (0, _rxjs.combineLatest)([(0, _rxjs.of)(keys), // Not 100% sure about this, surely this is just a vec?
    this._rpcCore.state.subscribeStorage(keys.map(key => key.setOutputType(outputType)))])));
  }

  retrieveMapEntries(creator) {
    return this.retrieveMapData(creator).pipe((0, _operators.map)(([keys, values]) => keys.map((key, index) => [key, values[index]])));
  }

  decorateStoragePrefixed(creator, decorateMethod) {
    return decorateMethod((...args) => args.length ? this._rpcCore.state.subscribeStorage([[creator, ...args]]).pipe((0, _operators.map)(([data]) => data)) : this.retrieveMapData(creator).pipe((0, _operators.map)(([, values]) => values)));
  }
