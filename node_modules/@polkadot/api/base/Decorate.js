...
var _getHasher = _interopRequireDefault(require("@polkadot/metadata/Decorated/storage/fromMetadata/getHasher"));
...
class Decorate extends _Events.default {
  ...
  _decorateStorageEntry(creator, decorateMethod) {
    ...
    decorated.keyPrefix = () => (0, _util2.u8aToHex)(creator.keyPrefix);
  ...
  _retrieveMapKeys({
    ...
    const headKey = this.createType('Raw', (0, _util2.u8aConcat)(iterKey, meta.type.isDoubleMap && !(0, _util2.isUndefined)(arg) && !(0, _util2.isNull)(arg) ? (0, _getHasher.default)(meta.type.asDoubleMap.hasher)(this.createType(meta.type.asDoubleMap.key1.toString(), arg).toU8a()) : new Uint8Array([]))).toHex();
    const startSubject = new _rxjs.BehaviorSubject(headKey);
    return this._rpcCore.state.getKeysPaged ? startSubject.pipe((0, _operators.switchMap)(startKey => this._rpcCore.state.getKeysPaged(headKey, PAGE_SIZE_KEYS, startKey).pipe((0, _operators.map)(keys => keys.map(key => key.decodeArgsFromMeta(meta))))), (0, _operators.tap)(keys => {
    ...
    (0, _operators.map)(keysArr => keysArr.reduce((result, keys) => result.concat(keys), []))) : this._rpcCore.state.getKeys(headKey).pipe((0, _operators.map)(keys => keys.map(key => key.decodeArgsFromMeta(meta))));
  }

  _retrieveMapEntries(entry, arg) {
    const outputType = (0, _StorageKey.unwrapStorageType)(entry.meta.type, entry.meta.modifier.isOptional);
    return this._retrieveMapKeys(entry, arg).pipe((0, _operators.switchMap)(keys => (0, _rxjs.combineLatest)([(0, _rxjs.of)(keys), (0, _rxjs.from)(Array(Math.ceil(keys.length / PAGE_SIZE_VALS)).fill(0)).pipe((0, _operators.concatMap)((_, index) => {
    ...
    }), (0, _operators.toArray)())])), (0, _operators.map)(([keys, valsArr]) => {
      const vals = valsArr.reduce((result, vals) => result.concat(vals), []);
      return keys.map((key, index) => [key, this.createType(outputType, vals[index].isSome ? vals[index].unwrap().toHex() : undefined)]);
    }));
  }
