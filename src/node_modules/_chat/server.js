...
async function init () {
  ...
  wss.on('connection', function connection (ws) {
    ws.on('message', function incoming (message) {
      ...
      const [name, id] = flow
      log({ type: 'chat', data: { text: 'New message', message: JSON.stringify(message) } })
      if (type === 'join') {
        if (!connections[name]) {
          connections[name] = { name, counter: id, ws }
          log({ type: 'chat', data: { text: 'history', history: JSON.stringify(history) } })
          history.forEach(data => {
            log({ type: 'chat', data: { text: 'tell to', name, data: JSON.stringify(data) } })
