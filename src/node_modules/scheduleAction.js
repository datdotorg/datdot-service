...
function blockgenerator ({ blockinterval = 6000, intrinsics, getLogger }, log, emit) {
  ...
  init = setInterval(async () => {
    ...
    while (temp.length) {
      if ((performance.now() - startTime) > (blockinterval - 200)) {
        ...
        const text = `!!! WARNING !!! not able to check  or execute the remaining ${temp.length} intrinsics in scheduler during blockinterval`
        console.log(text)
      ...
      if (!id) {
        ...
        log({ type: 'block', data: [`Executing scheduled intrinsic: ${type}`] })
        const intrinsic = intrinsics[type]
        await intrinsic(getLogger(from), data)
      }
