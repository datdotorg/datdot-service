...
const make_message = require('message-maker')

module.exports = datdot_explorer

function datdot_explorer ({ addresses }) {
  const recipients = []
  const make = make_message('datdot-explorer')
  const logs = terminal({mode: 'compact', expanded: false}, protocol('logs'))
  const info = old(addresses, protocol('logs-stream'))
  const status_footer = footer({page: 'explorer', name: 'explorer-status-footer'}, protocol('explorer-status-footer'))
  const app = bel`<div class="${css.wrap}" data-state="debug">${status_footer}</div>`
  setTimeout(() => {app.append(info.logs)}, 2000)
  return app

  function terminal_layout_changed (args) {
    const result = args.filter( i => i.selected )[0]
    const text = result.text.split(' ')[0].toLowerCase()
    recipients['logs-stream'](make({type: 'layout-mode', data: {mode: text}}))
  }
  ...
  function get (msg) {
    ...
    const to = head[1] !== undefined ? head[1].split('/')[0].trim() : undefined
    const role = head[0].split(' / ')[1]
    recipients['logs'](msg)
    if (type === 'layout-mode') return recipients[to](make({type, data}))
    if (type === 'selected') return terminal_layout_changed(data.selected)
  }
}
