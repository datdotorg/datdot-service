...
function asdf (addresses, protocol) {
  const recipients = []
  const send = protocol(get)
  const make = make_message(`logs-stream`)
  window.log = console['log'].bind('console')
  const logs = terminal({mode: 'compact', expanded: false}, log_protocol('logs'))
  ...
  function start () {
    ...
    send(make({to: 'explorer-status-footer', type: 'total', data: 0}))
  ...
  return {logs, status: show()}

  function log_protocol (name) {
    return send => {
  ...
  function get (msg) {
    ...
    if (type === 'layout-mode') recipients['logs'](make({type, data}))
    recipients['logs'](msg)
  }
}
