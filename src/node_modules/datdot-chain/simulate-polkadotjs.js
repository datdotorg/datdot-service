  ...
  create: async ({ name, provider }) => {
    ...
    function connect (resolve, reject) {
      ...
      ws.on('open', function open () {
        ...
        console.log(`[${name}] connected to chain`)
        if (instance.backlog) {
          if (instance.backlog.length) {
            console.log(`[${name}] flushing message buffer to chain`)
          ...
          instance = { name, counter: 0, ws, send, api }
          resolve(api)
        }
      })
      ws.on('error', function error (err) {
        console.error({ error })
        console.error(`[${name}:error] disconnecting from chain`)
        ws.close('custom close')
      })
      ws.on('close', function close (data) { reconnect(data) })
    }
    function reconnect (data) {
      console.log({ data })
      console.log(`[${name}] disconnected from chain`)
      console.log(`[${name}] trying to reconnect to chain`)
      instance.backlog = []
      instance.send = (...args) => {
        console.log(`[${name}] buffering chain message`)
        instance.backlog.push(args)
        if (instance.backlog.length > 100) {
          console.error(`[${name}:WARNING] too many message without connection to chain`)
        }
      }
      const miliseconds = randomIntFromInterval(500, 1500)
      setTimeout(() => { // @TODO: implement backoff strategy
        connect(() => {}, err => reconnect(err))
      }, miliseconds)
    }
  }
}
