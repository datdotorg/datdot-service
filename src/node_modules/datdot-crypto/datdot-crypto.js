...
async function verify_hash (index, nodes, data) {  
  return new Promise(async (resolve, reject) => {
    const node = nodes.find(node => node.index === index*2)
    const data_hash = hash(b4a.from(data))
    console.log({node: node.hash, data_hash })
    const v = b4a.compare(node.hash, data_hash) // returns 0
