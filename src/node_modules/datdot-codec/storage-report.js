...
function decode (message, log) {
  try {
    ...
    var { storageChallengeID, proof_of_contact, reports } = data
    log({ type: 'codec', data: { text: `DECODING MESSAGE`, data: message.args[0] }, reports  })
    data.proof_of_contact = Buffer.isBuffer(proof_of_contact) ? proof_of_contact : b4a.from(proof_of_contact, 'hex')
    
    for (var i = 0, len = data.reports.length; i < len; i++) {
      const p = data.reports[i].p
