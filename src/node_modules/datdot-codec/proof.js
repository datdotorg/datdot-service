...
function decode (data) {
  try {
    ...
    var { type, index, encoded_data, encoded_data_signature, p } = message
...
function to_string (p) {
  ...
  if (p.hash) p.hash.nodes.forEach(node => { node.hash = node.hash.toString('hex') })
...
function to_buffer (p) {
  ...
  p = JSON.parse(p)
  if (p.hash) p.hash.nodes.forEach(node => { node.hash = b4a.from(node.hash, 'hex') })
  if (p.upgrade) {
