...
async function datdotChain (profile) {
  ...
  log({ type: '@todo', data: 'allow `datdot-chain` to spawn new or connect to existing chain node' })

  // provider = new WsProvider(`${address}:${port}`)
  const API = await rerun(() => ApiPromise.create({ name, provider, types, log }))
  const chainAPI = {
    getBlockNumber,
    newUser,
    registerForWork,
  ...
  async function submitPerformanceChallenge (opts) {
    const { performanceChallengeID, reports, signer, nonce } = opts
    const tx = await API.tx.datVerify.submitPerformanceChallenge(performanceChallengeID, reports)
    // tx.signAndSend(signer, await makeNonce(nonce))
    tx.signAndSend(signer, await makeNonce(nonce), status)
  }
  // LISTEN TO EVENTS
