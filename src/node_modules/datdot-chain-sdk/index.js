...
async function datdotChain (profile) {
  ...
  async function hostingSetupReport (opts) {
    const { report, signer, nonce } = opts
    const tx = await API.tx.datVerify.hostingSetupReport(report)
    // tx.signAndSend(signer, await makeNonce(nonce))
    tx.signAndSend(signer, await makeNonce(nonce), status)
  }
  async function hosterReplacementReport (opts) {
    const { report, signer, nonce } = opts
    const tx = await API.tx.datVerify.hosterReplacementReport(report)
    // tx.signAndSend(signer, await makeNonce(nonce))
    tx.signAndSend(signer, await makeNonce(nonce), status)
  }
