...
const hyper_store = require('../_datdot-service-helpers/hyper')
...
const sponsor_plan = require('datdot-simulate-user/behavior/sponsor-plan')
...
async function user ({name, behavior}, config, logport) {
  ...
  try {
    ...
    vaultAPI.name = name
    vaultAPI.log = log
    vaultAPI.config = config
    const hyper = hyper_store(vaultAPI, bootstrap)
    vaultAPI.hyper = hyper
    log({ type: 'user', data:  { text: 'hyper store' } })
    const chainAPI = await datdot_chain(vaultAPI)
    log({ type: 'user', data:  { text: 'chainAPI' } })
    const serviceAPI = await datdot_service(vaultAPI, { chainAPI })
    ...
    for (var i = 0, len = behavior.length; i < len; i++) {
      ...
      if (behavior_name === 'register_chain_account') {
        await register_chain_account(vaultAPI, { chainAPI })
      } else if (behavior_name === 'author_hypercore') {
        await author_hypercore(vaultAPI, { chainAPI })
      } else if (behavior_name === 'sponsor_plan') {
        await sponsor_plan(vaultAPI, { chainAPI })
      } else if (behavior_name === 'offer_service') {
        await offer_service(vaultAPI, { serviceAPI, chainAPI })
      }
    }
  } catch (error) {
...
function captureErrors (log) {
  ...
  process.on('exit', (res) => log({ type: 'user', data: { text: 'exiting now', res } }))
}
