...
async function author_hypercore (vaultAPI, APIS) {
  const { log, config, hyper } = vaultAPI
  const { chainAPI } = APIS
  ...
  const tid = setTimeout(() => {
    log({ type: 'timeout', data: { texts: 'error: author - timeout' } })
    return
  }, DEFAULT_TIMEOUT)

  const { feed } = await hyper.new_task({ log })
  ...
  await hyper.connect({
    feed,
    swarm_opts: { role: 'author', topic, mode: { server: true, client:false } },
    onpeer,
    done,
    log
  })
  ...
  setTimeout(() => { // TODO: - figure out why we need to wait
    ...
    clearTimeout(tid)
    log({ type: 'author', data: { text: `Send the keys ${JSON.stringify(keys)}` } })
  }, 3000)

  function onpeer ({ peerkey, stringtopic }) {
    log({ type: 'author', data: { text: `onpeer callback`, stringtopic, peerkey } })
    remotestringkey = peerkey.toString('hex')
  }

  async function done ({ peerkey }) {
    const pubkey = account.noisePublicKey.toString('hex')
    await done_task_cleanup({ role: 'author', remotestringkey: peerkey, topic, state: account.state[pubkey], log })                   
  }
