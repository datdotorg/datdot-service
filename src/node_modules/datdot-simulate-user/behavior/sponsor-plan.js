...
async function sponsor_plan (profile, APIS) {
  ...
  async function getProofs (components, datasets) {
    ...
    for (var i = 0, len = datasets.length; i < len; i++) {
      all.push(new Promise(async (resolve, reject) => {
        ...
        // make feed
        const { feed } = await store.load_feed({ feedkey: keybuff, log })
        log({ type: 'sponsor', data: { text: `feedkey`, feedkey: feed.key.toString('hex'), discovery: feed.discoveryKey} })    
        
        // connect to author
        await store.connect({
          swarm_opts: { topic: feed.discoveryKey, mode: { server: false, client: true } },
          log
        })
        ...
        await remove_task_from_cache({ store, topic: feed.discoveryKey, cache: account.cache, log })
        resolve()
