...
async function hosting_plan (profile, APIS) {
  ...
  chatAPI.on(async keys => {
    log({ type: 'sponsor', data: [`Got the keys, publishing data now => ${JSON.stringify(keys)}`] })
  ...
  async function getProofs (components, datasets) {
    ...
    for (var i = 0, len = datasets.length; i < len; i++) {
      all.push(new Promise(async (resolve, reject) => {
        ...
        log({ type: 'sponsor', data: { text: `feedkey and stringkey`, feedkey: feedkey.toString('hex')} })        

        const role = 'sponsor'
        await load_feed({ role, task_id, swarmAPI, chainAPI, account, feedkey, next, log })
        
        async function next ({ feed, log }) {
          // await swarmAPI.replicate({ account, socket, role, feed, log })
          await feed.update()
          const v = feed.length - 1
          log({ type: 'sponsor', data: { text: `Feed version`, v } })      
          // another check if this signature is already on chain
          if (signatures[v]) return
          console.log({ CORE: feed.core, TREE: feed.core.tree.crypto.data })
          const signature = await feed.core.blocks.tree.signature
          log({ type: 'sponsor', data: { text: `Got a signature`, signature } })      
          // const nodes = await feed.core.tree.roots
          const nodes = await get_nodes(feed, v, v)
          log({ type: 'sponsor', data: { text: `Got nodes`, nodes } })      
