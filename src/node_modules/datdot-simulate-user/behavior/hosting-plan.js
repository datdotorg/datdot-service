...
const HosterStorage = require('../../../roles/hoster/hoster-storage.js')
...
//  }
  ...
  async function getProofs (components, datasets) {
    ...
    for (var i = 0, len = datasets.length; i < len; i++) {
      all.push(new Promise(async (resolve, reject) => {
        ...
        log({ type: 'sponsor', data: { text: `feedkey and stringkey`, feedkey, stringkey } })

        // var feed = new hypercore(RAM, feedkey, { valueEncoding: 'binary' })
        // log({ type: 'sponsor', data: { text: `New feed`, stringkey } })
        // await ready(feed)
        ...
        if (account.storages.has(stringkey)) {
          ...
          feed = storage.feed
          log({ type: 'sponsor', data: { text: `Existing feed`, amendmentID, stringkey } })
        } else {
          feed = new hypercore(RAM, feedkey, { valueEncoding: 'binary' })
          log({ type: 'sponsor', data: { text: `New feed`, stringkey } })
          await ready(feed)
        ...
        async function onconnection (socket) {
          log({ type: 'sponsor', data: [`onconnection`] })
          if (!replicates[feedkey.toString('hex')]) {
            replicates[feedkey.toString('hex')] = true
            ...
            finished(stream, (err) => {
              if (err) {
                ...
                try { socket.end() }
                catch(err) { log({ type: 'sponsor', data: { text: `Closing socket error`, err } }) }
              }
            })
            ...
            feed.on('sync', async () => {
              ...
              const v = feed.length - 1
              // another check if this signature is already on chain 
              if (signatures[v]) return
              const signature = await get_signature(feed, v)
              const nodes = await get_nodes(feed, v, v)
              proofs.push({ feed_ref, signature, nodes })
              log({ type: 'sponsor', data: { text: `Hypercore replicated`, isInitiator: socket.isInitiator, proofs } })
              resolve()
            })
          }
        }
  ...
  async function getKeyAndSignature (dataset_ref, components) {
    ...
    if (signatures.length) {
      ...
      return { signatures, feedkey: Buffer.from(feedkey, 'hex'), sig: { index, signature: feed.signatures[index] }, feed_ref }
    } else {
      return { signatures, feedkey: Buffer.from(feedkey, 'hex'), feed_ref }
    }
  }
