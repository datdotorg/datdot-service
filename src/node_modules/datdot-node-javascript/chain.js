...
async function performance_challenge_timeout (log, data) {
  ...
  attestorIDs.forEach(id => removeJob({ id, role: 'attestor', doneJob: cid, idleProviders: DB.status.idleAttestors, action: () => tryNextChallenge({ id, log }) }, log))
...
async function amendment_timeout (log, data) {
  ...
  const { id } = data
  const { providers } = getAmendmentByID(id)
  const failed = providers.flat()
...
async function make_performance_challenge (log, data) {
  ...
  const idleProviders = DB.status.idleAttestors
  const attestors = select({ idleProviders, role: 'attestor', newJob, amount: 1, avoid, plan: {}, log })
  ...
  log({ type: 'challenge', data: {text:'Making new performance chalenge', items: {challenge: type, newJob, attestors, cid: id} }})
  feed.challenge = {
    status: 'active',
    timestamp: performance.now(),
...
async function _amendment_report (user, { name, nonce }, status, args) {
  ...
  if (failed.length) _retry_amendment({ failed, amendmentID, log })
    ...
    encoders.forEach(id => removeJob({ id, role: 'encoder', doneJob: amendmentID, idleProviders: DB.status.idleEncoders, action: () => try_next_amendment(log) }, log))
    attestors.forEach(id => removeJob({ id, role: 'attestor', doneJob: amendmentID, idleProviders: DB.status.idleAttestors, action: () => try_next_amendment(log) }, log))
    
    for (var i = 0, len = hosters.length; i < len; i++) {
      const user = getUserByID(hosters[i])
      const jobs = Object.keys(user.hoster.jobs).map(job => Number(job))
      log(`Hosting started: contract: ${contractID}, amendment: ${amendmentID}, hoster: ${hosters[i]}, jobs: ${jobs}, challenges: ${JSON.stringify(user.hoster.challenge)}`)
...
async function _retry_amendment ({ failed, amendmentID, log }) {
  ...
  const doneJob = amendmentID
  for (var i = 0, len = failed.length; i < len; i++) {
    const id = failed[i]
    if (attestors.includes(id)) {
      attestors.splice(attestors.indexOf(id), 1)
      attestors.forEach(id => removeJob({ id, role: 'attestor', doneJob, idleProviders: DB.status.idleAttestors, action: () => try_next_amendment(log) }, log))
    }
    else if (hosters.includes(id)) {
      hosters.splice(hosters.indexOf(id), 1)
      removeJob({ id, role: 'hoster', doneJob, idleProviders: DB.status.idleHosters, action: () => try_next_amendment(log) }, log)
    }
    else if (encoders.includes(id)) {
      encoders.splice(encoders.indexOf(id), 1)
      removeJob({ id, role: 'encoder', doneJob, idleProviders: DB.status.idleEncoders, action: () => try_next_amendment(log) }, log)
    }
  }
