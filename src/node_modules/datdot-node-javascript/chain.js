...
async function _publish_plan (user, { name, nonce }, status, args) {
  ...
  if (!store_root_signatures(proofs, feed_ids, log)) return log({ type: 'chain', data: { text: 'Plan can not be published, invalid proof' }})
...
function store_root_signatures (proofs, feed_ids, log) {
  proofs.map(({ feed_ref, signature, nodes }, i) => {
    ...
    if (not_verified) return log({ type: 'chain', data: { text: 'Error: Proof could not be verified' } })
