...
async function storage_challenge_timeout (log, data) {
  const {user} = data
  var { status, timestamp, cid } = user.hoster.challenges.storage
  user.hoster.challenges.storage.status = 'timed-out'
  log({ type: 'storage-challenge-timeout', data: {challenges: JSON.stringify(user.hoster.challenges.storage), perf: performance.now() - timestamp} })
...
async function _amendment_report (user, { name, nonce }, status, args) {
  ...
  if (!failed.length) {
    ...
    for (var i = 0, len = hosters.length; i < len; i++) {
      ...
      const jobs = Object.keys(user.hoster.jobs).map(job => Number(job))
      log(`Hosting started: contract: ${contractID}, amendment: ${amendmentID}, hoster: ${hosters[i]}, jobs: ${jobs}, challenges: ${JSON.stringify(user.hoster.challenges.storage)}`)
      // schedule challenges for each hoster only once
      if (!user.hoster.challenges.storage) {
        user.hoster.challenges.storage = { status: 'pending' }
        await start_new_storage_challenge(user, log)
      }
    }
    encoders.forEach(id => {
...
async function _storage_challenge_report (attestor, { name, nonce }, status, args) {
  ...
  var { status, cid, sid } = challenges.storage
  if (attestor.id !== attestorID) {
    challenges.storage.status = 'failed'
    await start_new_storage_challenge(hoster, log)
  ...
  log({ type: 'info', data: { status, cid, sid  } })

  if (challenges.storage.cid !== storageChallengeID) {
    challenges.storage.status = 'failed'
    await start_new_storage_challenge(hoster, log)
    return log({ type: 'challenge fail', data: `Not an active storage challenge: CID ${cid}/ID ${storageChallengeID},  challenge_CID: ${JSON.stringify(getItemByID(cid))} challenge_challengeID: ${JSON.stringify(getItemByID(storageChallengeID)) }` })
  }

  evaluate_storage_challenge_report({ storageChallenge, reports, log })
  challenges.storage.status = 'done'
  log({ type: 'challenge done', data: `Storage challenge confirmed ${JSON.stringify(challenges.storage)}` })
  await start_new_storage_challenge(hoster, log)
...
async function start_new_storage_challenge (user, log) {
  var { status, cid, sid } = user.hoster.challenges.storage
