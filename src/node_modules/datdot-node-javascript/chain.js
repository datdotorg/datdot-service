...
async function _performance_challenge_report (user, { name, nonce }, status, args) {
  ...
  for (var i = 0, len = report.length; i < len; i++) {
    const { stats, hoster: hoster_id, event_sig } = report[i]
    if (!stats || !event_sig || !datdot_crypto.verify_signature(event_sig, Buffer.from(`${performanceChallengeID}`, binary), getUserByID(hoster_id).signingKey)) {
      status: 'fail'
      log({ type: 'info', data: { text: `Error: Performance challenge for hoster: ${hoster_id} failed` } })
    }
    if (report) log({ type: 'info', data: { text: `Performance challenge confirmed for hoster: ${hoster_id}` } })
    if (!status) status = 'done'
    unbook_and_rate({ id: userID, role: 'attestor', task: performanceChallengeID, type: 'performance', status: 'done', log })
    unbook_and_rate({ id: hoster_id, role: 'hoster', task: performanceChallengeID, status, type: 'performance', log })
  }
}
...
// async function unbook_and_rate ({ providers, taskID }, log) {
...
function unbook_and_rate ({ id, role, task, status, type, opts, log }) {
  ...
  if (user[role].tasks[task]) {
    ...
    if (role === 'hoster') {
      ...
      else if (type === 'performance') reward = 40
    }
    ...
    if (status === 'cancel' || 'undefined') {} // do nothing
    next_task(log)
  }
}
