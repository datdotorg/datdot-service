  ...
  const scheduler = blockgenerator({ blockinterval, intrinsics }, log.sub('blockgenerator'), async blockMessage => {
    ...
    async function emitBlock () {
      ...
      const promises = Object.entries(connections).map(([name, { ws, handler }]) => new Promise((resolve, reject) => {
        ...
        eventpool.forEach(([log, message]) => { handler(message) })
        resolve()
      }))
...
async function _amendment_report (user, { name, nonce }, status, args) {
  ...
  if (!failed.length) {
    ...
    for (var i = 0, len = hosters.length; i < len; i++) {
      ...
      schedule_new_storage_challenge(user, log)
    }
    encoders.forEach(id => {
...
async function schedule_new_storage_challenge (user, log) {
  ...
  log({ type: 'info', data: { text: 'NEW CHALLENGE',  storageChallenge: challenges.storage } })
  ...
  challenges.storage = {
    ...
    sid: scheduleAction({ 
      ...
      from: log, data: {user}, delay: 2, type: 'storage_challenge_timeout' 
    }),
    // make new challenge
