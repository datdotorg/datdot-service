...
async function init () {
  ...
  wss.on('connection', function connection (ws, req) {
    ...
    console.log('Got connection in chain.js', ip)
    ws.on('message', async function incoming (message) {
      ...
      if (method) {
        ...
        if (result === undefined) return log({ type: 'chain', data: { text: 'Query not found', type, flow } })
...
function getUserIDByNoiseKey(key) {
  ...
  console.log('Got user by Noise key', id)
  return id
}
...
async function storage_challenge_timeout (log, data) {
  ...
  const { hoster: hoster_id, attestor: attestor_id} = storageChallenge
  unbook_and_rate({ id: attestor_id, role: 'attestor', task: cid, type: 'storage', status: 'fail', log })
...
async function performance_challenge_timeout (log, data) {
  ...
  attestorIDs.forEach(id => unbook_and_rate({ id, role: 'attestor', task: cid,  type: 'performance', status: 'fail', log }))
  unbook_and_rate({ id: attestor_id, role: 'attestor', task: cid, type: 'performance', status: 'fail', log })
...
async function make_storage_challenge (log, data) {
  ...
  book({ type, selectedProviders, role: 'attestor', newTask: id, log })
  book({ type, selectedProviders: [{ id: hoster_id }], role: 'hoster', newTask: id, log })
  //
  log({ type: 'info', data: { text: 'Making new storage challenge for', items: { type, newTask: id, hoster_id, amendments, cid: id } } })
  user.hoster.challenge = {
    status: 'active',
    timestamp: performance.now(),
    sid: scheduleAction({ from: log, data: {user}, delay: 5, type: 'storage_challenge_timeout' }), 
    cid: id 
  }
  // emit event
...
async function make_performance_challenge (log, data) {
  ...
  for (var i = 0, len = contractIDs.length; i < len; i++) {
    ...
    performanceChallenge.hosters = hosterIDs
  }
  ...
  book({ type, selectedProviders: attestors, role: 'attestor', newTask: id, log })
  book({ type, selectedProviders: hosterIDs.map(id => { id }), role: 'hoster', newTask: id, log })
  log({ type: 'challenge', data: {text:'Making new performance chalenge', items: {challenge: type, newTask: id, attestors, cid: id} }})
  feed.challenge = {
    status: 'active',
    timestamp: performance.now(),
    sid: scheduleAction({ from: log, data: {feedID}, delay: 5, type: 'performance_challenge_timeout' }), 
    cid: id 
  }
  // emit event
...
async function _publish_plan (user, { name, nonce }, status, args) {
  ...
  if (!planValid({ plan })) return log({ type: 'info', data: [`Error:  Plan from and/or until are invalid`] })
  ...
  take_next_plan_from_priority(priority_queue.take(), log) // schedule the plan execution
}

async function unpublishPlan (user, { name, nonce }, status, args) {
  const [planID] = args
  const plan = getPlanByID(planID)
  if (!plan.sponsor === user.id) return log({ type: 'info', data: [`Error:  Only a sponsor is allowed to unpublish the plan`] })
...
async function _amendment_report (user, { name, nonce }, status, args) {
  ...
  if (user.id !== attestorID) return log({ type: 'info', data: { text: `Error: this user can not submit the attestation`, providers: JSON.stringify(amendment.providers), user: user.id } })
  ...
  for (var i = 0, len = hosters.length; i < len; i++) {
    ...
    const sig_buf = Buffer.from(sig)
    const data = Buffer.from(`${amendmentID}/${i}`, 'binary')
    const signingKey  = Buffer.from(getUserByID(hoster_id).signingKey)
    log({ type: 'chain', data: { text: 'Verifying signatures', sig_buf, data, signingKey } })
    if (!datdot_crypto.verify_signature(sig_buf, data, signingKey)) return log({ type: 'info', data: { text: `Error: unique_el_signature for hoster: ${hoster_id} could not be verified` } })
  }
  log({ type: 'info', data: { text: `amendmentReport hoster signatures verified`, amendmentID } })
  ...
  attestors.forEach(id => unbook_and_rate({ id, role: 'attestor', task: amendmentID,  type: 'setup', status: 'done', log }))
  ...
  //   await start_new_performance_challenge(feed.id, log)
  // }
...
async function _storage_challenge_report (attestor, { name, nonce }, status, args) {
  ...
  const { storageChallengeID,  storage_challenge_signature, reports } = response
  const storageChallenge = getStorageChallengeByID(storageChallengeID)
  const { attestor: attestorID, hoster: hoster_id, checks } = storageChallenge
  const user = getUserByID(hoster_id)
  ...
  log({ type: 'storage challenge', data: { text: `Received report`, storageChallengeID, fields: Object.keys(response), status, cid, sid } })
  // is it a valid report
  if (attestor.id !== attestorID || user.hoster.challenge.cid !== storageChallengeID ) {
    return log({ type: 'storage challenge', data: { text: `Error: This not a valid challenge`, storageChallengeID } })
  }
  // attestor failed, start a new challenge
  if (!reports.length || !datdot_crypto.verify_signature(storage_challenge_signature, messageBuf, signingKeyBuf)) { 
    user.hoster.challenge.status = 'fail'
    unbook_and_rate({ id: attestorID, role: 'attestor', task: storageChallengeID, type: 'storage', status: 'fail', log })
    await start_new_storage_challenge(user, log)
    return log({ type: 'storage challenge', data: { text: `Error: This challenge's report is empty`, storageChallengeID } })
  }
  // attestor succeeded, evaluate report & start a new challenge
  unbook_and_rate({ id: attestorID, role: 'attestor', task: storageChallengeID, type: 'storage', status: 'done', log })
  var { status, all, proved } = analyse_storage_proof({ checks, reports, log })
  ...
  log({ type: 'storage challenge', data: { text: `Storage challenge analysis`, status } })
  await start_new_storage_challenge(user, log)
}

function  analyse_storage_proof ({ checks, reports, log }) {
  log({ type: 'storage challenge', data: { text: `Analysing report`, reports } })
  ...
  for (var i = 0, len = reports.length; i < len; i++) {
    ...
    const { feedkey, signatures } = getFeedByID(getContractByID(contractID).feed)
    const not_verified = datdot_crypto.merkle_verify({
      feedKey: Buffer.from(feedkey, 'hex'), 
      hash_index: checks[contractID].index * 2, 
      version, 
      signature: Buffer.from(signatures[version], 'binary'), 
      nodes
    })
    if (not_verified) continue
    proved++
  }
...
async function start_new_storage_challenge (user, log) {
  ...
  scheduleAction({ from: log, data: {hoster_id: user.id}, delay: 2, type: 'make_storage_challenge' })
}
/*----------------------
...
async function start_new_performance_challenge (feedID, log) {
  ...
  scheduleAction({ from: log, data: {feedID}, delay: 2, type: 'make_performance_challenge' })
}
...
async function _performance_challenge_report (user, { name, nonce }, status, args) {
  ...
  const userID = user.id
  log({ type: 'performance challenge', data: { text: `Submitting report`, challenge_id, userID } })
  const { attestors, feed: feed_id } = getPerformanceChallengeByID(challenge_id)
  const [attestor_id] = attestors
  const feed = getFeedByID(feed_id)
  const { challenge: { cid }, contracts: contractIDs } = feed
  if (attestor_id !== userID || cid !== challenge_id) {
    return log({ type: 'performance challenge', data: { text: `Error: This not a valid challenge`, challenge_id } })
  }
  // attestor failed, start a new challenge
  if (!reports.length) { 
    feed.challenge.status = 'fail'
    unbook_and_rate({ id: attestorID, role: 'attestor', task: challenge_id, type: 'performance', status: 'fail', log })
    await start_new_performance_challenge(feed_id, log)
    return log({ type: 'performance challenge', data: { text: `Error: This challenge's report is empty`, challenge_id } })
  }

  for (var i = 0, len = reports.length; i < len; i++) {
    const { stats, hoster: hoster_id, event_sig } = reports[i]
    if (!stats || !event_sig || !datdot_crypto.verify_signature(event_sig, Buffer.from(`${challenge_id}`, binary), getUserByID(hoster_id).signingKey)) {
      feed.challenge.status = 'fail'
      unbook_and_rate({ id: attestorID, role: 'attestor', task: challenge_id, type: 'performance', status: 'unknown', log })
    ...
    unbook_and_rate({ id: userID, role: 'attestor', task: challenge_id, type: 'performance', status: 'done', log })
    unbook_and_rate({ id: hoster_id, role: 'hoster', task: challenge_id, status, type: 'performance', opts: { grade }, log })
    await start_new_performance_challenge(feed_id, log)
  }
}
...
function store_root_signatures (proofs, feed_ids, log) {
  proofs.map(({ feed_ref, signature, nodes }, i) => {
    ...
    if (not_verified) return log({ type: 'info', data: { text: 'Error: Proof could not be verified' } })
...
async function take_next_plan_from_priority (next, log) {
...
async function init_amendment (contractID, reuse, log) {
  ...
  const contract = getContractByID(contractID)
  if (!contract) return log({ type: 'info', data: `Error: No contract with this ID: ${contractID}` })
...
function book ({ type, selectedProviders, role, newTask, log }) {
  log({ type: 'info', data: { text: `Start task`, type, selectedProviders, role, newTask } })
...
function unbook_and_rate ({ id, role, task, status, type, opts, log }) {
  ...
  // log({ type: 'info', data: { text: `Unbooking and rating`,  type, task, role, status } })
  if (!user[role].tasks[task]) return log({ type: 'info', data: { text: `Error: Not a valid task`, task, role, type, user: JSON.stringify(user) } })
  var reward
  var grade
  ...
  if (role === 'hoster') {
    if (type === 'storage') {
      ...
      const { grade } = opts // grade = expected vs. delivered performance
      if (status === 'done') {
    ...
    grade = 1
    delete user[role].tasks[task]
    if (!DB.status.idleAttestors.includes(id)) DB.status.idleAttestors.push(id)
...
async function next_task (log) {
  ...
  for (var start = new Date(); DB.queues.tasks.length && (new Date() - start < 4000);) {
    ...
    log({ type: 'attestors task queue', data: { text: 'next task', fnName, temp_queue: temp_queue.length, len: DB.status.idleAttestors.length, task: JSON.stringify(task), tasks: JSON.stringify(DB.queues.tasks) } })
    if (fnName === 'amendment' && DB.status.idleAttestors.length && DB.status.idleEncoders.length >= 3 && DB.status.idleHosters.length >= 3) {
      ...
      if (providers) {
        ...
        log({ type: 'info', data: { text: `Providers for amendment (${task_id})`,  providers} })
      ...
      if (selectedProviders.length) {
        ...
        book({ type: fnName, selectedProviders, role: 'attestor', newTask: id, log })
        log({ type: 'storage', data: { text: 'Booked attestor', task: id } })
...
async function _retry_amendment ({ failed, amendmentID, log }) {
  return
  log({ type: 'info', data: { text: 'Retrying amendment', failed, amendmentID } })
  ...
  for (var i = 0, len = failed.length; i < len; i++) {
    ...
    if (attestors.includes(id)) {
      ...
      unbook_and_rate({ id, role: 'attestor', task, type: 'setup', status: 'fail', log })
    }
  ...
  log({ type: 'info', data: { text: 'Retry amendment', providers: reuse, newID } })
  // TODO ACTION find new provider for the contract (makeAmendment(reuse))
