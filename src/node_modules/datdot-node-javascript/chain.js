...
async function storage_challenge_timeout (log, data) {
  ...
  remove_task_and_reward({ id: attestorID, role: 'attestor', task: cid, idleProviders: DB.status.idleAttestors, status: 'fail', log })
...
async function performance_challenge_timeout (log, data) {
  ...
  attestorIDs.forEach(id => remove_task_and_reward({ id, role: 'attestor', task: cid, idleProviders: DB.status.idleAttestors, status: 'fail', log }))
...
async function make_storage_challenge (log, data) {
  ...
  start_task({ type, selectedProviders, role: 'attestor', newTask: id, log })
  //
...
async function make_performance_challenge (log, data) {
  ...
  start_task({ type, selectedProviders: attestors, role: 'attestor', newTask: id, log })
...
async function _amendment_report (user, { name, nonce }, status, args) {
  ...
  if (failed.length) _retry_amendment({ failed, amendmentID, log })
    ...
    encoders.forEach(id => remove_task_and_reward({ id, role: 'encoder', task: amendmentID, idleProviders: DB.status.idleEncoders, status: 'done', log }))
    attestors.forEach(id => remove_task_and_reward({ id, role: 'attestor', task: amendmentID, idleProviders: DB.status.idleAttestors, status: 'done', log }))
...
async function _storage_challenge_report (attestor, { name, nonce }, status, args) {
  ...
  remove_task_and_reward({ id: attestorID, role: 'attestor', task: storageChallengeID, idleProviders: DB.status.idleAttestors, status: 'done', log })
...
function  evaluate_storage_challenge_report ({ storageChallenge, reports, log }) {
  ...
  for (var i = 0, len = reports.length; i < len; i++) {
    ...
    // reward_hoster({ type: 'storage', id: storageChallenge.hoster })
...
async function _submitPerformanceChallenge (user, { name, nonce }, status, args) {
  ...
  remove_task_and_reward({ id: userID, role: 'attestor', task: performanceChallengeID, idleProviders: DB.status.idleAttestors, status: 'done', log })
  // hosters.forEach(hoster => reward_hoster({ type: 'performance', id: hoster.id }))
  // emit event
...
function start_task ({ type, selectedProviders, role, newTask, log }) {
...
// async function remove_task_and_reward ({ providers, taskID }, log) {
...
function remove_task_and_reward ({ id, role, task, idleProviders, status, log }) {
  ...
  if (user[role].tasks[task]) {
    ...
    if (status === 'done') {
      if (role === 'attestor') user.balance += 5 
      if (role === 'encoder') user.balance += 3 
      user.rating++ 
    }
    if (status === 'fail') { user.rating--}
    if (status === 'cancel') {}
    next_task(log)
  }
}
    ...
    if (fnName === 'amendment' && DB.status.idleAttestors.length && DB.status.idleEncoders.length >= 3 && DB.status.idleHosters.length >= 3) {
      ...
      if (providers) {
        ...
        ;['attestor','encoder','hoster'].forEach(role => {
          start_task({ type, selectedProviders: providers[`${role}s`], role, newTask: task_id, log })
        })
      ...
      if (selectedProviders.length) {
        ...
        start_task({ type: fnName, selectedProviders, role: 'attestor', newTask: id, log })
        // emit event
...
// if (DB.active.storageChallenges[id] ) const challenge = getStorageChallengeByID(storageChallengeID)
    ...
    hoster_ids.forEach((hoster_id, i) => {
      remove_task_and_reward({ id: hoster_id, role: 'hoster', task: contractID, idleProviders: DB.status.idleHosters, status: 'cancel', log })
      const { feed: feedID } = getContractByID(contractID)
...
async function _retry_amendment ({ failed, amendmentID, log }) {
  ...
  for (var i = 0, len = failed.length; i < len; i++) {
    ...
    if (attestors.includes(id)) {
      ...
      remove_task_and_reward({ id, role: 'attestor', task, idleProviders: DB.status.idleAttestors, status: 'fail', log })
    }
    else if (hosters.includes(id)) {
      hosters.splice(hosters.indexOf(id), 1)
      remove_task_and_reward({ id, role: 'hoster', task, idleProviders: DB.status.idleHosters, status: 'fail', log })
    }
    else if (encoders.includes(id)) {
      encoders.splice(encoders.indexOf(id), 1)
      remove_task_and_reward({ id, role: 'encoder', task, idleProviders: DB.status.idleEncoders, status: 'fail', log })
    }
  }
