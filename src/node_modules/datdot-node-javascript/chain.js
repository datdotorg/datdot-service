...
async function init () {
  ...
  const scheduler = blockgenerator({ blockinterval, intrinsics }, log.sub('blockgenerator'), async blockMessage => {
    ...
    async function emitBlock () {
      const promises = Object.entries(connections).map(([name, { ws, handler }]) => new Promise((resolve, reject) => {
        ...
        eventpool.forEach(([log, message]) => {
          log({ type: 'info', data: [`emit chain event ${JSON.stringify(message)}`] })
          handler(message)
        })
        resolve()
      }))
  ...
  wss.on('connection', function connection (ws) {
    ws.on('message', async function incoming (message) {
      ...
      const [from, id] = flow
      log({ type: 'info', data: `${JSON.stringify(type)} ${JSON.stringify(flow)}` })
...
async function storage_challenge_timeout (log, data) {
  const {user} = data
  log({ type: 'storage-challenge-timeout', data: {perf: performance.now() - user.hoster.challenges.storage.timestamp} })
...
async function _storage_challenge_report (attestor, { name, nonce }, status, args) {
  ...
  if (challenges.storage.cid !== storageChallengeID) return log({ type: 'info', data: `Not an active storage challenge` })
...
async function start_storage_challenges (user, log) {
  ...
  log({ type: 'info', data: { text: 'NEW CHALLENGE',  storageChallenge: user.hoster.challenges.storage } })
  user.hoster.challenges.storage = {
    timestamp: performance.now(),
    sid: scheduleAction({ 
      // set a followup (usign scheduler)
      from: log, data: {user}, delay: 1, type: 'storage_challenge_timeout' 
    }),
    // make new challenge
