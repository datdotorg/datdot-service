...
async function _amendment_report (user, { name, nonce }, status, args) {
  ...
  for (var i = 0, len = hosters.length; i < len; i++) {
    ...
    if (!datdot_crypto.verify_signature(sig, data, signingKey)) return log({ type: 'info', data: { text: `Error: unique_el_signature for hoster: ${hoster_id} could not be verified` } })
  }
...
async function _storage_challenge_report (attestor, { name, nonce }, status, args) {
  ...
  const { attestor: attestorID, hoster: hosterID, checks } = storageChallenge
  ...
  if (attestor.id !== attestorID || user.hoster.challenge.cid !== storageChallengeID ) {
    return log({ type: 'storage challenge', data: { text: `This not a valid challenge`, storageChallengeID } })
  }
  ...
  if (!reports.length || !datdot_crypto.verify_signature(storage_challenge_signature, messageBuf, signingKeyBuf)) { 
    ...
    await start_new_storage_challenge(user, log)
    return log({ type: 'storage challenge', data: { text: `This challenge's report is empty`, storageChallengeID } })
  }
...
async function _performance_challenge_report (user, { name, nonce }, status, args) {
  ...
  if (attestor_id !== userID || cid !== challenge_id) {
    return log({ type: 'performance challenge', data: { text: `This not a valid challenge`, challenge_id } })
  }
  // attestor failed, start a new challenge
  if (!reports.length) { 
    ...
    return log({ type: 'performance challenge', data: { text: `This challenge's report is empty`, challenge_id } })
  }
...
function book ({ type, selectedProviders, role, newTask, log }) {
  log({ type: 'info', data: { text: `Start task`, role, newTask } })
  selectedProviders.forEach(({ id, role }) => {
    const provider = getUserByID(id)
...
function unbook_and_rate ({ id, role, task, status, type, opts, log }) {
  ...
  if (!user[role].tasks[task]) return log({ type: 'info', data: { text: `Not a valid task`, task } })
  var reward
  var grade
  log({ type: 'info', data: { text: `Unbooking and rating`, task } })
  
  if (role === 'hoster') {
    if (type === 'storage') {
      const { all, proved } = opts
      reward = 50 * proved
      const success_rate = math.divide(proved,all).toFixed(1)
      if (status === 'done') grade = success_rate * all
      else if (status === 'fail') grade = all
      // if (success_rate < 0.8) { find replacement }        
    }
    ...
    grade = 1
    user[role].tasks[task] = undefined
...
function hasCapacity (provider, role, log) {
  log({ type: 'info', data: { text: 'Has capacity', role, provider: JSON.stringify(provider) } })
