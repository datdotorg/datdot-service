...
async function storage_challenge_timeout (log, data) {
  ...
  task_done({ id: attestorID, role: 'attestor', task: cid, idleProviders: DB.status.idleAttestors, action: () => tryNextChallenge({ attestorID, log }) }, log)
...
async function performance_challenge_timeout (log, data) {
  ...
  attestorIDs.forEach(id => task_done({ id, role: 'attestor', task: cid, idleProviders: DB.status.idleAttestors, action: () => tryNextChallenge({ id, log }) }, log))
...
async function make_storage_challenge (log, data) {
  ...
  const newTask = storageChallengeID
  const type = 'NewStorageChallenge'
  avoid[hoster_id] = true
  const idleProviders = DB.status.idleAttestors
  const selectedProviders = select({ idleProviders, role: 'attestor', newTask, amount: 1, avoid, plan: {}, log })
  const [attestor] = selectedProviders
  if (!attestor) return DB.queues.attestorsTaskQueue.push({ fnName: 'NewStorageChallenge', opts: { storageChallenge } })
  storage_challenge.attestor = attestor.id
  task_start({ type, selectedProviders, idleProviders, role: 'attestor', newTask }, log)
  //
  log({ type: 'info', data: { text: 'Making new storage challenge for', items: { type, newTask, hoster_id, tasks, cid: storageChallengeID } } })
...
async function make_performance_challenge (log, data) {
  ...
  const idleProviders = DB.status.idleAttestors
  const attestors = select({ idleProviders, role: 'attestor', newTask, amount: 1, avoid, plan: {}, log })
  if (!attestors.length) return DB.queues.attestorsTaskQueue.push({ fnName: 'NewPerformanceChallenge', opts: { performanceChallenge } })
  performanceChallenge.attestors = attestors.map(attestor => attestor.id)
  task_start({ type, selectedProviders: attestors, idleProviders, role: 'attestor', newTask }, log)
  log({ type: 'challenge', data: {text:'Making new performance chalenge', items: {challenge: type, newTask, attestors, cid: id} }})
  feed.challenge = {
    status: 'active',
    timestamp: performance.now(),
  ...
  // emit event
  log({ type: 'info', data: [type, newTask] })
  emitEvent(type, [newTask], log)
}
...
async function registerRole (user, role, log) {
  ...
  try_next_amendment(log)
  // TODO: replace with: `findNextTask()`
  // tryNextChallenge({ attestorID: userID, log }) // check for attestor only tasks (storage & perf challenge)
...
async function _amendment_report (user, { name, nonce }, status, args) {
  ...
  if (failed.length) _retry_amendment({ failed, amendmentID, log })
    ...
    encoders.forEach(id => task_done({ id, role: 'encoder', task: amendmentID, idleProviders: DB.status.idleEncoders, action: () => try_next_amendment(log) }, log))
    attestors.forEach(id => task_done({ id, role: 'attestor', task: amendmentID, idleProviders: DB.status.idleAttestors, action: () => try_next_amendment(log) }, log))
...
async function _storage_challenge_report (attestor, { name, nonce }, status, args) {
  ...
  if (attestor.id !== attestorID) {
    user.hoster.challenge.status = 'failed'
    await start_new_storage_challenge(user, log)
  ...
  task_done({ id: attestorID, role: 'attestor', task: storageChallengeID, idleProviders: DB.status.idleAttestors, action: () => tryNextChallenge({ attestorID, log }) }, log)
...
async function _submitPerformanceChallenge (user, { name, nonce }, status, args) {
  ...
  task_done({ id: userID, role: 'attestor', task: performanceChallengeID, idleProviders: DB.status.idleAttestors, action: () => tryNextChallenge({ attestorID: userID, log }) }, log)
}
...
async function activate_amendment (id, log) {
  return new Promise(async (resolve, reject) => {
    ...
    const providers = getProviders(getPlanByID(plan_id), amendment.providers, newTask, log)
    if (!providers) {
    ...
    const { scheduleAction } = await scheduler
    amendment.sid = scheduleAction({ from: log, data: { id }, delay: 2, type: 'amendment_timeout' })
    // emit event
    log({ type: 'info', data: { text: 'new event emitted', type, newTask } })
    emitEvent(type, [newTask], log)
    resolve()
  })
}
...
function task_start ({ type, selectedProviders, idleProviders, role, newTask }, log) {
...
// async function task_done ({ providers, taskID }, log) {
...
function task_done ({ id, role, task, idleProviders, action }, log) {
  const provider = getUserByID(id)
...
function tryNextChallenge ({ attestorID, log }) {
  // TODO add timeouts
  log({ type: 'attestors task queue', data: { text: 'next challenge', attestorID } })
  if (DB.queues.attestorsTaskQueue.length) {
    const next = DB.queues.attestorsTaskQueue[0]
    log({ type: 'attestors task queue', data: { text: 'next challenge', next: JSON.stringify(next) } })
    if (next.fnName === 'NewStorageChallenge' && DB.status.idleAttestors.length) {
      ...
      const newTask = storageChallenge.id
      const type = 'NewStorageChallenge'
      const idleProviders = DB.status.idleAttestors
      const selectedProviders = select({ idleProviders, role: 'attestor', newTask, amount: 1, avoid, plan, log })
      const [attestor] = selectedProviders
      if (selectedProviders.length) {
        DB.queues.attestorsTaskQueue.shift()
    ...
    if (next.fnName === 'NewPerformanceChallenge' && DB.status.idleAttestors.length) {
      ...
      const newTask = performanceChallenge.id
      const type = 'NewPerformanceChallenge'
      const attestors = select({ idleProviders: DB.status.idleAttestors, role: 'attestor', newTask, amount: 1, avoid, plan, log })
      if (attestors.length) {
        DB.queues.attestorsTaskQueue.shift()
        performanceChallenge.attestors = attestors.map(attestor => attestor.id)
        task_start({
...
async function _retry_amendment ({ failed, amendmentID, log }) {
  ...
  for (var i = 0, len = failed.length; i < len; i++) {
    ...
    if (attestors.includes(id)) {
      ...
      attestors.forEach(id => task_done({ id, role: 'attestor', task, idleProviders: DB.status.idleAttestors, action: () => try_next_amendment(log) }, log))
    }
    else if (hosters.includes(id)) {
      hosters.splice(hosters.indexOf(id), 1)
      task_done({ id, role: 'hoster', task, idleProviders: DB.status.idleHosters, action: () => try_next_amendment(log) }, log)
    }
    else if (encoders.includes(id)) {
      encoders.splice(encoders.indexOf(id), 1)
      task_done({ id, role: 'encoder', task, idleProviders: DB.status.idleEncoders, action: () => try_next_amendment(log) }, log)
    }
  }
