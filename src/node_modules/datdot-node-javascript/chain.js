...
async function amendment_timeout (log, data) {
  ...
  // _amendment_report(user, { name, nonce }, status, [report])
...
async function _amendment_report (user, { name, nonce }, status, args) {
  ...
  const contract = getContractByID(contractID)
  const { status, sid, plan: planID, feed: feedID } = contract
  const plan = getPlanByID(planID)
  const [attestorID] = attestors
  ...
  if (!sid) log({ type: 'info', data: { text: 'No scheduler in', contract } })
  cancelAction(sid)
...
async function activate_amendment (id, log) {
  return new Promise(async (resolve, reject) => {
    ...
    const { scheduleAction } = await scheduler
    const amendment = getAmendmentByID(id)
    amendment.sid = scheduleAction({ from: log, data: {}, delay: 10, type: 'amendment_timeout' })
    // emit event
