...
async function make_performance_challenge (log, data) {
  ...
  const idleProviders = DB.status.idleAttesters
  const attesters = select({ idleProviders, role: 'attester', newTask: id, amount: 1, avoid, plan: {}, log })
...
async function _performance_challenge_report (user, { name, nonce }, status, args) {
  ...
  for (var i = 0, len = reports.length; i < len; i++) {
    ...
    unbook_and_rate({ id: userID, role: 'attester', task: challenge_id, type: 'performance', status: 'done', log })
