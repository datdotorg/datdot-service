...
async function make_performance_challenge (log, data) {
  ...
  for (var i = 0, len = contractIDs.length; i < len; i++) {
    ...
    log({ type: 'chain', data: { text: 'Hoster IDs from active amendment', hosterIDs } })
  ...
  log({ type: 'challenge', data: {text:'Making new performance challenge', performanceChallenge, items: {challenge: type, newTask: id, attestors, cid: id} }})
  feed.challenge = {
    status: 'active',
    timestamp: performance.now(),
...
async function _publish_plan (user, { name, nonce }, status, args) {
  ...
  const verified = await store_root_signatures(proofs, feed_ids, log)
...
async function _amendment_report (user, { name, nonce }, status, args) {
  ...
  if (user.id !== attestorID) return log({ type: 'chain', data: { text: `Error: this user can not submit the attestation`, providers: JSON.stringify(amendment.providers), user: user.id } })
  ...
  for (var i = 0, len = hosters.length; i < len; i++) {
    ...
    const sig_buf = b4a.from(sig)
    const data = b4a.from(`${amendmentID}/${i}`, 'binary')
    const signingKey  = b4a.from(getUserByID(hoster_id).signingKey)
    log({ type: 'chain', data: { text: 'Verifying signatures', sig: sig_buf.toString('hex'), data: data.toString('hex'), signingKey: signingKey.toString('hex') } })
...
async function _storage_challenge_report (attestor, { name, nonce }, status, args) {
  ...
  const messageBuf = b4a.alloc(varint.encodingLength(storageChallengeID))
  varint.encode(storageChallengeID, messageBuf, 0)
  const signingKeyBuf = b4a.from(user.signingKey, 'binary')
...
function  analyse_storage_proof ({ checks, reports, log }) {
  ...
  for (var i = 0, len = reports.length; i < len; i++) {
    ...
    const not_verified = datdot_crypto.merkle_verify({
      ...
      version, 
      signature: b4a.from(signatures[version], 'binary'), 
      nodes
    })
...
async function _performance_challenge_report (user, { name, nonce }, status, args) {
  ...
  if (!reports.length) { 
    ...
    unbook_and_rate({ id: attestor_id, role: 'attestor', task: challenge_id, type: 'performance', status: 'fail', log })
  ...
  for (var i = 0, len = reports.length; i < len; i++) {
    ...
    if (!stats || !event_sig || !datdot_crypto.verify_signature(event_sig, b4a.from(`${challenge_id}`, binary), getUserByID(hoster_id).signingKey)) {
      feed.challenge.status = 'fail'
      unbook_and_rate({ id: attestor_id, role: 'attestor', task: challenge_id, type: 'performance', status: 'unknown', log })
...
async function store_root_signatures (proofs, feed_ids, log) {
  const all = proofs.map(async ({ feed_ref, p }, i) => {
    ...
    const len = p.upgrade.length
    feed.signatures[len] = p.upgrade.signature
  })
...
async function publish_plan_components (log, components, feed_ids) {
  ...
  const dataset_ids = await Promise.all(dataset_items.map(async item => {
    console.log('publishing plan components', item)
...
async function make_contracts (plan, log) {
  ...
  for (var i = 0; i < datasets.length; i++) {
    console.log('making contract for dtaset', datasets[i])
...
function book ({ type, selectedProviders, role, newTask, log }) {
  log({ type: 'chain', data: { text: `Start task`, type, selectedProviders, role, newTask } })
  ...
  for (var start = new Date(); DB.queues.tasks.length && (new Date() - start < 4000);) {
    ...
    if (fnName === 'amendment' && DB.status.idleAttestors.length && DB.status.idleEncoders.length >= 3 && DB.status.idleHosters.length >= 3) {
      ...
      if (providers) {
        ...
        log({ type: 'chain', data: { text: `Providers for amendment (${task_id})`,  providers} })
