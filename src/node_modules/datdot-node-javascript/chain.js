...
******************************************************************************/
const intrinsics = { plan_execution, amendment_followup, storage_challenge_timeout, make_storage_challenge }
...
async function storage_challenge_timeout (log, data) {
  ...
  log({ type: 'storage-challenge-timeout', data: {storageChallengeID, perf: performance.now() - user.hoster.challenges.storage.timestamp} })
...
async function make_storage_challenge (log, data) {
  ...
  user.hoster.challenges.storage = {
    ...
    sid: scheduleAction({ from: log, data: {user}, delay: 2, type: 'storage_challenge_timeout' }), cid: id 
  }
  // emit event
  log({ type: 'info', data: { text: 'Making new storage challenge for', items: { type, newJob, hoster_id, jobs, cid: id } } })
...
async function _amendment_report (user, { name, nonce }, status, args) {
  ...
  if (!failed.length) {
    ...
    for (var i = 0, len = hosters.length; i < len; i++) {
      ...
      const jobs = Object.keys(user.hoster.jobs).map(job => Number(job))
      log(`Hosting started: contract: ${contractID}, amendment: ${amendmentID}, hoster: ${hosters[i]}, jobs: ${jobs}`)
      // schedule challenges for each hoster only once
      if (!user.hoster.challenges.storage) await schedule_new_storage_challenge(user, log)
    }
    encoders.forEach(id => {
...
async function _storage_challenge_report (attestor, { name, nonce }, status, args) {
  ...
  const info_msg = { type: 'info', data: { text: `Received StorageChallenge`, storageChallengeID, fields: Object.keys(response) } }
  log(info_msg)
  ...
  if (attestor.id !== attestorID) return log({ type: 'info', data: [`Only the attestor can submit this storage challenge`] })
  removeJob({ id: attestorID, role: 'attestor', doneJob: storageChallengeID, idleProviders: DB.status.idleAttestors, action: () => tryNextChallenge({ attestorID, log }) }, log)
  ...
  if (!datdot_crypto.verify_signature(storage_challenge_signature, messageBuf, signingKeyBuf)) return log({ type: 'info', data: [`Storage challenge failed ${storageChallengeID}`] })

  log({ type: 'info', data: { cid: challenges.storage.cid, storage: challenges.storage } })

  if (challenges.storage.cid !== storageChallengeID) return log({ type: 'fail', data: `Not an active storage challenge: CID ${challenges.storage.cid}/ID ${storageChallengeID},  challenge_CID: ${JSON.stringify(getItemByID(challenges.storage.cid))} challenge_challengeID: ${JSON.stringify(getItemByID(storageChallengeID)) }` })

  evaluate_storage_challenge_report({ storageChallenge, reports, log })
  await schedule_new_storage_challenge(hoster, log)

  return log({ type: 'info', data: `Storage challenge confirmed ${storageChallengeID}` })
...
async function schedule_new_storage_challenge (user, log) {
  ...
  if (challenges.storage) {
    ...
    log({ type: 'info', data: {text:'RESET OLD STORAGE CHALLENGE', challenge: challenges.storage } })
  }
