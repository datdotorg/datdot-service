...
function make_storage_challenge ({hoster_id, log}) {
  ...
  // emit event
  console.log('Making new storage challenge for', {hoster_id, jobs, cid: id })
...
async function _storage_challenge_report (attestor, { name, nonce }, status, args) {
  ...
  const info_msg = { type: 'info', data: [`Received StorageChallenge`, storageChallengeID, Object.keys(response)] }
  log(info_msg)
  ...
  console.log(JSON.stringify(info_msg))
  console.log(challenges.storage.cid, challenges.storage)

  if (challenges.storage.cid !== storageChallengeID) return console.log({ type: 'info', data: [`Not an active storage challenge`] })

  evaluate_storage_challenge_report({ storageChallenge, reports, log })
  await schedule_new_storage_challenge(hoster, log)

  console.log('StorageChallengeConfirmed')
  return log({ type: 'info', data: [`Storage challenge failed ${storageChallengeID}`] })
...
function  evaluate_storage_challenge_report ({ storageChallenge, reports, log }) {
  ...
  for (var i = 0, len = reports.length; i < len; i++) {
    ...
    log({ type: 'info', data: ['storage confirmed for:', {contractID, hosterID: storageChallenge.hoster, check}] })
...
async function schedule_new_storage_challenge (user, log) {
  ...
  console.log('RESET OLD STORAGE CHALLENGE', challenges.storage)
...
async function activate_amendment (id, log) {
  return new Promise(async (resolve, reject) => {
    ...
    log({ type: 'info', data: [`Providers for amendment (${id}): ${JSON.stringify(providers)}`] })
...
async function start_storage_challenges (user, log) {
  ...
  console.log('NEW CHALLENGE', user.hoster.challenges.storage)
