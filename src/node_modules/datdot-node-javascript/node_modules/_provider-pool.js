...
// const providers = await getProviders({ hosters: 3, attesters: 1, encoders: 3 }, doesQualifyFn) // hosting
// const providers = await getProviders({ attesters: 5 }, doesQualifyFn) // proof of performance
// const providers = await getProviders({ attesters: 1 }, doesQualifyFn) // proof of storage
...
async function getProviders (opts = {}, doesQualifyFn, avoid = []) {
  ...
  async function find (roles, i, count = 0) { // TODO: maybe replace `find()` with another loop
    ...
    const role = roles[i] // e.g. ['hosters', 'attesters', 'encoders']
