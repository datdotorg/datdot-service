...
const empty_codec = { encode: x => x, decode: x => x }
// @TODO: maybe change body of log(...args) function and make it a codec
// @TODO: by default use empty codec

module.exports = sublog

function sublog (name = '', codec = empty_codec, emit = () => {}) {

  if (typeof codec === 'function') [codec, emit] = [empty_codec, codec]
  const loggers = {}
  ...
  function makelog (names) {
    ...
    function log (...args) {
      try {
        ...
        const { message: text, stack } = error
        loglog({ type: 'logger-fail', data: { text, stack } })
        throw error
      }
    }
    log.path = path
