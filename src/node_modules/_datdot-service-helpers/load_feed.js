...
async function load_feed ({ role, swarmAPI, task_id, account, feedkey, log }) {
    ...
    if (account.storages.has(stringkey)) {
        ...
        await storage.add_task(task_id)
        return { feed, ext, update_feed_storage }       
    } else {
        ...
        const storage = new HosterStorage({ db, feed, discovery, ext, log })
    ...
    async function update_feed_storage ({ account, feedkey, task_id, log }) {
        return new Promise(async (resolve, reject) => {
            const stringkey = feedkey.toString('hex')
            if (account.storages.has(stringkey)) { 
                const storage = account.storages.get(stringkey)
                log({ type: 'feed', data: { text: 'Updating feed storage', task_id, len: Object.keys(storage.tasks).length }})
                delete storage.tasks[task_id]
                if (!Object.keys(storage.tasks).length) await storage.destroy()
                log({ type: 'feed', data: { text: 'Feed storage updated', task_id, len: Object.keys(storage.tasks).length }})
                resolve()
            } else {
