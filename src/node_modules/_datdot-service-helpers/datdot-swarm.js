...
async function datdot_swarm () {
  ...
  //     // if (info.topics.includes(string_topic)) handle_new_connection_for_task_per_topic({ socket, info, string_topic }) // TODO: is topic buffer or string?
  //     handle_new_connection_for_task_per_topic({ socket, info, string_topic }) // TODO: is topic buffer or string?
  //   }
  ...
  //   //     const string_topic = topics[i]
  //   //     if (new_topic === string_topic) handle_new_connection_for_task_per_topic({ socket, info, string_topic }) // TODO: is topic buffer or string?
  //   //   }
  //   // })

  // })
  
  // const swarmAPI = { connect_topic, disconnect_topic, swarm }
  ...
  function connect_topic (log, task_id, topic, mode, callback) {
    ...
    log({ type: 'swarm', data: { Text: 'Connecting topic' } })
  ...
  async function disconnect_topic (log, topic, task_id) {
    log({ type: 'swarm', data: { Text: 'Disconnecting topic' } })
    const string_topic = topic.toString('hex')
...
async function handle_existing_topic (opts) {
  ...
  log({ type: 'swarm', data: { text: 'Adding event to existing topic', task_id, string_topic }})
...
async function handle_new_topic (opts) {
  ...
  log({ type: 'swarm', data: { Text: 'Joining new topic' } })
  store.topics[string_topic] = { discovery, tasks: { [task_id]: {  } } }
  store.tasks[task_id] = { callback }
}

function handle_new_connection_for_task_per_topic ({ socket, info, string_topic }) {
    ...
    for (var k = tasks_for_topic.length; k--;) {
      ...
      console.log({ type: 'swarm', data: { text: 'handle new connection for task per topic', string_topic, task_id } })
