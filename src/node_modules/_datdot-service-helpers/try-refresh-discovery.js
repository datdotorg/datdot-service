...
async function try_refresh_discovery ({ swarm, topic, tasks, log }) {
	return new Promise(async (resolve, reject) => {
		tasks = Object.assign({ 
			...
			attester2replacement_hoster: { count: 0 }, // { server: true }
			attester2active_hoster: { count: 0 }, // { client: true }
      attester2replacement_encoder: { count: 0 }, // { client: true }
      active_hoster2attester: { count: 0 }, // { server: true }
		}, tasks)

		const { 
			author, sponsor, encoder2author, encoder2attester, attester2encoder, attester2hoster, 
      hoster2author, hoster2attester, hoster, performance_attester, storage_attester, storage_hoster, 
      attester2replacement_hoster, attester2active_hoster, attester2replacement_encoder, active_hoster2attester
		} = tasks 
		
		const any_client_only = 
      sponsor.count || encoder2author.count || attester2encoder.count || 
      hoster2attester.count || performance_attester.count || storage_attester.count || 
      attester2active_hoster.count || attester2replacement_encoder.count
      ...
      !attester2active_hoster.count && !attester2replacement_encoder.count
		...
		const any_server_only = 
      author.count || encoder2attester.count || attester2hoster.count || hoster.count || 
      storage_hoster.count || attester2replacement_hoster.count || active_hoster2attester.count

		const no_server = !author.count && !encoder2attester.count && !attester2hoster.count && !hoster.count && 
      !storage_hoster.count && !attester2replacement_hoster.count && !active_hoster2attester.count
