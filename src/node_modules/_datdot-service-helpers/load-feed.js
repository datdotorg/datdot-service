...
/*
- update load_feed calls with grouping ext_cbs
*/

async function load_feed ({ role, account, feedkey, ext_cbs, targets, ontarget, log }) {
    const { onmessage, onerror } = Object.assign({ onmessage: noop, onerror: noop }, ext_cbs)
    log({ type: role, data: { text: 'start load_feed', targets }})
    ...
    function onconnection (cache) {
        ...
        return (connection, info) => {
            ...
            pump(connection, replicationStream, connection, (error) => {
                ...
                if (error) console.log({pumping_error: error, role})
            })
            ...
            function is_target () {
                ...
                for (var i = 0, len = targets_copy.length; i < len; i++) {
                    const hosterkey = targets_copy[i]
                    ...
                    if (remotekey.equals(hosterkey)) {
                        log({ type: role, data: { text: `Found the right hoster`, role, id: hosterkey.toString('hex'), feed: feed.key.toString('hex')  } })
                        targets_copy.splice(i, 1)
    
                        feed.replicate(replicationStream)
                        return ontarget(hosterkey)
                    }
                }
            }
