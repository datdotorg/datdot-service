...
module.exports = class FeedStorage {
  /**
  ...
  **/
  constructor ({ db, feed, discovery, ext, log }) {
    this.db = db
    this.log = log
    this.feed = feed
  ...
  async update (entry, log) {
    this.log({ type: 'storage', data: { text: 'Updating feed storage' }})
    this.entry = entry
  }

  async add_task (task_id, onconnection, log) {
    this.log({ type: 'storage', data: { text: 'Adding task to feed storage', task_id }})
    this.tasks[task_id] = { onconnection, log }
  }
  ...
  async storeEncoded ({ index, encoded_data_signature, encoded_data, unique_el, nodes }) {
    this.log({ type: 'hoster', data: { text: `Hoster storage - starting to store`, index, encoded_data: encoded_data.toString('hex') }})
  ...
  async getData (index, cb) {
    this.log({ type: 'storage', data: { text: 'getData intercepted', index }})
    var decoded
    try {
      const encoded_data = await this._getEncodedData(index)
      this.log({ type: 'storage', data : { text: 'got encoded data for', index, encoded_data } })
      // Got encoded data!
      ...
      this.log({ type: 'Error', data: { text: 'Encoded is not yet stored', index }})
      try {
        // If it exists, return it
        decoded = await this._getDecoded(index)
        this.log({ type: 'storage', data: { text: 'got decoded version for', index, decoded: decoded.toString('hex') } })
        return decoded
        // If we got the value, send it to the CB
      } catch(error) {
        this.log({ type: 'Error', data: { text: 'Error getting decoded', index }})
        console.log({ text: 'error getting decoded', error})
        cb(error)
      }
    }
    cb(null, decoded)
  ...
  // Meant to be used by the hypercore storage
  async putData (index, data, cb) {
    this.log({ type: 'storage', data: { text: 'putData intercepted', index }})
    // Check if encoded data exists
    try {
      const decoded = await this._getDecoded(index)
      // We already have the encoded version so whatever
      this.log({ type: 'storage', data: { text: 'We already have the encoded version', index }})
      this.log({ type: 'storage', data: { text: 'We already have the encoded version', decoded }})
      return cb(null)
    } catch (error) {
      // We don't have the encoded version so we should store the decoded version
      try {
        await this._putDecoded(index, data)
        this.log({ type: 'storage', data: { text: `We don't have the encoded version so we should store the decoded version`, index }})
        return cb(null)
      } catch (err) {
        cb(err)
      }
  ...
  async _getNodes (index) {
    try {
      ...
      this.log({ type: 'Error in _getNodes', data: err})
    }
  }
  ...
  async destroy (log) {
    log({ type: 'feed', data: { text: 'Destroying discovery and closing feed and db' }})
    await this.feed.close()
    await this.discovery.destroy()
    await this.db.clear()
    await this.db.close()
