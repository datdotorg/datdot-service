...
module.exports = class FeedStorage {
  ...
  **/
  ...
  async update (entry, log) {
    log({ type: 'storage', data: { text: 'Updating feed storage' }})
    console.log({ type: 'storage', data: { text: 'Updating', storage: this }})
    this.entry = entry
  }

  async add_task (task_id, onconnection, log) {
    log({ type: 'storage', data: { text: 'Adding task to feed storage', task_id }})
  ...
  async storeEncoded ({ index, encoded_data_signature, encoded_data, unique_el, nodes }) {
    this.log({ type: 'hoster', data: { text: `Hoster storage - starting to store`, encoded_data }})
  ...
  getData (index, cb) {
    this._getEncodedData(index)
      .then(async (encoded_data) => {
        console.log({ type: 'feed storage', data: { text: 'Got encoded data'} })
        // Got encoded data!
        ...
        console.log({ type: 'feed storage', data: { text: `Key doesn't exist? Try to get decoded version`} })
        // Key doesn't exist? Try to get decoded version
        const decoded = await this._getDecoded(index)
        console.log({ type: 'feed storage', data: { text: `Key exists - return it`} })
        // If it exists, return it
        return decoded
      }).then((decoded) => {
        console.log({ type: 'feed storage', data: { text: `we got the value, send it to the CB`} })
        // If we got the value, send it to the CB
        cb(null, decoded)
        
        // Else pass unexpected errors to the CB
      }).catch(cb => {
        console.log({ type: 'feed storage', data: { text: `Error in getData`} })
      })
  }
