...
async function done_task_cleanup ({ role, topic, state, log }) { // TODO: add remotestringkey when known
	...
	for (const remotestringkey of connections_for_task) {
		...
		// 	remotestringkey,
		// 	stringtopic,
		// 	task_conn: Object.keys(state.tasks).map(topic => state.tasks[topic].connections),
		// 	role,
		// 	state
		// })
		...
		if (targets[remotestringkey]) {
			...
			const target_task = get_target_tasks({ targets, remotestringkey, log })
