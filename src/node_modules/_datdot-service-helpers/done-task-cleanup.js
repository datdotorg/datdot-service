...
async function done_task_cleanup ({ role, topic, remotestringkey, peers, state, log }) { // TODO: add remotestringkey when known
    ...
    // roles: attester2encoder, attester2hoster, performance_attester, storage_attester, hoster2attester
    //        attester2replacement_encoder, attester2active_hoster
	...
	else if (tasks[stringtopic]) { 
		// roles: encoder/hoster/sponsor to author/anyone in general swarm
...
async function clean_general_task ({ state, role, stringtopic, peers, log }) {
    ...
    if (sockets[remotestringkey]) {
      ...
      if (!count) {
        ...
        // which means socket hasn't been incremented twice and when that task has been cleaned
        // sockets[remostringkeey] have been removed
        if (sockets[remotestringkey]) delete sockets[remotestringkey]
      }
    }
    if (tasks[stringtopic].connections[remotestringkey]) {
      remove_from_task_connections({ tasks, stringtopic, remotestringkey, log })
    ...
    log({ type: 'cleanup', data: { text: 'swarm left', stringtopic }})
	} else {
...
function remove_from_roles ({ tasks, role, stringtopic, log }) {
  ...
  if (!tasks[stringtopic].roles[role]) {
    ...
    if (role === 'hoster2author') return 
    return log({ type: 'cleanup', data: { text: 'error: no matching role', role, stringtopic }})
  }
  ...
  if (!tasks[stringtopic].roles[role].count) delete tasks[stringtopic].roles[role]
}
...
function close_streams_and_channels ({ sockets, remotestringkey, log }) {
    ...
    log({ type: 'cleanup', data: { type: `error: close_streams_and_channels`, remotestringkey, err } })
  }
}
...
function destroy_swarm ({ state, log }) {
  const { swarm } = state
  swarm.dht._listening = false
  swarm.dht.destroy()
  delete state.swarm
  log({ type: 'cleanup', data: { text: 'swarm destroyed' }})
}