...
async function done_task_cleanup ({ role, topic, remotestringkey, state, log }) { // TODO: add remotestringkey when known
	...
	var target_topics
	if (targets && targets[remotestringkey]) {
		target_topics = Object.keys(targets[remotestringkey].tasks)
	}
	log({ 
		type: 'cleanup', 
		data: { 
			...
			stringtopic,
			remotestringkey,
			conn_for_topic: Object.keys(tasks[stringtopic].connections), 
    ...
    // roles: attester2encoder, hoster2attester, performance_attester
		...
		if (!topic_tasks) {
			// // if no more tasks for this topic, remove all connections
			// const topic_connections = get_connections(tasks, stringtopic, log )
			// for (const peer of topic_connections) {
			// 	delete tasks[stringtopic].connections[peer]
			// }
