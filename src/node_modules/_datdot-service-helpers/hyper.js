...
async function load_replicationStream ({ account, connection, remotestringkey, log }) {
	...
	try {
		...
		if (sockets[remotestringkey]) {
			...
			add_stream_listeners({ cache: account.cache, remotestringkey, log })
		}
...
function handle_matching_topic ({ stringtopic, remotestringkey, cache, log }) {
	...
	replicate_feeds({ cache, stringtopic, replicationStream, log })
}

function replicate_feeds ({ cache, stringtopic, replicationStream, log }) {
	try {
		const { swarm, feeds } = cache
...
async function exchange_feedkey ({ account, remotestringkey, log }) { //get feedkey from or send to a target peer (on each custom topic)
	const { cache } = account
	const { targets, sockets, tasks } = cache
	...
	for (const stringtopic of target_tasks) {
		try {
			...
			if (!entry) {
				...
				if (msg_queue) msg_queue.forEach(message => onmessage({ message, remotestringkey, string_msg, cache }))
		...
		try {
			...
			async function cb () {
				...
				function dispacher (message) {
					...
					onmessage ? onmessage({ message, remotestringkey, string_msg, cache }) : msg_queue.push(message)
				}
			}	
...
async function onmessage ({ message, remotestringkey, string_msg, cache }) {
	const { type, feedkey, stringtopic, proof_of_contact } = JSON.parse(message)
	const { feeds, targets, sockets } = cache
...
function add_stream_listeners ({ cache, remotestringkey, log }) {
	const { sockets } = cache
	const { replicationStream } = sockets[remotestringkey]
	...
	replicationStream.on('close', () => { 
		...
		if (sockets[remotestringkey]) {
			handle_onclose({ cache, remotestringkey, log })
		}
	})
...
function handle_onclose ({ cache, remotestringkey, log }) {
	log({ type: 'on close', data: { text: `handling on close event` } }) 
	const { swarm, sockets, targets, tasks, feeds } = cache
	...
	for (const stringtopic of all_topics) {
		...
		if (task.connections[remotestringkey]) {
			...
			if (!connections_for_socket && !targets[remotestringkey]) {
				if (!task.roles['author']) close_streams_and_channels({ sockets, remotestringkey, log })
