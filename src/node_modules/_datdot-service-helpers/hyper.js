...
function onconnection (account, conn_log) {
	return async (connection, peerInfo) => {
		try {
			...
			const { replicationStream } = await load_replicationStream({ account, connection, remotestringkey, log: conn_log })
			...
			if (peerInfo.topics.length) { // peer is a client
				for (var i = 0, len = peerInfo.topics.length; i < len; i++) {
					...
					if (connections[remotestringkey] || !tasks[stringtopic]) continue
					handle_matching_topic({ stringtopic, remotestringkey, cache, log: conn_log })
...
async function load_replicationStream ({ account, connection, remotestringkey, log }) {
	...
	try {
		...
		if (sockets[remotestringkey]) {
			...
			add_stream_listeners(replicationStream, log)
		}
		...
		function ondiscoverykey (discoverykey) { // client hasn't replicated the hypercores so they are asking for the 'discovery-key' event
			...
			handle_matching_topic({ stringtopic, remotestringkey, cache: account.cache, log })
		}
	} catch (err) {
...
function handle_matching_topic ({ stringtopic, remotestringkey, cache, log }) {
	const { sockets, tasks } = cache
	const replicationStream = sockets[remotestringkey].replicationStream
	add_connection_to_task({ connections: tasks[stringtopic].connections, remotestringkey })
	...
	replicate_feeds({ cache, stringtopic, remotestringkey, replicationStream, log })
}
...
async function exchange_feedkey ({ account, remotestringkey, log }) { //get feedkey from or send to a target peer (on each custom topic)
	...
	for (var i = 0; i < len; i++) {
		try {
			...
			add_connection_to_task({ connections, remotestringkey })
...
function new_channel ({ account, remotestringkey, log }) {
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			mux_proto.make_protocol(mux, opts, makePeer, log)
			
			// callback
			async function makePeer () {
				if (mux.opened(opts)) reject() // TODO: or just return
				// create and open a channel
				log({ type: 'protomux', data: { text: 'new channel cb', remotestringkey, is_channel: sockets[remotestringkey].channel ? 'yes' : 'no' }}) 
				channel = mux_proto.create_and_open_channel(mux, {...opts, onopen, onclose }, log)
			...
			async function onclose (isRemote, channel) { 
				const stream_opened = await mux.stream.opened
				log({ type: 'protomux', data: { text: 'onclose: either side closed channel', isRemote, stream_opened }})
				if (!isRemote) close_stuff({account, remotestringkey, log})  
			}
...
function add_connection_to_task ({ connections, remotestringkey }) {
	connections[remotestringkey] = true
}

function add_stream_listeners (replicationStream, log) {
