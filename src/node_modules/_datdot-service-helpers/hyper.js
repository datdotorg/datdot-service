...
async function _connect ( account, bootstrap, { swarm_opts, targets: Targets, onpeer = noop, done = noop, log }) {
    ...
    if (role === 'storage_hoster' || role === 'performance_hoster') {
			console.log({ fn: '_connect', name: log.path, role, done})
		}
    if (
			role === 'storage_hoster' || 
			role === 'storage_attester' ||
    ...
    if (!targetList.length) { // Join topic (if no targetList && no discovery yet)
			// store general swarm onpeer callback
			// targets store it under targets[remotestringkey][stringtopic].ontarget
			if (tasks[stringtopic].onpeer) tasks[stringtopic].onpeer.push(onpeer)
			else tasks[stringtopic].onpeer = [onpeer]
      ...
      const tid = setTimeout(() => {
        ...
        // we check for count to see if the connection was a match for any of the tasks (count would then not be a zero)
          ...
          conn_log({ type: 'onconnection', data: { text: 'ghost connection removed', remotestringkey }})
			...
			if (!sockets[remotestringkey]) await make_stream_and_channel({ account, tid, connection, remotestringkey, log: conn_log })
...
function handle_matching_topic ({ stringtopic, remotestringkey, account, log }) {
	...
	for (const onpeer of tasks[stringtopic].onpeer) {
...
function subscribe_to_topic_messages ({ account, string_msg, stringtopic, remotestringkey, log }) {
	...
	log({ type: 'onconnection', data: { text: `registry entry in target conn`, stringtopic, remotestringkey, entry: entry ? 'yes' : 'no'   } })
	if (!entry) {
...
async function onmessage ({ account, message, remotestringkey, string_msg }) {
	...
	// 	log({ type, data: { text: 'calling done', stringtopic, type } })
	// 	done({ type }) 
	// }
	else if (type === 'done') {
		// peer as already closed the task, but the callback is still stored in the onpeer array
		if (!tasks[stringtopic]) return 
		const mykey = account.noisePublicKey.toString('hex')
		// others => type done
		log({ type, data: { text: 'calling done', stringtopic, type } })
		if (done) done({ type, peerkey: mykey }) // targets store onpeer under targets[remotestringkey]?.tasks[stringtopic].onpeer
		else { // general swarm
			for (const onpeer of tasks[stringtopic].onpeer) {
				tasks[stringtopic].onpeer.shift({ peerkey: remotestringkey, stringtopic })
			}
		}
	}
}		
...
async function handle_done_task_cleanup ({ state, remotestringkey, log }) {
	...
	for (const stringtopic of all_topics) {
		...
		if (tasks[stringtopic].connections[remotestringkey]) {
			...
			if (tasks[stringtopic].roles['encoder2attester'] || tasks[stringtopic].roles['storage_hoster'] || tasks[stringtopic].roles['hoster2attester']) {
				// TASKS
        ...
        swarm.leavePeer(b4a.from(remotestringkey, 'hex')) // stop attempting direct connections to the peer
			...
			else if (tasks[stringtopic].roles['author'] || tasks[stringtopic].roles['hoster'] || tasks[stringtopic].roles['hoster2author']) {
				// TASKS
				var role 
				if (tasks[stringtopic].roles['author']) role = 'author'
				else if (tasks[stringtopic].roles['hoster']) role = 'hoster'
				else if (tasks[stringtopic].roles['hoster2author']) role = 'hoster2author'
...
account = {
	...
	state:  {
			...
			tasks: { // general swarm tasks or target tasks
				[stringtopic]: {
					connections: {
						[remotestringkey]: 'in progress' or 'in cleanup' 
					},
					roles: {
