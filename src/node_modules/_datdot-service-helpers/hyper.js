	...
	try {
		...
		for (const key of feedkeys) {
			...
			if (!feed.opened || feed.closing) {
				...
				continue // TODO: or should we reopen? but other side closed, so...
			}
...
async function exchange_feedkey ({ account, remotestringkey, log }) { //get feedkey from or send to a target peer (on each custom topic)
	...
	const channel = await new_channel({ account, remotestringkey, log }).catch(err => {
		return log({ type: 'exchange-feedkey', data: { text: 'Error: caught in new channel', remotestringkey, err }})
	})
	...
	// if (!channel.opened) return log({ type: 'exchange-feedkey', data: { text: 'Error: channel not opened', remotestringkey }})
...
function new_channel ({ account, remotestringkey, log }) {
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			// if channel already exists, try pairing again
			if (channel) {
				log({ type: 'protomux', data: { text: `Error: existing channel`, remotestringkey, opened: channel.opened, closed: channel.closed } })
				if (channel.opened) resolve(channel)
				cb()
			}
			...
			async function cb () {
				counter++
				log({ type: 'protomux', data: { text: 'mux pair cb', remotestringkey, counter, is_channel: channel ? 'yes': 'no' }}) 
				if (mux.opened(opts)) return reject()
				if (channel && channel.opened) {
					log({ type: 'protomux', data: { text: 'already have the open channel', remotestringkey, counter, is_channel: channel ? 'yes': 'no' }}) 
					resolve(channel)
				}
				// if (mux.opened(opts)) return
				channel = mux_proto.create_and_open_channel({ mux, opts: {...opts, onopen, onclose }, log })
				if (!channel) return reject('Error: mux_proto returned no channel')
...
function add_stream_listeners ({ state, remotestringkey, log }) {
	...
	replicationStream.on('close', () => { 
		...
		if (sockets[remotestringkey]) {
			const { channel } = sockets[remotestringkey]
			// assuming that error hasn't occured while opening the channel
			if (channel && !channel.opened) return
			// we are doing cleanup for connections with and without the channel
...
function handle_done_task_cleanup ({ state, remotestringkey, log }) {
