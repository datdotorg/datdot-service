...
const {
	done_task_cleanup, clean_general_task, get_tasks_count_for_topic,
	close_feeds, remove_from_tasks, get_connections, remove_from_feeds,
	decrease_socket_count, remove_from_targets,	close_streams_and_channels,
	get_target_tasks,	remove_from_target_tasks, remove_from_task_connections,
	remove_from_registry,	get_tasks, set_status_for_connection
...
-----------------------------------------------------------------------------------------------*/
async function _new_task (account, { singleUseKey, newfeed = true, field, feedkey, topic, log }) { // newfeed false for receivers of feedkey (attester, hoster)
    ...
    var stringkey
    var stringtopic
    const pubkey = singleUseKey || account.noisePublicKey.toString('hex')
    if (!account.state) account.state = { pubkey: { sockets: {}, feeds: {}, targets: {}, tasks: {}, amendments: {} } }
    if (!account.state[pubkey]) account.state[pubkey] = { sockets: {}, feeds: {}, targets: {}, tasks: {}, amendments: {} }
    const feeds = account.state[pubkey].feeds    
    try {
...
async function _connect ( account, bootstrap, { swarm_opts, targets: Targets, onpeer = noop, done = noop, log }) {
    ...
    try {
      ...
      var pubkey
      pubkey = swarm_opts.singleUseKeys ? 
        swarm_opts.singleUseKeys.publicKey.toString('hex') 
        : account.noisePublicKey.toString('hex')
      const state = account.state[pubkey]
      ...
      if (role === 'performance_attester') {
        const { publicKey, secretKey } = swarm_opts.singleUseKeys
      ...
      if (!swarm) {
        ...
        account.state[pubkey].swarm = swarm
        log({ type: 'store connect', data: { text: 'new swarm', mode }})
        swarm.on('connection', onconnection({ account, pubkey }))
      }
      ...
      if (!targetList.length) { // Join general topic (if no targetList && no discovery yet)
        ...
        for (const remotestringkey of targetList) {
          ...
          if (sockets[remotestringkey]) {
            ...
            // TODO: if existing topic (for some other task) and this is a perf attester, 
            // how will hoster know we are trying to get data for the feed they never replicated
            handle_target_connection({ account, pubkey, remotestringkey, log }) // feed will be returned with ontarget callback
          } else {				
            // Join peer (if no socket connection && no discovery yet)
            log({ type: 'store connect', data: { text: 'Target - joinPeer', role, remotestringkey, stringtopic, mode }})
            // await swarm.join(topic, mode).flushed()
            await swarm.joinPeer(b4a.from(remotestringkey, 'hex'))
            const listen = await swarm.listen() // explicitly start listening for incoming connections
          }
        }
      }
      resolve()
...
-----------------------------------------------------------------------------------------------*/
function onconnection ({ account, pubkey }) {
    ...
    try {
      ...
      if (!sockets[remotestringkey]) { 
        await make_stream_and_channel({ account, pubkey, tid, connection, remotestringkey, log: conn_log })
      }
      // this is the target
      if (targets[remotestringkey]) {
        clearTimeout(tid)
        handle_target_connection({ account, pubkey, remotestringkey, log: conn_log }) 
      ...
      for (const topic of peer_topics) {
        ...
        if (
          ...
          clearTimeout(tid)
          handle_matching_topic({ stringtopic, pubkey, remotestringkey, is_server: false, account, log: conn_log })
      }         
    } catch (err) {
...
async function make_stream_and_channel ({ account, pubkey, tid, connection, remotestringkey, log }) {
	log({ type: 'onconnection', data: { text: 'loading replication stream and channel', remotestringkey } })
  return new Promise(async (resolve, reject) => {
    try {
      const { tasks, sockets, targets, swarm } = account.state[pubkey]
      ...
      add_stream_listeners({ account, pubkey, remotestringkey, log })
      
      const channel = await new_channel({ account, pubkey, remotestringkey, log })
      ...
      function ondiscoverykey (discoverykey) { // client hasn't replicated the hypercores so they are asking for the 'discovery-key' event
        ...
        clearTimeout(tid)
        handle_matching_topic({ stringtopic, pubkey, remotestringkey, is_server: true, account, log })
      }
    } catch (err) {
...
function handle_matching_topic ({ stringtopic, pubkey, remotestringkey, is_server, account, log }) {
	const { sockets, tasks } = account.state[pubkey]
	...
	replicate_feeds({ account, pubkey, stringtopic, remotestringkey, replicationStream, log })
	subscribe_to_topic_messages({ account, string_msg, pubkey, stringtopic, remotestringkey, log })
}

function replicate_feeds ({ account, pubkey, stringtopic, remotestringkey, replicationStream, log }) {
	try {
    const { swarm, feeds, targets } = account.state[pubkey]
...
async function handle_target_connection ({ account, pubkey, remotestringkey, log }) { //get feedkey from or send to a target peer (on each custom topic)
	const { targets, sockets, tasks } = account.state[pubkey]
	const { channel, replicationStream } = sockets[remotestringkey]
	...
	try {		
		...
		for (const stringtopic of target_tasks) {
			try {	
				const { msg, role, feed, ontarget, status, log } = targets[remotestringkey].tasks[stringtopic]
				...
				if (status === 'in progress' && sockets[remotestringkey]) {  
					log({ type: 'exchange-feedkey', data: { text: 'this task already in progress', remotestringkey, stringtopic }})
					continue
				}
        targets[remotestringkey].tasks[stringtopic].status = 'in progress'
				increase_socket_count({ sockets, remotestringkey, log })
				add_to_connections({ tasks, stringtopic, remotestringkey })
          ...
          const tid = setTimeout(() => {
            // timeout - something went wrong with the conneciton between these 2 peers
            // if task (hosting setup) failed and was cleaned in between, just return
            if (!targets[remotestringkey]?.tasks[stringtopic]) return
            // let us try to reconnect them by removing the stream and channels
            targets[remotestringkey].tasks[stringtopic].status = null
            log({ type: 'protomux', data: { text: 'error: timeout - feedkey-ack not received', remotestringkey, stringtopic, tasks: Object.keys(tasks) }})
            ...
            if (sockets[remotestringkey] && tasks_with_peer_in_progress.length <= 1) { // only this task is in progress
              close_streams_and_channels({ sockets, remotestringkey, log })
              delete sockets[remotestringkey]
              delete tasks[stringtopic].connections[remotestringkey]
              log({ type: 'onconnection', data: { text: 'connection removed', remotestringkey }})
            }
          }, 1500)
...
function subscribe_to_topic_messages ({ account, string_msg, pubkey, stringtopic, remotestringkey, log }) {
  try {
    ...
    if (!entry) {
      ...
      if (msg_queue) msg_queue.forEach(message => onmessage({ account, pubkey, message, remotestringkey, string_msg }))
...
function new_channel ({ account, pubkey, remotestringkey, log }) {
	log({ type: 'protomux', data: { text: `new channel`, remotestringkey } })
	return new Promise(async (resolve, reject) => {
		try {
			const { sockets } = account.state[pubkey]
			var counter = 0
			...
			async function cb () {
				...
				function dispacher (message) {
					...
					onmessage ? onmessage({ account, pubkey, message, remotestringkey, string_msg, account }) : msg_queue.push(message)
				}
			}	
...
async function onmessage ({ account, pubkey, message, remotestringkey, string_msg }) {
	const { type, challenge_id, hosterkey, feedkey, role, stringtopic, proof_of_contact } = JSON.parse(message)
	const { feeds, targets, sockets, tasks } = account.state[pubkey]
    ...
    delete tasks[stringtopic].connections[remotestringkey].tid
	  ...
	  log({ type: 'proof of contact', data: { proof_of_contact, remotestringkey } })
    // calling done for 'performance_attester/storage_attester'
		done({ type, proof: proof_of_contact, hkey: remotestringkey })
	}
	else if (type === 'done') {
...
function add_to_tasks ({ tasks, stringtopic, role, log }) {
	if (!tasks[stringtopic]) {
		...
		log({ type: 'hyper', data: { text: 'Existing topic', role, stringtopic }})
	}
}
...
function add_stream_listeners ({ account, pubkey, remotestringkey, log }) {
	const { sockets } = account.state[pubkey]
	const { replicationStream } = sockets[remotestringkey]
	...
	replicationStream.on('close', () => { 
		...
		if (sockets[remotestringkey]) {
      handle_replicationStream_onclose({ account, pubkey, remotestringkey, log })
		}
	})
}

async function handle_replicationStream_onclose ({ account, pubkey, remotestringkey, log }) {
  const state = account.state[pubkey]
	...
	for (const stringtopic of all_topics) {
      ...
      // --------- TARGET ------------
        ...
        tasks[stringtopic].roles['encoder2attester']?.count || 
				...
				tasks[stringtopic].roles['active_hoster2attester']?.count
				) {
...
account = {
	...
	state:  {
			...
			tasks: { // general swarm tasks or target tasks
				[stringtopic]: {
          amendments: { // for handling hostingSetup_failed
            id: { peers }
          }
					connections: {
			...
			feeds: {
				[field]: { //stringtopic or custom field for performance challenges
					[stringkey]: { feed }
				}
			}
