...
function onconnection (account, conn_log) {
	return async (connection, peerInfo) => {
		try {
			const { state } = account
			const { targets, tasks, sockets } = state
...
async function exchange_feedkey ({ account, remotestringkey, log }) { //get feedkey from or send to a target peer (on each custom topic)
	...
	// var { channel } = sockets[remotestringkey]
	try {
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			async function onclose (isRemote, channel) { 
				...
				if (counter === 2) {
					// this connection won't work, let's wait for new connection from same peer
					// close_streams_and_channels({ sockets, remotestringkey, log })
					// remove_from_sockets({ sockets, remotestringkey, log })		
					// we should also remove connections[remotestringkey], but we don't know the topic		
				}
...
function add_stream_listeners ({ state, remotestringkey, log }) {
	...
	replicationStream.on('close', () => { 
		log({ type: 'replicationStream on', data: { text: `on close: replicationStream closed`, socket_exist: sockets[remotestringkey] ? 'yes' : 'no' } }) 
		if (sockets[remotestringkey]) {
...
async function handle_done_task_cleanup ({ state, remotestringkey, log }) {
	...
	for (const stringtopic of all_topics) {
		...
		if (task.connections[remotestringkey]) {
			...
			// --------- NO TARGET/GENERAL SWARM -------------
				...
				const topic_tasks_before = get_tasks_for_roles({ tasks, stringtopic, log })
				console.log({ 
					roleee2: role, 
					name: log.path, 
					rolesss: tasks[stringtopic].roles, 
					topic_tasks_before, 
					topic_tasks_after: get_tasks_for_roles({ tasks, stringtopic, log }), 
					remotestringkey,
	...
	// 			var target_tasks = get_target_tasks({ targets, remotestringkey, log })
	// 			for (const stringtopic of target_tasks) {
	// 				const { status, role } = targets[remotestringkey].tasks[stringtopic]
	// 				if (
	// 					role === 'hoster2author' || role === 'encoder2author' ||
	// 					role === 'hoster2attester' || role === 'attester2encoder'
	// 				) continue
	// 				if (status === 'done') {
	...
	// 		// // --------- GENERAL SWARM -------------
	// 		// 	decrease_socket_count({ sockets, remotestringkey, log })
	// 		// 	for (const role of Object.keys(tasks[stringtopic].roles)) {
	// 		// 		remove_from_roles({ tasks, role, stringtopic, log })
	// 		// 	}
	// 		// 	// --------- SOCKETS -------------
	// 		// 	const connections_for_socket = get_socket_count({ sockets, remotestringkey, log })
...
account = {
	...
	state:  {
			...
			tasks: {
				[stringtopic]: {
					connections: {
						[remotestringkey]: true // when connection is established
					},
					roles: {
