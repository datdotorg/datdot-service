...
function onconnection (account, conn_log) {
	return async (connection, peerInfo) => {
      ...
      // console.log({
      //   text: 'onconnection-log',
      //   name: conn_log.path,
      //   remotestringkey,
      //   tasks: Object.keys(state.tasks),
      //   targets: Object.keys(state.targets),
      //   roles: Object.keys(tasks).map(topic => [topic, JSON.stringify(Object.keys(tasks[topic].roles).map(role => [role, tasks[topic].roles[role]]))]),
      //   swarm_conn: [...swarm.connections].map(conn => conn.remotePublicKey.toString('hex')),
      //   peerInfo
      // })
			
			await load_replicationStream({ account, tid, connection, remotestringkey, log: conn_log })
...
async function load_replicationStream ({ account, tid, connection, remotestringkey, log }) {
	...
	try {
		...
		add_to_sockets({ sockets, tid, remotestringkey, connection, replicationStream, mux, log })
...
async function exchange_feedkey ({ account, remotestringkey, log }) { //get feedkey from or send to a target peer (on each custom topic)
	...
	log({ type: 'onconnection', data: { text: `exchange feedkey`, remotestringkey, target_tasks: Object.keys(targets[remotestringkey].tasks).length } })				

	const { tid } = sockets[remotestringkey]
  clearTimeout(tid)
	try {
		...
		return log({ type: 'exchange-feedkey', data: { text: 'Error: caught in exchange feedkey', remotestringkey, err }})
	}
...
function new_channel ({ account, remotestringkey, log }) {
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			function onopen () { 
				log({ type: 'protomux', data: { text: 'onopen: either side opened channel', mux_opened: mux.opened({ protocol: 'datdot/alpha' }) }}) 
			...
			async function onclose (isRemote, channel) { 
				log({ type: 'protomux', data: { text: 'onclose: either side closed channel', counter, opened: channel.opened, closed: channel.closed }})
...
async function onmessage ({ message, remotestringkey, string_msg, state }) {
	...
	const { ontarget, role, done, log } = targets[remotestringkey].tasks[stringtopic]
...
function add_to_sockets ({ sockets, remotestringkey, tid, connection, replicationStream, mux, log }) {
	log({ type: 'hyper', data: { text: 'Adding to sockets', remotestringkey, does_exist: sockets[remotestringkey] ? 'yes' : 'no' }})
	sockets[remotestringkey] = { socket: connection, tid, replicationStream, mux }
}
...
function add_stream_listeners ({ state, remotestringkey, log }) {
	...
	replicationStream.on('close', () => { 
		log({ type: 'replicationStream on', data: { text: `on close: replicationStream closed`, remotestringkey, socket_exist: sockets[remotestringkey] ? 'yes' : 'no' } }) 
		if (sockets[remotestringkey]?.count) {
			const { channel, count } = sockets[remotestringkey]
			...
			// }
			// error occured while opening the channel
...
async function handle_done_task_cleanup ({ state, remotestringkey, log }) {
	...
	for (const stringtopic of all_topics) {
		...
		if (task.connections[remotestringkey]) {
			// --------- TARGET ------------
			if (task.roles['encoder2attester'] || task.roles['attester2hoster']) {
				// TARGETS
			...
			// --------- NO TARGET/GENERAL SWARM -------------
				...
				const topic_tasks_before = get_tasks_count_for_topic({ tasks, stringtopic, log })
				console.log({ 
					roleee2: role, 
					name: log.path, 
					rolesss: tasks[stringtopic].roles, 
					topic_tasks_before, 
					topic_tasks_after: get_tasks_count_for_topic({ tasks, stringtopic, log }), 
					remotestringkey,
...
account = {
	...
	state:  {
			...
			tasks: {
				[stringtopic]: {
					connections: {
						[remotestringkey]: true // when connection is established [ true, false ] REMOVE array 
					},
					roles: {
