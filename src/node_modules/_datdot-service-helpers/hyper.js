...
/*-----------------------------------------------------------------------------------------------------
	...
	async function exchange_feedkey ({ account, remotestringkey, log }) { //get feedkey from or send to a target peer (on each custom topic)
		...
		if (!channel) log({ type: 'exchange-feedkey', data: { text: 'Error: no channel', remotestringkey }})
		log({ type: 'exchange-feedkey', data: { text: 'new channel returned', remotestringkey }})
	...
	function new_channel ({ account, remotestringkey, log }) {
		return new Promise(async (resolve, reject) => {
			try {
				...
				async function cb (opened) {
					log({ type: 'protomux', data: { text: 'make protocol cb', opened }}) 
					const { channel: existing_channel } = sockets[remotestringkey]
					if (existing_channel) {
						log({ type: 'protomux', data: { text: 'existing channel in cb' }}) 
						return resolve(existing_channel)
					}
					...
					log({ type: 'protomux', data: { text: 'resolving new channel', remotestringkey }}) 
					resolve(channel)
