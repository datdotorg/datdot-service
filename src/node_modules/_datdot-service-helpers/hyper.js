...
-----------------------------------------------------------------------------------------------*/
async function _new_task (account, { newfeed = true, field, feedkey, topic, signal, log }) { // newfeed false for receivers of feedkey (attester, hoster)
...
-----------------------------------------------------------------------------------------------*/
async function _connect ( account, bootstrap, { swarm_opts, targets: Targets, onpeer = noop, done = noop, signal, log }) {
	return new Promise (async (resolve, reject) => {
    signal.addEventListener("abort", () => { reject(signal.reason) })
    try {
      ...
      if (!swarm) {
        ...
        swarm.on('connection', onconnection({ account, signal }))
      }
      ...
      if (!targetList.length) { // Join general topic (if no targetList && no discovery yet)
        ...
        for (const remotestringkey of targetList) {
          ...
          if (sockets[remotestringkey]) {
            ...
            handle_target_connection({ account, remotestringkey, signal, log }) // feed will be returned with ontarget callback
          } else {				
...
-----------------------------------------------------------------------------------------------*/
function onconnection ({ account, signal }) {
	return async (connection, peerInfo) => {
      ...
      try {
        ...
        if (!sockets[remotestringkey]) { 
          await make_stream_and_channel({ account, tid, connection, remotestringkey, signal, log: conn_log })
          resolve()
        }
        // this is the target
...
async function make_stream_and_channel ({ account, tid, connection, remotestringkey, signal, log }) {
