...
async function _connect ( account, bootstrap, { swarm_opts, targets: Targets, onpeer = noop, done = noop, log }) {
    ...
    if (!targetList.length) { // Join topic (if no targetList && no discovery yet)
			...
			if (tasks[stringtopic].roles[role].done) tasks[stringtopic].roles[role].done.push(done)
			else tasks[stringtopic].roles[role].done = [done]
	...
	try {		
		...
		for (const stringtopic of target_tasks) {
			try {	
				...
				if (connections[remotestringkey] === 'in progress' && sockets[remotestringkey]) {  
...
async function onmessage ({ account, message, remotestringkey, string_msg }) {
    ...
    await feed.ready().catch(safetyCatch)
	  ...
	  log({ type: 'proof of contact', data: { proof_of_contact, hosterkey } })
		...
		done({ type, proof: proof_of_contact, role:'performance_attester/storage_attester', hosterkey })
	}
	...
	// }
		...
		if (done) { // calling done for targets
			...
			if (role.includes('author')) {
				for (const done of tasks[stringtopic].done) {
					tasks[stringtopic].roles['author'].done.shift({ peerkey: remotestringkey, stringtopic })
				}
			}
			if (role.includes('hoster')) {
				for (const done of tasks[stringtopic].done) {
					tasks[stringtopic].roles['hoster'].done.shift({ peerkey: remotestringkey, stringtopic })
				}
			}
			if (role.includes('hoster2author')) {
				for (const done of tasks[stringtopic].done) {
					tasks[stringtopic].roles['hoster2author'].done.shift({ peerkey: remotestringkey, stringtopic })
				}
			}
		}
	}
	...
	for (const stringtopic of all_topics) {
        ...
        swarm.leavePeer(b4a.from(remotestringkey, 'hex')) // stop attempting direct connections to the peer
			...
			// --------- NO TARGET/GENERAL SWARM -------------
				...
				// task is still in progres, we set it to ''
				...
				set_status_for_connection({ status: '', tasks, stringtopic, remotestringkey})
			}
		}
