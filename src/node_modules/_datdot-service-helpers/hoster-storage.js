...
module.exports = class hosterStorage {
  ...
  **/
  ...
  async storeEncoded ({ index, encoded_data_signature, encoded_data, unique_el, p }) {
    try {
      // index, encoded_data, signed_data
      // this.log({ type: 'hoster', data: { text: `Hoster storage - storing encoded`, index, encoded_data, p }})
      ...
      await this._putUniqueEl(index, b4a.from(unique_el, 'binary'))
      // console.log({ before_storing: p})
      const proof_to_string = proof_codec.to_string(p)
      // console.log({ proof_to_string })
      await this._putProof(index, b4a.from(proof_to_string))
  ...
  async getProofOfStorage (index) {
    const [encoded_data, encoded_data_signature, p] = await Promise.all([
      this._getEncodedData(index),
      this._getEncodedDataSignature(index),
      this._getProof(index)
    ])
    console.log({ proof_from_storage: b4a.toString(p), to_buffer: proof_codec.to_buffer(b4a.toString(p)) })
  ...
  async _getProof (index) {
    try {
      ...
      this.log({ type: 'Error in _getProof', data: err})
    }
  }
  ...
  async _putEncodedDataSignature (index, encoded_data_signature) {
    return this.db.put(makeKey(ENCODED_DATA_SIGNATURE_PREFIX, index), encoded_data_signature, Buffer.from([0, 255]), err => {
  ...
  async _putProof (index, proof) {
    return this.db.put(makeKey(PROOF_PREFIX, index), proof, Buffer.from([0, 255]), err => {
      if (err) this.log({ type: 'Error', data: { text: '_putProof', err } })
    })
  }
