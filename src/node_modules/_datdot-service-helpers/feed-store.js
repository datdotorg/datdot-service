...
async function _load_feed(account, { config, extension = {}, swarm_opts, peers, feedkey, log }) {
    ...
    if (fresh) {
        ...
        if (!account.storages.has(stringkey)) { // make  new feed as a hoster
            ...
            await ready(feed)
            log({ type: 'feed-store', data: { text: 'New feed for intercept', stringkey, stringtopic }})
            ...
            log({ type: 'feed-store', data: { text: 'Got the storage for feedkey', stringkey, stringtopic }})
            console.log({ log: log.path, storage })
            feed = storage.feed
        }
    ...
    if (!cache.topics[stringtopic].feeds[stringkey]) cache.topics[stringtopic].feeds[stringkey] = { feed }
    ...
    if (!discovery) cache.topics[stringtopic].discovery = swarm.join(topic, mode)
    ...
    console.log({feed_returned: feed})
    return feed
    ...
    function onconnection (cache, log) {
        ...
        return (connection, info) => {
            ...
            console.log({remotestringkey, sockets: Object.keys(cache.sockets)})
            if (socket) {
                log({ type: 'load-feed', data: { text: 'existing connection', remotestringkey, isInitiator: connection.isInitiator } })
                if (peerList.length) is_on_peerList()
                return // see if this connection is already in use
            }
            ...
            if (info.topics.length) {
                ...
                for (var i = 0, len = info.topics.length; i < len; i++) {
                    ...
                    if (topics[Topic]) {
                        ...
                        if (!topics_socket) topics[Topic].sockets[remotestringkey] = socket
                        Object.keys(topics[Topic].feeds).forEach(key => {
                            const { feed } = topics[Topic].feeds[key]
                            log({ type: 'load-feed', data: { text: 'matching topic', remotestringkey, isInitiator: connection.isInitiator, topic: Topic } })
                ...
                replicationStream.on('discovery-key', (discoverykey) => { // client hasn't replicated the hypercores so they are asking for the 'discovery-key' event
                    log({ type: 'load-feed', data: { text: 'discovery key' } })
            ...
            connection.on('close', (err) => { log({ type: 'feed-store', data: { text: `Connection closed` } }) })
            connection.on('end', (err) => { log({ type: 'feed-store', data: { text: `Connection ended` } }) })
        }
    }
}
