...
function onconnection (account) {
	return async (connection, peerInfo) => {
		try {
			...
			const { topics, sockets } = account.cache
			...
			// console.log('sockets[remotestringkey] count', sockets[remotestringkey].count)
			// if (sockets[remotestringkey].count > 1) return
			...
			if (peerInfo.topics.length) {
				for (var i = 0, len = peerInfo.topics.length; i < len; i++) {
					...
					if (topics[stringtopic]) {
						...
						replicate_feeds({ account, remotestringkey, stringtopic, replicationStream, log: conn_log })
					}
				}
			} 
		} catch (err) {
