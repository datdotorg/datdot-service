...
async function _load_feed(account, { config, extension = {}, swarm_opts, peers, feedkey, log }) {
    ...
    if (fresh) {
        ...
        if (!discovery) discovery = swarm.join(topic, mode).flushed()
    ...
    // REUSABLE/PERSISTENT UNINTERCEPTED FEEDS
    else { 
            ...
            */
            ...
            if (info.topics.length) {
                for (var i = 0, len = info.topics.length; i < len; i++) {
                    ...
                    log({ type: 'feed-store', data: { text: 'topics', Topic, isInitiator: connection.isInitiator } })
                    if (topics[Topic]) {
