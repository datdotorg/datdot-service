...
function onconnection (account, conn_log) {
	return async (connection, peerInfo) => {
		try {
			...
			const { topics, peers, sockets, swarm } = account.cache
			...
			conn_log({ type: 'onconnection', data: { text: 'connection callback' } })
			...
			// const connections = []
			// for (const [key, value] of swarm.connections.entries()) {
			// 	connections.push(key.remotePublicKey)
			// }
			// console.log({ remotekey, connections })
			...
			if (peerInfo.topics.length) {
				for (var i = 0, len = peerInfo.topics.length; i < len; i++) {
					...
					if (topics[stringtopic]) {
						...
						set_socket({ topics, stringtopic, key: remotestringkey, log: conn_log })
						replicate_feeds({ cache: account.cache, stringtopic, replicationStream, log: conn_log })
					}
				}
			} 
		} catch (err) {
