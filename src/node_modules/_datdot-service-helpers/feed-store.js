	...
	if (peerList.length === 0) { // Join topic (if no peerList && no discovery yet)
		...
		for (var i = 0; i < peerList.length; i++) {
			...
			if (sockets[remotestringkey]) {
				...
				exchange_feedkey({ cache, remotestringkey, log }) // feed will be returned with onpeer callback
	...
	return async (connection, peerInfo) => {
		try {
			...
			if (target) {
				...
				return exchange_feedkey({ cache, remotestringkey, log: conn_log }) // this is the peer we need to exchange feedkeys with
			}
...
/*-----------------------------------------------------------------------------------------------------
	...
	async function exchange_feedkey ({ cache, remotestringkey, log }) { //get feedkey from or send to a target peer (on each custom topic)
