...
function onconnection (account, log) {
	return async (connection, peerInfo) => {
		...
		const { replicationStream } = await load_replicationStream_and_channel({ account, connection, remotestringkey, log })
		if (target) {
			log({ type: 'onconnection', data: { text: 'target exists', remotestringkey } })
			return exchange_feedkey({ account, remotestringkey, log }) // this is the peer we need to exchange feedkeys with
		}
...
/*-----------------------------------------------------------------------------------------------------
	...
	async function new_channel ({ account, remotestringkey, log }) {
		return new Promise(async (resolve, reject) => {
			...
			const opts = { protocol: 'exchange-feedkey' }
			...
			function cb (isChannelOpen) {
				log({ type: 'channel opened cb', data: {isChannelOpen } })
				if (isChannelOpen || sockets[remotestringkey].channel) return resolve()
				mux.unpair(opts)
				const channel = mux_proto.create_and_open_channel(mux, {...opts, onopen, onclose })
				sockets[remotestringkey].channel = channel
				
				// add message types for different encodings (i.e. string...)
				sockets[remotestringkey].mux_msg_types = { string_msg: mux_proto.add_string_msg(channel, dispacher) }
				const string_msg = sockets[remotestringkey].mux_msg_types.string_msg
				log({ type: 'open protomux channel', data: { text: `string msg type created` } })
	...
	async function exchange_feedkey ({ account, remotestringkey, log }) { //get feedkey from or send to a target peer (on each custom topic)
		...
		// if (!channel) {
		// 	await new_channel({ account, remotestringkey, log })
		// 	log({ type: 'exchange feedkey', data: { text: 'make new channel' } })
		// } else {
		// 	channel.open()
		// }
		...
		for (var i = 0; i < len; i++) {
			...
			console.log({ type: 'exchange feedkey', data: { text: `is on for peer and topic`, remotestringkey, peer: Peers[remotestringkey], topics: Peers[remotestringkey].topics, stringtopic, entry: Peers[remotestringkey].topics[stringtopic]  } })				
