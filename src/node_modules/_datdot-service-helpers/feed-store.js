...
async function _load_feed(account, { make = true, feedkey, topic, log }) { // newfeed false for receivers of feedkey (attestor, hoster)
	// log({ type: 'feed-store', data: { text: 'loading feed' }})
	var feed
	var stringkey
	var stringtopic
	...
	if (stringtopic && stringkey && feeds[stringtopic]) { // get existing feed
		...
		if (make) {  // attestor and hoster have to wait to receive feedkey first, so make is false for them
			if (feedkey) {
				// log({ type: 'feed-store', data: { text: 'New feed', stringkey: feedkey.toString('hex') }})
				...
				// log({ type: 'feed-store', data: { text: 'New feed', stringkey: 'none' }})
				await feed.ready().catch(safetyCatch)
				feedkey = feed.key
				stringkey = feedkey.toString('hex')
...
async function _connect ( account, bootstrap, { swarm_opts, targets: Targets, log }) {
	Targets = Object.assign({ targetList: [], ontarget: noop, msg: {} }, Targets)
	const { targetList, ontarget, msg, feed } = Targets
	...
	if (!tasks[stringtopic]) {
		...
		log({ type: 'feed-store', data: { text: 'New topic', role, stringtopic }})
	} else {
		tasks[stringtopic].roles[`${role}`] ? tasks[stringtopic].roles[`${role}`]++ : tasks[stringtopic].roles[`${role}`] = 1
		log({ type: 'feed-store', data: { text: 'Existing topic', role, stringtopic, tasks: tasks[stringtopic] }})
	}
	...
	if (!len) { // Join topic (if no targetList && no discovery yet)
		...
		for (var i = 0; i < len; i++) {
			...
			if (!targets[remotestringkey]) targets[remotestringkey] = { topics: { [stringtopic]: { ontarget, feed, msg, log } } }
			else targets[remotestringkey].topics[stringtopic] = { ontarget, feed, msg, log }
	...
	return async (connection, peerInfo) => {
		try {
			...
			if (sockets[remotestringkey]) {
				...
				conn_log({ type: 'Error', data: { text: 'Error: label issue', label1, label2 }})
				return
			}
...
/*-----------------------------------------------------------------------------------------------------
		...
		try {
			...
			if (sockets[remotestringkey]) {
				...
				replicationStream.on('error', (err) => { 
					...
					// TODO: figure out what to do with author (when do they close? they don't have taks)
				})
				...
				replicationStream.on('data', (data) => { 
					// log({ type: 'replicationStream on', data: { text: `ondata: replicationStream on data` } })
				})
	...
	async function exchange_feedkey ({ account, remotestringkey, log }) { //get feedkey from or send to a target peer (on each custom topic)
		...
		const peer_topics = Object.keys(targets[remotestringkey].topics)
		const { replicationStream, channel } = sockets[remotestringkey]
		log({ type: 'onconnection', data: { text: `exchange feedkey`, remotestringkey  } })				

		sockets_count({ sockets, remotestringkey, log })

		// TODO: optimize to create channel only for exchange feedkey targets
		...
		for (var i = 0; i < len; i++) {
			...
			tasks[stringtopic].connections[remotestringkey] = true
			const { msg, log } = targets[remotestringkey].topics[stringtopic]
			...
			try {
				...
				safetyCatch(err)
				log({ type: 'error', data: { text: 'Error: is on targetList', err } })
			}
		}

		async function onmessage (message) {
			const { type, feedkey, stringtopic } = JSON.parse(message)
			const { ontarget, msg, log } = targets[remotestringkey].topics[stringtopic]
			var feed
			if (type === 'feedkey') {	
	...
	function new_channel ({ account, remotestringkey, log }) {
		return new Promise(async (resolve, reject) => {
			...
			function onopen () { log({ type: 'protomux', data: { text: 'onopen: either side opened channel' }}) }
			function onclose () { 
				log({ type: 'protomux', data: { text: 'onclose: either side closed channel', /* snapshot: get_snapshot(account, remotestringkey) */ }}) 
			}
			...
			async function cb () {
				...
				function dispacher (message) {
					...
					else registry.set(stringtopic, { msg_queue: [message] })		

					log({ type: 'dispacher', data: { text: `new message`, message, registry: [...registry.entries()] } })
				}
			}	
		})
	}
	...
	function close_stuff ({account, remotestringkey, log}) {
		...
		topic_keys.forEach(stringtopic => {
			...
			const discovery = swarm.status(b4a(stringtopic, 'hex'))
			
			var tasks_for_topic = 0
			Object.keys(tasks[stringtopic].roles).forEach(role => { tasks_for_topic += tasks[stringtopic].roles[role] })

			// if job count for this remotestringkey is 0, close/end stuff
			if (sockets[remotestringkey] && sockets[remotestringkey].count === 0 && tasks_for_topic === 0) {
			...
			remotekeys.forEach(key => { 
				if (key === remotestringkey) {
					...
					// do we also delete targets? (yes if task was closed by other peer, no if task has not been finished yet)
					// do we delete feeds?
        ...
        sockets: {
            [remotestringkey]: { 
				...
				targets: {  
					[remotestringkey]: {
						topics: {
							[stringtopic1]: { feed, msg: { receive, send }, ontarget, log },
							[stringtopic2]: { feed, msg: { receive, send }, ontarget, log }
						}
					}
				},
    },
