	...
	return (connection, peerInfo) => {
		...
		if (peerInfo.topics.length) {
			for (var i = 0, len = peerInfo.topics.length; i < len; i++) {
				...
				if (topics[Topic]) {
					...
					if (Peers[Topic] && Peers[Topic].peerList.includes(remotestringkey)) return exchange_key(remotestringkey)
		...
		function exchange_key (remotestringkey) {
			...
			try {
				...
				if (Peers[stringtopic].peerList.length === 0) Peers[stringtopic] = void 0
