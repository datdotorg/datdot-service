	...
	if (peerList.length === 0) { // Join topic (if no peerList && no discovery yet)
		...
		for (var i = 0; i < peerList.length; i++) {
			...
			if (sockets[remotestringkey]) {
				...
				// Join topic (if no socket connection && no discovery yet)
				if (!discovery) {
					log({ type: 'store connect', data: { text: 'making new discovery' }})
					await swarm.join(topic, mode).flushed()
					topics[stringtopic].discovery = swarm.status(topic)
...
/*-----------------------------------------------------------------------------------------------------
	...
	function new_channel ({ account, remotestringkey, log }) {
		...
		function replicate_feeds ({ account, remotestringkey, stringtopic, replicationStream, log }) {
			const topics = account.cache.topics
			log({ type: 'replicate feeds', data: { text: 'feeds for topic', stringtopic, count: Object.keys(topics[stringtopic].feeds).length } })
			Object.keys(topics[stringtopic].feeds).forEach(key => {
				const feed = topics[stringtopic].feeds[key].feed
			...
			const { isServer} = swarm.status(topic)
			if (!isServer) swarm.leave(topic) // stop getting new connections on topic. leave will not close any existing connections.
		}
		...
		function exchange_feedkey ({ account, remotestringkey, log }) { //get feedkey from or send to a target peer (on each custom topic)
			...
			async function onmessage (message) {
				console.log('onmessage mux', message)
				...
				if (type === 'feedkey') {	
					...
					string_msg.send(JSON.stringify({ type: 'ack-feedkey', feedkey: feed.key.toString('hex') }))
...
async function _no_tasks (account, opts) {
	...
	// close all the feeds
	const stringkeys = Object.keys(topics[stringtopic].feeds)
	if (stringkeys) {
