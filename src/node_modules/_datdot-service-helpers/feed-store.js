	...
	async function exchange_feedkey ({ account, remotestringkey, log }) { //get feedkey from or send to a target peer (on each custom topic)
		...
		for (var i = 0; i < len; i++) {
			try {
				...
				// if (!tasks[stringtopic] || !tasks[stringtopic].connections[remotestringkey]) return
		...
		return new Promise(async (resolve, reject) => {
			...
			async function cb () {
				...
				function dispacher (message) {
					...
					log({ type: 'dispacher', data: { text: `new message`, message, stringtopic, registry: [...registry.entries()] } })
				}
			}	
		})
	}
