...
const DECODED_PREFIX = 5
const PROOF_PREFIX = 2
const ENCODER_ID_PREFIX = 3
const NODES_PREFIX = 4
const SIGNATURE_PREFIX = 1
const VERSION_PREFIX = 6
...
async storeEncoded ({index, proof, encoded, encoder_id, version, nodes, signature}) {
    // console.log('storing data',{index, proof, encoded, encoder_id, version, nodes, signature})
    ...
    await this._putSignature(index, signature)
    await this._putNodes(index, nodes)


    const versionBuf = Buffer.alloc(varint.encodingLength(version))
    varint.encode(version, versionBuf, 0)
    await this._putVersion(index, versionBuf)
  ...
  async getProofOfStorage (index) {
    const [encoded, proof, encoder_id, version, nodes, signature] = await Promise.all([
      this._getEncoded(index),
      this._getProof(index),
      this._getEncoderID(index),
      this._getVersion(index),
      this._getNodes(index),
      this._getSignature(index)
    ])
    return {
      ...
      encoded,
      proof,
      encoder_id: varint.decode(encoder_id),
      version: varint.decode(version),
      nodes,
      signature
    }
  }
  ...
  async _getVersion (index) {
    return this.db.get(makeKey(VERSION_PREFIX, index))
  }
  async _getSignature (index) {
    return this.db.get(makeKey(SIGNATURE_PREFIX, index))
  }

  async _putProof (index, proof) {
    return this.db.put(makeKey(PROOF_PREFIX, index), proof, Buffer.from([0, 255]), err => {
      if (err) console.log('_putProof', {err})
    })
  }
  async _putVersion (index, version) {
    return this.db.put(makeKey(VERSION_PREFIX, index), version, Buffer.from([0, 255]), err => {
      if (err) console.log('_putVersion', {err})
    })
  }
  ...
  async _putSignature (index, signature) {
    return this.db.put(makeKey(SIGNATURE_PREFIX, index), signature, Buffer.from([0, 255]), err => {
      if (err) console.log('_putSignature', {err})
    })
  }
  async _putEncoded (index, data) {
    return this.db.put(makeKey(ENCODED_PREFIX, index), data, Buffer.from([0, 255]), err => {
      if (err) console.log('_putEncoded', {err})
    })
  }
  ...
  async _putEncoderID (index, data) {
    return this.db.put(makeKey(ENCODER_ID_PREFIX, index), data, Buffer.from([0, 255]), err => {
      if (err) console.log('_putEncoderID', {err})
    })
  }
