...
module.exports = class HosterStorage {
  ...
  **/
  ...
  async storeEncoded (index, proof, encoded, encoder_pos, nodes, signature) {
    ...
    await new Promise((resolve, reject) => {
      this.feed._putBuffer(index, decoded, packet, {}, async (err) => {
        if (err) {
          console.log({err, index, signature})
          reject(err)
        }
        else resolve()
      })
    })
    // console.log('downloaded count', this.feed.downloaded())
    // const isAudited = await audit(this.feed)
    ...
    const posBuf = Buffer.alloc(varint.encodingLength(encoder_pos))
    varint.encode(encoder_pos, posBuf, 0)
    await this._putEncoderPos(index, posBuf)
  }

  // Invoked by whoever to test that the hoster is actually hosting stuff
  async getStorageChallenge (index) {
    const [encoded, proof, encoder_pos] = await Promise.all([
      this._getEncoded(index),
      this._getProof(index),
      this._getEncoderPos(index)
    ])
  ...
  getData (index, cb) {
    this._getEncoded(index)
      .then(async (encoded) => {
        ...
        // TODO (maybe it's ok)
        const decompressed = await brotli.decompress(encoded)
        const posBuf = await this._getEncoderPos(index)
        const encoder_pos = varint.decode(posBuf)
        const decoded = parse_decompressed(decompressed, encoder_pos)
      }, async () => {
  ...
  async _getDecoded (index) {
    ...
    async _getEncoderPos (index) {
      return this.db.get(makeKey(ENCODER_POS_PREFIX, index))
    }
  ...
  async _putEncoderPos (index, data) {
    return this.db.put(makeKey(ENCODER_POS_PREFIX, index), data, Buffer.from([0, 255]))
  }
