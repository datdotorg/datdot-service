...
function blockgenerator ({ blockinterval = 6000, intrinsics }, log, emit) {
  ...
  init = setInterval(async () => {
    ...
    scheduled = []
    const blockstats = stats[currentBlock] = {}
    while (temp.length) {
      const time_now = performance.now()
      if ((time_now - startTime) > (blockinterval - 200)) {
        scheduled = [...temp, ...scheduled]
        log({ type: 'Error', data: `not able to check  or execute the remaining ${temp.length} intrinsics in scheduler during blockinterval`, stats: JSON.stringify(stats) })
      ...
      if (!id) continue // old item - id removed by `cancelAction(id)`
        ...
        log({ type: 'scheduler', data: `Executing: ${type}`, time_now})
        const intrinsic = intrinsics[type]
        await intrinsic(from, data)
        const execution_duration = performance.now() - time_now
        ;(blockstats[type] || (blockstats[type] = [])).push(execution_duration)
        log({ type: 'scheduler', data: `Executed: ${type}`, execution_duration  })
      }
  ...
  function cancelAction (id) {
    log({ type: 'scheduler', data: { text: `Cancel: ${id}`, scheduled: scheduled.filter(entry => JSON.stringify(entry.id)) } })
