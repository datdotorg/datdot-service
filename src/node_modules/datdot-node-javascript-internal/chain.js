...
async function plan_execution (log, data) {
  ...
  DB.queues.tasks.push({ fnName: 'hostingSetup', contractID }) // TODO: sort tasks based on priority (RATIO!)
  next_task(log)
}
...
async function make_performance_challenge (log, data) {
  const { feedID } = data
  DB.queues.tasks.push({ fnName: 'performanceChallenge', feedID })
  next_task(log)
}
...
async function start_hosterReplacement ({ next, temp_queue, scheduleAction, log }) {
  ...
  if (positions.length > 2) {
    ...
    setTimeout(async() => {
      ...
      DB.queues.tasks.push({ fnName: 'hosterReplacement', failedHosters, avoid, amendmentID: new_amendment_id })
      next_task(log)
    }, 5000)
    return
  }
