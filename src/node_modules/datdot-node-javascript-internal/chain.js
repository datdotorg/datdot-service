...
async function next_task (log) {
  ...
  for (var start = new Date(); DB.queues.tasks.length && (new Date() - start < 4000);) {
    ...
    if (fnName === 'hostingSetup' && DB.status.idleAttesters.length && DB.status.idleEncoders.length >= 3 && DB.status.idleHosters.length >= 3) {
...
async function _retry_amendment ({ failed, amendmentID, log }) {
  ...
  const newID = await init_amendment({ contract_id, providers: reuse, type, log })
