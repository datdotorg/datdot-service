...
async function _hosterReplacement_report (user, { name, nonce }, status, args) {
  ...
  log({ type: 'chain', data: { text: 'Hoster replacement report', report: JSON.stringify(report)  } })
  const amendment = getAmendmentByID(amendmentID)
  const { contract: contractID, sid, pos } = amendment
  const { hosters, attesters, encoders } = amendment.providers
  const contract = getContractByID(contractID)
  const feedObj = getFeedByID(contract.feed)
  const [attesterID] = attesters
  ...
  if (user.id !== attesterID) return log({ type: 'chain', data: { text: `Error: this user can not submit the attestation`, providers: JSON.stringify(amendment.providers), user: user.id } })
  // check if right amendment
  if (contract.amendments[contract.amendments.length - 1] !== amendmentID) return log({ type: 'chain', data: [`Error: this amendment for hosterReplacement has expired`, amendmentID] })
  // cancel amendment schedule
  const { cancelAction } = await scheduler
  if (!sid) log({ type: 'chain', data: { text: 'No scheduler for', contract } })
  cancelAction(sid)
  ...
  log({ type: 'chain', data: { text: 'hosterReplacement successfull', amendmentID } })
  // unbook
  for (var i = 0; i < hosters.length; i++) {
    if (i === pos) continue
    unbook_and_rate({ id: hosters[i], role: 'hoster', task, type, status: 'done', log })
  }
  for (const id of encoders) unbook_and_rate({ id, role: 'encoder', task, type, status: 'done', log })
...
async function _storage_challenge_report (user, { name, nonce }, status, args) {
  ...
  try {
    ...
    // challenged_hoster.hoster.challenge = storage challenge (hoster doesn't get notified about the performance challenge)
    ...
    if (res_status === 'fail') {
      ...
      for (const stringID of Object.keys(challenged_hoster.hoster.tasks)) {
        const amendmentID = Number(stringID)
        log({ type: 'storage challenge', data: { text: `calling drop hosting and start hoster replacement for failed hoster`, tasks: Object.keys(challenged_hoster.hoster.tasks), amendmentID, task: challenged_hoster.hoster.tasks[amendmentID] } })
        if (challenged_hoster.hoster.tasks[amendmentID] !== 'hosting') continue
        dropHosting({ amendmentID, userID: challenged_hoster.id, type: 'hosting', status: 'pause', log })
        await startHosterReplacement({ user: challenged_hoster, amendmentID, log })
      }

      return await cleanup_and_new_storage_challenge({ user: challenged_hoster, status: storageChallenge.status, log })
    }
    ...
    if (res_status === 'invalid-proof' || res_status === 'no-data') {
      ...
      unbook_and_rate({ id: attester_id, role: 'attester', task: challenge_id, type, status: att_status, log })
      unbook_and_rate({ id: hoster_id, role: 'hoster', task: challenge_id, type, status: host_status, opts: { all, proved }, log })
      return await cleanup_and_new_storage_challenge({ user, status: storageChallenge.status, log })
    ...
    unbook_and_rate({ id: attester_id, role: 'attester', task: challenge_id, type, status: att_status, log })
    unbook_and_rate({ id: hoster_id, role: 'hoster', task: challenge_id, type, status: host_status, opts: { all, proved }, log })
    await cleanup_and_new_storage_challenge({ user: challenged_hoster, status: storageChallenge.status, log })
    // TODO - remove: this is here just for the test
    if (challenged_hoster.id !== 0) return
    for (const stringID of Object.keys(challenged_hoster.hoster.tasks)) {
      const amendmentID = Number(stringID)
      if (challenged_hoster.hoster.tasks[amendmentID] !== 'hosting') continue
      await startHosterReplacement({ user: challenged_hoster, amendmentID, log })
...
async function cleanup_and_new_storage_challenge ({ user, status, log }) {
  var { cid, sid } = user.hoster.challenge
  const { scheduleAction, cancelAction } = await scheduler
  if (user.hoster.challenge === 'stop') return
  if (status === 'pending' || status === 'done' || status === 'timed-out' || status === 'fail') {
  ...
  if (status !== 'fail') scheduleAction({ from: log, data: {hoster_id: user.id}, delay: 5, type: 'make_storage_challenge' })
}
/*----------------------
...
async function _performance_challenge_report (user, { name, nonce }, status, args) {
  ...
  if (!reports || !hoster_ids.length) { 
    ...
    await cleanup_and_new_performance_challenge(feed_id, log)
    return
  }
...
async function storage_challenge_timeout (log, data) {
  ...
  await cleanup_and_new_storage_challenge({ user, status: storageChallenge.status, log })
}
...
async function hoster_replacement_timeout (log, data) {
  ...
  const { pos, providers: { encoders, attesters, hosters } } = getAmendmentByID(id)
  for (const id of hosters) {
    unbook_and_rate({ id, role: 'hoster', task, type: 'hosterReplacement', status: 'unknown', log })
  }
  for (const id of encoders) {
    unbook_and_rate({ id, role: 'encoder', task, type: 'hosterReplacement', status: 'unknown', log })
  }
  for (const id of attesters) {
    const user = getUserByID(id)
    _pause({ args: { status: 'fail', log }, user })
    unbook_and_rate({ id, role: 'attester', task, type: 'hosterReplacement', status: 'fail', log })
  }
  const user = getUserByID(hosters[pos])
  const failed = [...attesters, ...hosters, ...encoders]
  // const failed = [...attesters]
  await _retry_hosterReplacement({ failed, amendmentID: id, log })
}
...
async function make_storage_challenge (log, data) {
  ...
  log({ type: 'chain', data: { text: 'Making new storage challenge for', items: { task_id: id, hoster_id, amendments, cid: id } } })
  // push to tasks
...
async function init_amendment (opts) {
  ...
  log({ text: 'initializing amendment', amendment: JSON.stringify(amendment) })
  return id
}
...
async function _pause ({ user, args }) {
  ...
  if (status === 'fail') { // we assume it's a computer or network glitch
    ...
    user.status.paused = { until: block + 2 }
...
async function cancelHostingJobs ({ user, log }) {
  ...
  for (const id of amendmentIDs) {
    dropHosting({ amendmentID: id, userID: user.id, type: 'hosting', status: 'pause', log })
    await startHosterReplacement({ user, amendmentID: id, log })
  }
}
async function startHosterReplacement ({ user, amendmentID, avoid = {}, log }) {
  // find providers (1 encoder, 1 attester, 1 hoster)
  const { providers: { hosters, encoders }, contract: contractID } = getAmendmentByID(amendmentID)
  const { amendments } = getContractByID(contractID)
  const active_amendment = getAmendmentByID(amendments[amendments.length-1])
  log({ type: 'chain', data: { texts: 'start hoster replacement', user_id: user.id, hosters: JSON.stringify(hosters), amendmentID: active_amendment.id, type: active_amendment.type } })
  ...
  const opts = { 
    ...
    ref: active_amendment.id, log }
    const new_amendment_id = await init_amendment(opts)
    DB.queues.tasks.push({ fnName: type, avoid, id: new_amendment_id })
  ...
  const { providers: { hosters }, pos } = getAmendmentByID(amendmentID)
  const avoid = {}
  for (const id of failed) avoid[id] = true
  const user = getUserByID(hosters[pos])
  await startHosterReplacement({ user, amendmentID, avoid, log })
}
function dropHosting ({ amendmentID, userID, type, status, log }) {
...
async function replaceOtherPausedHosters ({ user, log }) {
  ...
  const amendments = getAmendmentsForHoster(user)
  log({ type: 'chain', data: { texts: 'replace other paused hosters', user_id: user.id, amendments } })
...
async function next_task (log) {
  ...
  for (var start = new Date(); DB.queues.tasks.length && (new Date() - start < 4000);) {
    ...
    if (fnName === 'hostingSetup' || fnName === 'retry_hostingSetup' 
      ...
      const user = getUserByID(hoster_id)
      if (user.status.paused) return // don't challenge paused hosters
      ...
      for (const contractID of feed.contracts) {
        const contract = getContractByID(contractID)
      ...
      feed.challenge = { status: 'active', timestamp: performance.now(), sid, cid: task_id }
    }
      ...
      for (var i = 0; i < providers.encoders; i++) {
        if (i === pos) continue
        avoid[providers.encoders[i]] = true
      }
      for (var i = 0; i < providers.hosters; i++) {
        if (i === pos) continue
        avoid[providers.hosters[i]] = true
      }
      ...
      log({ type: 'chain', data: { 
        text: 'got providers for hosterReplacement 1', 
        hosters: task.providers.hosters,
        encoders: task.providers.encoders,
        selected_host: hoster.id,
        selected_enc: encoder.id,
        pos,
        amendment: JSON.stringify(getAmendmentByID(task_id))
      } })
      
      book({ type: fnName, selectedProviders: task.providers.encoders.map(id => ({ id })), role: 'encoder', task_id, log })
      book({ type: fnName, selectedProviders: task.providers.hosters.map(id => ({ id })), role: 'hoster', task_id, log })
      book({ type: fnName, selectedProviders: task.providers.attesters.map(id => ({ id })), role: 'attester', task_id, log })
      
      log({ type: 'chain', data: { 
        text: 'got providers for hosterReplacement 2', 
        hosters: task.providers.hosters,
        encoders: task.providers.encoders,
        selected_host: hoster.id,
        selected_enc: encoder.id,
...
async function trigger_challenges ({ hosters, contractID, amendmentID, feedObj, log}) {
  for (var i = 0, len = hosters.length; i < len; i++) {
    ...
    if (!user.hoster.challenge.status) {
      ...
      await cleanup_and_new_storage_challenge({ user, status: 'pending', log })
    }
  }
...
function cancelContracts (plan, log) {
  ...
  for (var i = 0; i < contracts.length; i++) {
    ...
    for (const id of hosters) {
      ...
      dropHosting({ amendmentID, userID: id, type: 'hosting', status: 'cancel', log })
    }
  }
}
