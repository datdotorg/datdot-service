...
async function _amendment_report (user, { name, nonce }, status, args) {
  ...
  if (failed.length) {
    ...
    return _retry_amendment({ failed, amendmentID, log })
  }
...
async function _storage_challenge_report (user, { name, nonce }, status, args) {
  ...
  const { storageChallengeID: challenge_id,  status: challenge_status, proof_of_contact, reports } = response
  ...
  try {
    ...
    // user.hoster.challenge = storage challenge (hoster doesn't get notified about the performance challenge)
    const challenge = user.hoster.challenge 
    ...
    if (!valid_report) return log({ type: 'storage challenge', data: { text: `error: This not a valid challenge`, challenge_id } })

    if (challenge_status === 'fail') {
      log({ type: 'storage challenge', data: { text: `: fail - hoster timed out`, proof_of_contact } })
      att_status = 'unknown'
      host_status = 'fail'
      ...
      return await start_new_storage_challenge(user, log)    
    }
    if (!proof_of_contact || !reports || !challenge_status) {
      log({ type: 'storage challenge', data: { text: `: error: in storage challenge`, proof_of_contact, reports, challenge_status } })
      att_status = 'fail'
      host_status = 'unknown'
      unbook_and_rate({ id: attester_id, role: 'attester', task: challenge_id, type, status: att_status, log })
      unbook_and_rate({ id: hoster_id, role: 'hoster', task: challenge_id, type, status: host_status, opts: { all, proved }, log })
    ...
    if (challenge_status === 'invalid-proof' || challenge_status === 'no-data') {
      host_status = 'fail'
      ...
      unbook_and_rate({ id: hoster_id, role: 'hoster', task: challenge_id, type, status: host_status, opts: { all, proved }, log })
      return await start_new_storage_challenge(user, log)
    } else if (challenge_status === 'success') {
...
async function _performance_challenge_report (user, { name, nonce }, status, args) {
  ...
  const feedObj = getFeedByID(feed_id)
  ...
  const valid_report = is_valid_report({ attester_id, user_id, challenge, challenge_id })
  if (!valid_report) return log({ type: 'performance challenge', data: { text: `error: This not a valid challenge`, challenge_id } })
  ...
  if (!reports || !hoster_ids.length) { 
    ...
    for (const contract_id of contractIDs) {
      const contract = await chainAPI.getContractByID(contract_id)
  ...
  for (const hoster_id of hoster_ids) {
    ...
    if (reports[hoster_id].status === 'fail') {
      ...
      await start_new_performance_challenge(feed_id, log)
      log({ type: 'performance challenge', data: { text: `error: hoster failed the challenge`, challenge_id } })
    } else {
      ...
      if (!stats || !proof_of_contact || !proof_verified) {
        ...
        unbook_and_rate({ id: hoster_id, role: 'hoster', task: challenge_id, type, status: host_status, opts: { grade }, log })
        await start_new_performance_challenge(feed_id, log)
        return log({ type: 'performance challenge', data: { text: `error: attester failed`, challenge_id } })
      } else {
        att_status = 'done'
        host_status = 'done'
    ...
    log({ type: 'performance challenge', data: { text: `Hoster successfully completed performance challenge`, hoster_id, feed_id, challenge_id } })
  }
...
async function hoster_replacement_timeout (log, data) {
  const { id } = data
  log({ type: 'chain', data: { text: 'Hoster replacement timed out', id } })
  const { hoster_replacement: { encoder, attester, hoster } } = getAmendmentByID(id)
}
...
async function make_storage_challenge (log, data) {
  ...
  const user = getUserByID(hoster_id)
  const amendments = getAmendmentsForHoster(user)
  ...
  const type = 'storageChallenge'
  avoid[hoster_id] = true
  const idleProviders = DB.status.idleAttesters
  const selectedProviders = select({ idleProviders, role: 'attester', amount: 1, avoid, plan: {}, log })
  const [attester] = selectedProviders
  if (!attester) return DB.queues.tasks.push({ fnName: 'storageChallenge', id })
  storage_challenge.attester = attester.id
  book({ type, selectedProviders, role: 'attester', task_id: id, log })
  book({ type, selectedProviders: [{ id: hoster_id }], role: 'hoster', task_id: id, log })
  //
  log({ type: 'chain', data: { text: 'Making new storage challenge for', items: { type, task_id: id, hoster_id, amendments, cid: id } } })
...
async function make_performance_challenge (log, data) {
  ...
  const idleProviders = DB.status.idleAttesters
  let attesters = select({ idleProviders, role: 'attester', amount: 1, avoid, plan: {}, log })
  if (!attesters.length) return DB.queues.tasks.push({ fnName: type, id })
  performanceChallenge.attesters = attesters.map(attester => attester.id)
  book({ type, selectedProviders: attesters, role: 'attester', task_id: id, log })
  book({ type, selectedProviders: performanceChallenge.hosters.map(id => ({ id, role: 'hoster' })), role: 'hoster', task_id: id, log })
  log({ type: 'challenge', data: {text:'Making new performance challenge', performanceChallenge, items: {challenge: type, task_id: id, attesters, cid: id} }})
...
async function init_amendment (opts) {
  const { contractID, providers, type, pos, ref, log } = opts
...
function getProviders (plan, reused, task_id, log) {
  ...
  const attesters = select({ idleProviders: DB.status.idleAttesters, role: 'attester', amount: attesterAmount, avoid, plan, log })
  if (!attesters.length) return log({ type: 'chain', data: [`missing attesters`] })
  const encoders = select({ idleProviders: DB.status.idleEncoders, role: 'encoder', amount: encoderAmount, avoid, plan, log })
  if (encoders.length !== encoderAmount) return log({ type: 'chain', data: [`missing encoders`] })
  const hosters = select({ idleProviders: DB.status.idleHosters, role: 'hoster', amount: hosterAmount, avoid, plan, log })
...
function book ({ type, selectedProviders, role, task_id, log }) {
  ...
  for (var i = 0, len = selectedProviders.length; i < len; i++) {
    ...
    provider[role].tasks[task_id] = true
...
function unbook_and_rate ({ id, role, task, status, type, opts, log }) {
  ...
  if (role === 'hoster') {
    if (type === 'storage') { // storage challenge
      var all
      if (!opts) {
...
async function pause (opts) {
  const { status, user, until, log } = opts
  const block = await chainAPI.getBlockNumber()
  removeFromIdle(user.id)
  ...
  if (status === 'fail') { // we assume it's a computer or network glitch
    ...
    if (isHoster(user)) {
      ...
      setTimeout(() => { // if no upause on time => replace
        cancelHostingJobs({ user, log })
      }, 10 * 60 * 1000) // blocktime is 10s
    }
  }
    ...
    if (isHoster(user)) {
      ...
      setTimeout(() => { // if no upause on time => replace
        cancelHostingJobs({ user, log })
      }, block_diff * 10 * 1000) // blocktime is 10s
    }
  }
    ...
    if (isHoster(user)) cancelHostingJobs({ user, log })
  }
  user.reputation -= grade
}
async function unpause (opts) {
  const { user, feedID, log } = opts
  const { until } = user.status.paused
  const block = await chainAPI.getBlockNumber()
  var err
  if (block > until) err = new Error('invalid unpause', { cause: 'not unpaused on time'})
  if (user.status.paused) {
    user.status.paused = undefined
    addToIdle(user.id) // user is again available for jobs
  } else err = new Error('invalid unpause', { cause: 'no status paused' })
  if (err) throw err
}
function isHoster (user) {
  return user.hoster.tasks.length
}
function cancelHostingJobs ({ user, log }) {
  const amendmentIDs = getAmendmentsForHoster(user)
  for (const id of amendmentIDs) {
    dropHosting({ amendmentID: id, userID: user.id, type: 'hosting', status: 'pause' })
    startHosterReplacement({ user, amendmentID: id, log })
  }
}
function startHosterReplacement ({ user, amendmentID, log }) {
  // find providers (1 encoder, 1 attester, 1 hoster)
  const amendment = getAmendmentByID(amendmentID)
  const { providers: { hosters, encoders, attesters }, 
    contract: contractID } = amendment
  const len = hosters.length
  var providers
  var pos
  for (var i = 0; i < len; i++) {
    if (hosters[i] === user.id) {
      pos = 0
      providers = { 
        hosters: [...hosters.filter(id => id !== i)],
        encoders: [...encoders.filter(id => id !== i)],
      }
    }
  }
  const type = 'hosterReplacement'
  const opts = { 
    contractID, providers, type, pos, 
    ref: amendmentID, log }
  const new_amendment_id = init_amendment(opts)
  DB.queues.tasks.push({ fnName: type, id: new_amendment_id })
  emitEvent('hosterReplacement', [new_amendment_id], log)
}
function dropHosting ({ amendmentID, userID, type, status }) {
...
async function replaceOtherPausedHosters ({ user, log }) {
  ...
  for (const id of amendments) {
    const { hosters} = await chainAPI.getAmendmentByID(id).providers
    const other_hosters = hosters.filter(id => id !== user.id)
    other_hosters.forEach(async hoster_id => {
      const hoster = await chainAPI.getItemByID(hoster_id)
...
function getAmendmentsForHoster (user) {
  return Object.keys(user.hoster.tasks).map(task => getItem(Number(task))).filter(task => task.contract)
}
function removeFromIdle (id) {
  DB.status.idleHosters.splice(id, 1)
  DB.status.idleAttesters.splice(id, 1)
  DB.status.idleEncoders.splice(id, 1)
}
...
async function next_task (log) {
  ...
  for (var start = new Date(); DB.queues.tasks.length && (new Date() - start < 4000);) {
    ...
    if (fnName === 'hostingSetup' || fnName === 'retry_hostingSetup'
        && DB.status.idleAttesters.length && DB.status.idleEncoders.length >= 3 && DB.status.idleHosters.length >= 3) {
      ...
      if (providers) {
        ...
        ;['attester','encoder','hoster'].forEach(role => {
          book({ type, selectedProviders: providers[`${role}s`], role, task_id: task_id, log })
        })
      ...
      log({ type: 'storage', data: { text: 'storageChallenge in nextTask', task: JSON.stringify(task) } })
      const { hoster: hoster_id, checks } = task
      avoid[hoster_id] = true
      const idleProviders = DB.status.idleAttesters
      const selectedProviders = select({ idleProviders, role: 'attester', amount: 1, avoid, plan: {}, log })
      ...
      if (selectedProviders.length) {
        ...
        book({ type: fnName, selectedProviders, role: 'attester', task_id, log })
      ...
      // TODO: schedule timeout
      const { hosters, feed: feed_id } = task
      const feed = getFeedByID(feed_id)
      ...
      const selectedProviders = select({ idleProviders: DB.status.idleAttesters, role: 'attester', amount: 1, avoid, plan, log })
      if (selectedProviders.length) {
        DB.queues.tasks.shift()
        task.attesters = selectedProviders.map(provider => provider.id)
        book({ type: fnName, selectedProviders, role: 'attester', task_id, log })
        book({ type: fnName, selectedProviders: hosters.map(id => { id }), role: 'hoster', task_id, log })
        // emit event
        log({ type: 'chain', data: [fnName, task_id] })
        emitEvent(fnName, [task_id], log)
      } else {
    ...
    else if (fnName === 'hosterReplacement' && DB.status.idleAttesters.length && DB.status.idleEncoders.length && DB.status.idleHosters.length) { 
      const { providers, pos, contract: contractID } = task
      ...
      task.providers.attesters = [attester]

      const [encoder] = select({ idleProviders: DB.status.idleEncoders, role: 'encoder', amount: 1, avoid, plan, log })
      if (!encoder) return log({ type: 'chain', data: [`missing encoder`] })
      book({ type: fnName, selectedProviders: [encoder], role: 'encoder', task_id, log })
      task.providers.encoders.splice(pos, 0, encoder)
    
      const [hoster] = select({ idleProviders: DB.status.idleHosters, role: 'hoster', amount: 1, avoid, plan, log })
      if (!hoster) return log({ type: 'chain', data: [`missing hoster`] })
      book({ type: fnName, selectedProviders: [hoster], role: 'hoster', task_id, log })
      task.providers.hosters.splice(pos, 0, hoster)
...
// if (DB.active.storageChallenges[id] ) const challenge = getStorageChallengeByID(storageChallengeID)
    ...
    const active_amendment = getAmendmentByID(amendments[amendments.length-1])
    const amendmentID = active_amendment.id
    ...
    hoster_ids.forEach((hoster_id, i) => {
      ...
      dropHosting({ amendmentID, userID: hoster_id, type: 'hosting', status: 'cancel' })
    })
...
async function _retry_amendment ({ failed, amendmentID, log }) {
  ...
  const newID = await init_amendment({ contractID, providers: reuse, type, log })
