...
async function init () {
  ...
  async function signAndSend (msgtype, flow, data, name, status) {
    ...
    try {
      ...
      const opts = { user, name, nonce, status, args }
      if (!user) return void log({ type: 'chain', data: [`UNKNOWN SENDER of: ${data}`] }) // TODO: maybe use status() ??
      else if (type === 'registerForWork') {
        DB.queues.tasks.push({ fnName: 'RegisterForWork', opts })
        next_task(log)
      }
      else if (type === 'publishPlan') {
        DB.queues.tasks.push({ fnName: 'PublishPlan', opts })
        next_task(log)
      }
      else if (type === 'unpublishPlan') {
        DB.queues.tasks.push({ fnName: 'UnpublishPlan', opts })
        next_task(log)
      }
      else if (type === 'hostingSetupReport') {
        DB.queues.tasks.push({ fnName: 'HostingSetupReport', opts })
        next_task(log)
      }
      else if (type === 'hosterReplacementReport') {
        DB.queues.tasks.push({ fnName: 'HosterReplacementReport', opts })
        next_task(log)
      }
      else if (type === 'submitStorageChallenge') {
        DB.queues.tasks.push({ fnName: 'SubmitStorageChallenge', opts })
        next_task(log)
      }
      else if (type === 'submitPerformanceChallenge') {
        DB.queues.tasks.push({ fnName: 'SubmitPerformanceChallenge', opts })
        next_task(log)
      }
      else if (type === 'pause') {
        DB.queues.tasks.push({ fnName: 'Pause', opts })
        next_task(log)
      }
      else if (type === 'unpause') {
        DB.queues.tasks.push({ fnName: 'Unpause', opts })
        next_task(log)
      }
      // else if ...
...
/*----------------------
      REGISTER FOR WORK
------------------------*/
async function _register_for_work (user, { name }, args) {
  const log = connections[name].log
  let [form] = args
...
/*----------------------
      (UN)PUBLISH PLAN
------------------------*/
async function _publish_plan (user, { name }, args) {
  const log = connections[name].log
  log({ type: 'chain', data: [`Publishing a plan`] })
  let [data] = args
  const { plan, components, proofs = {}  } = data
  publish_plan_components_and_execute_plan({ plan, components, proofs, user, log })
}

async function publish_plan_components_and_execute_plan ({ plan, components, proofs, user, log }) {
  const { program } = plan
...
async function _unpublishPlan (user, { name }, args) {
  const log = connections[name].log
  const [planID] = args
...
async function _storage_challenge_report (user, { name }, status, args) {
  const log = connections[name].log
  ...
  try {
    ...
    const valid_report = is_valid_report({ attester_id, user_id, challenge, challenge_id, log })
    if (!valid_report) return log({ type: 'storage challenge', data: { text: `error: not a valid challenge`, challenge_id, challenge: challenge.cid, attester_id, user_id, status: challenge.status, hoster: challenged_hoster.id } })
    ...
    if (res_status === 'fail') {
      ...
      const active_amendments = getActiveAmendmentsFor({ user: challenged_hoster, log })
      ...
      for (const amendmentID of amendmentIDs) {
        ...
        await replaceHoster({ failedHosters: [challenged_hoster.id], amendmentID, log })
      }
    ...
    if (reports.length !== Object.keys(checks).length) {
      ...
      host_status = status
      log({ type: 'storage challenge', data: { text: `Storage challenge analysis`, challenge_id, status, all, proved, hoster_challenge: JSON.stringify(challenged_hoster.hoster.challenge) } })
    }
    unbook_and_rate({ id: attester_id, role: 'attester', task: challenge_id, type, status: att_status, log })
    unbook_and_rate({ id: hoster_id, role: 'hoster', task: challenge_id, type, status: host_status, opts: { all, proved }, log })
...
async function cleanup_and_new_storage_challenge ({ user, log }) {
  ...
  user.hoster.challenge = { status: 'pending', timestamp }
...
async function _performance_challenge_report (user, { name }, args) {
  const log = connections[name].log
  ...
  const valid_report = is_valid_report({ attester_id, user_id, challenge, challenge_id, log })
  if (!valid_report) return log({ type: 'performance challenge', data: { text: `error: not a valid challenge`, attester_id, user_id, challenge, challenge_id } })
...
async function plan_execution (log, data) {
  ...
  if (DB.status.idleAttesters.length && DB.status.idleEncoders.length >= 3 && DB.status.idleHosters.length >= 3) {
    const fnName = 'hostingSetup'
    const { scheduleAction } = await scheduler
    const amendment_id = await init_amendment({ 
      contractID, providers: { encoders: [], attesters: [], hosters: [] }, type: 'hostingSetup', log 
    })
    log({ type: 'chain', data: { text: 'Plan execution', contractID, amendment_id } })

    const task = getItemByID(amendment_id)
    const { contract: contract_id, providers: reuse } = task
    const contract = getContractByID(contract_id)
    const { plan: plan_id } = contract
    const providers = getProviders(getPlanByID(plan_id), reuse, amendment_id, log)
    if (providers)  {
      ;['attester','encoder','hoster'].forEach(role => {
        book({ type: fnName, selectedProviders: providers[`${role}s`], role, task_id: amendment_id, log })
      })
      const keys = Object.keys(providers)
      for (var i = 0, len = keys.length; i < len; i++) {
        providers[keys[i]] = providers[keys[i]].map(item => item.id)
      }
      log({ type: 'chain', data: { text: `Providers for amendment (${amendment_id}) booked`,  providers} })
      task.providers = providers
      // set status 
      task.status = 'in progress'
      // schedule timeout action
      task.sid = scheduleAction({ from: log, data: { id: amendment_id }, delay: 5, type: 'amendment_timeout' })
      contract.status = 'active'
      // emit event
      emitEvent(fnName, [amendment_id], log)
    }
  } else {
    DB.queues.tasks.push({ fnName, contractID }) // TODO: sort tasks based on priority (RATIO!)
    next_task(log)
  }
}
...
async function performance_challenge_timeout (log, data) {
  ...
  attesterIDs.forEach(id => {
    unbook_and_rate({ id, role: 'attester', task: cid,  type: 'performance', status: 'fail', log }) 
    _pause({ args: { status: 'fail', log }, user: getUserByID(id) })
  })
  await cleanup_and_new_performance_challenge(feedID, log)
}
...
async function make_storage_challenge (log, data) {
  ...
  const { scheduleAction } = await scheduler
  start_storageChallenge({  next: { fnName: 'storageChallenge', hoster_id }, scheduleAction, log })
}

async function make_performance_challenge (log, data) {
  const { feedID } = data
  const { scheduleAction } = await scheduler
  start_performanceChallenge({ next: { fnName: 'performanceChallenge', feedID }, scheduleAction, log })
}
...
function is_valid_report ({ attester_id, user_id, challenge, challenge_id, log }) {
  const { cid, status } = challenge
  if (attester_id !== user_id || cid !== challenge_id || status !== 'active' ) {
    log({ type: 'chain', data: { text: `error: Not valid`, attester_id, user_id, cid, challenge_id, status, challenge: JSON.stringify(challenge) } })
    return false
  }
  else return true
}
...
async function _unpause (user, { name }) {
  const log = connections[name].log
...
async function cancelHostingJobs ({ user, log }) {
  const active_amendments = getActiveAmendmentsFor({ user, log })
  const amendmentIDs = active_amendments.map(amendment => amendment.id)
  ...
  for (const id of amendmentIDs) {
    ...
    await replaceHoster({ failedHosters: [user.id], amendmentID: id, log })
  }
}
async function replaceHoster ({ failedHosters, amendmentID, avoid, log }) {
  DB.queues.tasks.push({ fnName: 'hosterReplacement', failedHosters, amendmentID, avoid  })
  next_task(log)
}
...
async function _retry_hosterReplacement ({ failed, amendmentID, log }) {
  ...
  await replaceHoster({ failedHosters, amendmentID, avoid, log })
}
  ...
  for (const id of amendmentIDs) {
    ...
    other_hosters.forEach(async hoster_id => {
      ...
      if (hoster.status.paused) {
        await dropHosting({ amendmentID: id, userID: user.id, type: 'hosting', status: 'cancel', log })
        replaceHoster({ failedHosters: [user.id], amendmentID: id, log })
      }
    })
  }
}
function getActiveAmendmentsFor ({ user, log }) {
  const amendments = Object.keys(user.hoster.tasks)
    ...
    .filter(task => {
      ...
      if (task.status === 'done' && (
        task.type === 'hostingSetup' || task.type === 'retry_hostingSetup' ||
        task.type === 'hosterReplacement' || task.type === 'retry_hosterReplacement') 
      ) return true
    })
  log({ 
    type: 'chain', 
    data: { 
      texts: 'getActiveAmendments for hoster', 
      hoster: user.id, 
...
// HOSTING SETUP
async function start_hostingSetup ({ next, temp_queue, scheduleAction, log }) {
  const { fnName, contractID } = next
  var task_id
  // hostingSetup
  if (fnName === 'hostingSetup') {
  ...
  // retry_hostingSetup
  if (fnName === 'retry_hostingSetup') {
    // make new amendment
    // TODO: implement real reuse logic, for now we just start from scratch
    const providers = { encoders: [], attesters: [], hosters: [] }
    const amendment_id = await init_amendment({ contractID, providers, type: 'retry_hostingSetup', log })
    task_id = amendment_id
    log({ type: 'chain', data: { text: 'Retry amendment', amendment_id } })
  }      
...
async function start_storageChallenge ({ next, temp_queue, scheduleAction, log }) {
  // select an attester
  // tell them which hoster to challenge
  // tell them which subset of contracts & chunks to challenge
  const { hoster_id, fnName } = next
  const avoid = {}
  const user = getUserByID(hoster_id)
  const status = user.hoster.challenge.status
  if (status === 'pending' || status === 'active') {
    if (temp_queue) temp_queue.push(next)
    else {
      DB.queues.tasks.push({ fnName: 'storageChallenge', hoster_id })
      next_task(log)
    }
    const timestamp = performance.now()
  ...
  const amendments = getActiveAmendmentsFor({ user, log })
  const contract_ids = amendments.map(amendment => amendment.contract)
  const active_contracts = []
  for (const id of contract_ids) {
    const contract = getContractByID(id)
    log({ type: 'storage', data: { text: 'hosters contracts', contract_ids, id, contract: JSON.stringify(contract) } })
  ...
  log({ type: 'chain', data: { text: 'Making new storage challenge for', items: { task_id, hoster_id, user: JSON.stringify(user), amendments, cid: id } } })

  // TODO: schedule timeout
  log({ type: 'storage', data: { text: 'storageChallenge in nextTask', task: JSON.stringify(storage_challenge), user_paused: user.status.paused } })
...
// PERFORMANCE CHALLENGE
async function start_performanceChallenge ({ next, temp_queue, scheduleAction, log }) {
  const { fnName, feedID } = next
  const avoid = {}
  const feed = getFeedByID(feedID)
...
async function start_hosterReplacement ({ next, temp_queue, scheduleAction, log }) {
  const { fnName, failedHosters, amendmentID, avoid = {} } = next
  // while hosterReplacement is in progress and we call start replacement for a new hoster, 
  // it's ok, because previous replacement will fail (as one of the active hosters)
  ...
  const { amendments, plan: planID } = getContractByID(contractID)

  const { positions,  _hosters, _attesters, _encoders, ref } = await findRefAmendment({ amendments, log})
    .catch(err => {
      log({ type: 'chain', data: { texts: 'error: no ref amendment', contractID, amendmentID } })
      return
    })

  log({ type: 'chain', data: { texts: 'start hoster replacement', failedHosters, avoid, positions, hosters: JSON.stringify(_hosters) } })
  
  const len = _hosters.length
  var providers = { hosters: [..._hosters], encoders: [..._encoders] }
  for (var i = 0; i < len; i++) {
    if (failedHosters.includes(_hosters[i])) positions.push(i)
  }
  ...
  if (positions.length > 2) {
    ...
    setTimeout(async() => {
      ...
      await replaceHoster({ failedHosters, amendmentID: new_amendment_id, log })
    }, 5000)
    return
  }
  ...
  const opts = { contractID, providers, type: 'hosterReplacement', positions, ref, log }
  const new_amendment_id = await init_amendment(opts)
  const task_id = new_amendment_id
  const task = getItemByID(task_id)
  ...
  if (!attester) {
    temp_queue.push(next)
    return log({ type: 'chain', data: [`missing attester`] })
  }
  ...
  if (!encoders.length) {
    temp_queue.push(next)
    return log({ type: 'chain', data: [`missing encoders`] })
  }
  ...
  if (!hosters.length) {
    temp_queue.push(next)
    return log({ type: 'chain', data: [`missing hosters`] })
  }
  for (var i = 0; i < amount; i++) {   
...
async function findRefAmendment({ amendments, log}) {
  ...
  */
  var _hosters, _attesters, _encoders
  const positions = []
  var ref
  for (var i = amendments.length; i--;) {
    const id = amendments[i]
    const amendment = getAmendmentByID(id)
    const { status, type } = amendment
    ...
    if (type ==='hosterReplacement' && status === 'done') {
      ref = id
      _hosters = amendment.providers.hosters
      _attesters = amendment.providers.attesters
      _encoders = amendment.providers.encoders
      return resolve({ positions,  _hosters, _attesters, _encoders, ref })
    } 
    else if (type ==='hosterReplacement' && status === 'init') {
      ref = amendment.ref
      positions.push(...amendment.positions)
      const ref_amendment = getAmendmentByID(ref)
      _hosters = ref_amendment.providers.hosters
      _attesters = ref_amendment.providers.attesters
      _encoders = ref_amendment.providers.encoders
      return resolve({ positions,  _hosters, _attesters, _encoders, ref })
    }
    ...
    else if (type ==='retry_hostingSetup' && status === 'done') {
      ref = id
      _hosters = amendment.providers.hosters
      _attesters = amendment.providers.attesters
      _encoders = amendment.providers.encoders
...
async function next_task (log) {
  const { scheduleAction } = await scheduler
  const temp_queue = []
  for (var start = new Date(); DB.queues.tasks.length && (new Date() - start < blockinterval);) {
    const next = DB.queues.tasks[0]
    const { fnName, opts } = next 
    DB.queues.tasks.shift()
    log({ type: 'tasks', data: { text: 'next task', fnName, temp_queue: temp_queue.length, len: DB.status.idleAttesters.length, tasks: JSON.stringify(DB.queues.tasks) } })
    if (fnName === 'RegisterForWork') {
      const { user, name, args } = opts
      _register_for_work(user, { name }, args)
    }
    else if (fnName === 'PublishPlan') {
      const { user, name, args } = opts
      _publish_plan(user, { name }, args)
    }
    else if (fnName === 'UnpublishPlan') {
      const { user, name, args } = opts
      _unpublishPlan(user, { name }, args)
    }
    else if (fnName === 'HostingSetupReport') {
      const { user, name, nonce, status, args } = opts
      _hostingSetup_report(user, { name, nonce }, status, args)
    }
    else if (fnName === 'HosterReplacementReport') {
      const { user, name, nonce, status, args } = opts
      _hosterReplacement_report(user, { name, nonce }, status, args)
    }
    else if (fnName === 'SubmitStorageChallenge') {
      const { user, name, status, args } = opts
      _storage_challenge_report(user, { name }, status, args)
    }
    else if (fnName === 'SubmitPerformanceChallenge') {
      const { user, name, args } = opts
      _performance_challenge_report(user, { name }, args)
    }
    else if (fnName === 'Pause') {
      const { user, args } = opts
      _pause(user, args)
    }
    else if (fnName === 'Unpause') {
      const { user, name } = opts
      _unpause(user, { name })
    }
    else if (fnName === 'hostingSetup' || fnName === 'retry_hostingSetup' && 
        DB.status.idleAttesters.length && DB.status.idleEncoders.length >= 3 && 
        DB.status.idleHosters.length >= 3) {
      start_hostingSetup({ next, temp_queue, scheduleAction, log })
    }
    
    else if (fnName === 'storageChallenge' && DB.status.idleAttesters.length) {
      start_storageChallenge({ next, temp_queue, scheduleAction, log })
    }
    
    else if (fnName === 'performanceChallenge' && DB.status.idleAttesters.length) {
      start_performanceChallenge({ next, temp_queue, scheduleAction, log })
    }
    
    else if (fnName === 'hosterReplacement' || fnName === 'retry_hosterReplacement' && 
             DB.status.idleAttesters.length && DB.status.idleEncoders.length && 
             DB.status.idleHosters.length) { 
      start_hosterReplacement({ next, temp_queue, scheduleAction, log })
    }
  }
...
async function _retry_amendment ({ failed, amendmentID, log }) {
  ...
  DB.queues.tasks.push({ fnName: 'retry_hostingSetup', contractID }) // TODO: sort tasks based on priority (RATIO!)
  next_task(log)
}
