...
async function _storage_challenge_report (user, { name, nonce }, status, args) {
  ...
  try {
    ...
    if (!valid_report) return log({ type: 'storage challenge', data: { text: `error: This not a valid challenge`, challenge_id, attester_id, user_id, challenge_id } })

    if (res_status === 'fail') {
      storageChallenge.status = 'fail'
      challenged_hoster.hoster.challenge.status = storageChallenge.status
      log({ type: 'storage challenge', data: { text: `: fail - hoster timed out`, hoster_id, proof_of_contact } })
      att_status = 'unknown'
      host_status = 'fail'
...
async function storage_challenge_timeout (log, data) {
  ...
  log({ type: 'storage-challenge', data: { text: 'error: storage challenge timeout', user: user.id, status, cid, perf: performance.now() - timestamp} })
...
async function startHosterReplacement ({ user, amendmentID, avoid = {}, log }) {
  ...
  const active_amendment = getAmendmentByID(amendments[amendments.length-1])
  log({ type: 'chain', data: { texts: 'start hoster replacement', user_id: user.id, avoid, hosters: JSON.stringify(hosters), amendmentID: active_amendment.id, type: active_amendment.type } })
  const len = hosters.length
  var providers = { hosters: [...hosters], encoders: [...encoders] }
  var pos
  for (var i = 0; i < len; i++) {
    if (hosters[i] === user.id) pos = i
  }
  const type = 'hosterReplacement'
  const opts = { 
  ...
  for (var start = new Date(); DB.queues.tasks.length && (new Date() - start < 4000);) {
    ...
    if (fnName === 'hostingSetup' || fnName === 'retry_hostingSetup' 
      ...
      for (var i = 0; i < providers.encoders.length; i++) {
        if (i === pos) continue // same encoder can be reused, they didn't fail
      ...
      log({ type: 'chain', data: { 
        text: 'getting providers for hosterReplacement', 
        hosters: providers.hosters,
        encoders: providers.encoders,
        pos,
        avoid_keys: Object.keys(avoid),
