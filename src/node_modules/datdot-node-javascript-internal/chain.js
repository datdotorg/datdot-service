...
async function _storage_challenge_report (attester, { name, nonce }, status, args) {
  ...
  const { status: proof_status, proof } = proof_of_contact 
  ...
  try {
    ...
    const valid_report = check_if_report_valid({ attester, attesterID, challenge, storageChallengeID })
    if (!valid_report) return log({ type: 'storage challenge', data: { text: `error: This not a valid challenge`, storageChallengeID } })

    if (!proof_of_contact || !reports || !status) {
      log({ type: 'storage challenge', data: { text: `: error: in storage challenge`, proof_of_contact } })
      att_status = 'fail'
      host_status = 'unknown'
      log({ type: 'storage challenge', data: { text: `: error: attester provided incomplete report`, valid_proof } })
      unbook_and_rate({ id: attesterID, role: 'attester', task: storageChallengeID, type, status: att_status, log })
      unbook_and_rate({ id: hoster_id, role: 'hoster', task: storageChallengeID, type, status: host_status, opts: { all, proved }, log })
      return await start_new_storage_challenge(user, log)
    }
    // verify proof of contact error
    if (proof_status === 'invalid-proof' || proof_status === 'no-proof') {
      host_status = 'fail'
    ...
    log({ type: 'storage challenge', data: { text: `error: storage challenge`, storageChallengeID, attesterID, err } })
  }
...
function check_if_report_valid ({ attester, attesterID, challenge, storageChallengeID }) {
  const {  cid, status } = challenge
  if (attester.id !== attesterID || cid !== storageChallengeID || status !== 'active' ) return false
  else return true
}
