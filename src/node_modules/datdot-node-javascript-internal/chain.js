...
async function next_task (log) {
  ...
  for (var start = new Date(); DB.queues.tasks.length && (new Date() - start < 4000);) {
    ...
    // HOSTING SETUP
    if (fnName === 'hostingSetup' || fnName === 'retry_hostingSetup' && DB.status.idleAttesters.length && 
       DB.status.idleEncoders.length >= 3 && DB.status.idleHosters.length >= 3) {
      const { contractID } = next
      var task_id
      // hostingSetup
    ...
    // HOSTER REPLACEMENT
      ...
      DB.status.idleAttesters.length && DB.status.idleEncoders.length && DB.status.idleHosters.length) { 

      const { failedHosters, amendmentID, avoid = {} } = next
      // while hosterReplacement is in progress and we call start replacement for a new hoster, 
      // it's ok, because previous replacement will fail (as one of the active hosters)
