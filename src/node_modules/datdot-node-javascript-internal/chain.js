...
async function make_storage_challenge (log, data) {
  ...
  var contractsToCheck = contract_ids.length > 20 ? 
    get_random_ids({ items: contract_ids, max: 20 }) : contract_ids
  ...
  for (const contractID of contractsToCheck ) {
    const { ranges } = getContractByID(contractID)
...
function startHosterReplacement ({ user, amendmentID, log }) {
  ...
  function dropHosting ({ amendmentID, topic, remotekeys, userID, type, status }) {
...
async function _retry_amendment ({ failed, amendmentID, log }) {
  emitEvent('hostingSetup_failed', [amendmentID, failed], log)
  const task = amendmentID
  const amendment = getAmendmentByID(amendmentID)
  const reuse = Object.assign({}, amendment.providers)
  const { providers: { hosters, attesters, encoders }, contract: contractID } = amendment
  const contract = getContractByID(contractID)
  // check if previous
  const prev_amendment_id = contract.amendments[contract.amendments.length-2]
  const prev_amendment = getAmendmentByID(prev_amendment_id)
  ...
  for (var i = 0, len = failed.length; i < len; i++) {
    const id = failed[i]
    const user = getUserByID(id)
    log({ type: 'chain', data: { text: 'Failed user', failed, user, amendmentID } })
    // did user fail in this amendment OR was the previous also fail and this user was one of the participants
    // it is possible that attester keeps te connection although that peer might have disconnected in between 
    // and that is the reason the amendment failed again
    if (attesters.includes(id) || (prev_amendment?.status === 'fail' && prev_amendment?.providers.attesters.includes(id))) { 
      reuse.attesters = []
      _pause({ args: { status: 'fail', log }, user })
      unbook_and_rate({ id, role: 'attester', task, type: 'hostingSetup', status: 'fail', log })
    }
    else if (hosters.includes(id) || prev_amendment?.status === 'fail' && prev_amendment?.providers.hosters.includes(id)) {
      reuse.hosters.splice(hosters.indexOf(id), 1)
      _pause({ args: { status: 'fail', log }, user })
      unbook_and_rate({ id, role: 'hoster', task, type: 'hostingSetup', status: 'fail', log })
    }
    else if (encoders.includes(id) || (prev_amendment?.status === 'fail' && prev_amendment?.providers.encoders.includes(id))) {
      reuse.encoders.splice(encoders.indexOf(id), 1)
      _pause({ args: { status: 'fail', log }, user })
  ...
  log({ type: 'chain', data: { text: 'Retry amendment', reuse, newID } })
