...
function startHosterReplacement ({ user, amendmentID, log }) {
  ...
  const amendment = getAmendmentByID(amendmentID)
  const { providers, contract: contractID } = amendment
  const reuse = { 
    encoders: [...providers.encoders], 
    attesters: [...providers.attesters], 
    hosters: [...providers.hosters.filter(id => id !== user.id)] 
  }
  const new_amendment_id = init_amendment (contractID, reuse, log)
  DB.queues.tasks.push({ fnName: 'HosterReplacement', id: new_amendment_id })
...
async function next_task (log) {
  ...
  for (var start = new Date(); DB.queues.tasks.length && (new Date() - start < 4000);) {
    ...
    if (fnName === 'amendment' && DB.status.idleAttesters.length && DB.status.idleEncoders.length >= 3 && DB.status.idleHosters.length >= 3) {
      ...
      const { providers, contract: contractID } = task
      ...
      task.hosters.push(hoster.id)
      
      task.hoster_replacement = {
        hoster: hoster.id, encoder: encoder.id, attester: attester.id
      }
      // emit event
      log({ type: 'chain', data: [fnName, task_id] })
      emitEvent(fnName, [task_id], log)
