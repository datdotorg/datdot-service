...
async function _hosterReplacement_report (user, { name, nonce }, status, args) {
  ...
  if (contract.amendments[contract.amendments.length - 1] !== amendmentID) log({ type: 'chain', data: [`Warning: this amendment for hosterReplacement has expired`, amendmentID] })
...
async function hoster_replacement_timeout (log, data) {
  ...
  log({ type: 'chain', data: { text: 'Hoster replacement timed out', amendmentID } })
  const amendment = getAmendmentByID(amendmentID)
  amendment.status = 'timed-out'
...
async function cancelHostingJobs ({ user, log }) {
  const amendmentIDs = getActiveAmendmentsFor(user)
...
async function start_HosterReplacement ({ failedHosters, amendmentID, avoid, log }) {
  ...
  */
  ...
  var positions = []
  var ref
  const reversed = amendments.reverse()
  ...
  if (positions.length > 2) {
    log({ type: 'chain', data: { texts: 'error: no more hosters available for this feed, hosting replacement can not be done', contractID } })
    return
  }
...
async function _retry_hosterReplacement ({ failed, amendmentID, log }) {
  ...
  const { providers: { hosters }, positions } = getAmendmentByID(amendmentID)
  const avoid = {}
...
function getActiveAmendmentsFor (user) {
  const amendments = Object.keys(user.hoster.tasks)
    ...
    .filter(task => {
      // don't include tasks tat are in progress or failed 
      // (i.e. hosterReplacement role for some new contract
      // that is not done yet or might even fail)
      if (task.status === 'done') return task.contract
    })
  return amendments
}
