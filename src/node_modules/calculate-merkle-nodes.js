...
// NOTES BY ALEX FROM HOSTER.js

 
      // get all available hashnodes
      // const nodes = []
      // for (var i = 0; i < (feed.length * 2) - 1; i++) nodes[i] = add(i)
      ...
      //   })
      // }
      // const hashnodes = (await Promise.all(nodes)).filter(x=>x).sort(numsort)
      // function numsort (a, b) { return a < b ? -1 : 1 }
  
      // const last = feed.length - 1
      // const calculate = require('calculate-merkle-nodes')
      // const temp = {}
      // indizes.forEach(index => {
      //   const i = BigInt(index)
      //   const n = BigInt(last)
      //   const asdf = calculate(i,n).map(n => Number(n))
      //   temp[index] = asdf.map(x => Number(x))
      // })
      // const proofs = [...new Set(Object.entries(temp).flatMap(([key, val]) => val))].sort(numsort)
  
      // const missing = proofs.filter(x => hashnodes.indexOf(x) === -1)
      // console.log({proofs, hashnodes, missing})
  
      // let a = [...new Set([hashnodes])]
      // let b = new Set(xxx)
      // let difference = new Set(a.filter(x => !b.has(x)))
      // let intersection = new Set(a.filter(x => b.has(x)))
      
      // console.log({difference, intersection})
