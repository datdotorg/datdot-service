    ...
    accounts.forEach(async account => {
      ...
      await register.signAndSend(account, ({ events = [], status }) => {
        ...
        if (status.isInBlock) {
          on((err, event) => {
            if (err) LOG(err)
            if (event.method === "NewPin" && account.address === event.data[0].toString()) {
              LOG(`You (${account.address}) have been selected as a hoster!`)
              const request = event.data[1].toString() // feedkey + swarmkey ?
              cb(request)
            }
          })
          handleRegisterHoster(account.address, events)
        }
      })
    })
  }
...
function requestHosting (opts) {
  { request } = opts
  const requestHosting = API.tx.datVerify.requestHosting(request)
  await requestHosting.signAndSend(account, async ({ events = [], status }) => {
    LOG(`Requesting hosting: ${account.address} `, status.type)
    if (status.isInBlock) // do something
  })
}
...
async function on (cb) {
  const unsub = API.query.system.events((events) => {
    events.forEach(async (record) => {
      ...
      LOG('NEW EVENT: METHOD:', event.method)
      LOG('NEW EVENT: DATA', event.data.toString())
      cb(null, event)
    })
  })
}
