...
async function compare_encodings ({ messages, key, msg, log }) {
  return new Promise(async (resolve, reject) => {
    log({ type: 'attestor', data: { text: `comparing encodings starts` } })
    ...
    log({ type: 'attestor', data: { text: `comparing encodings for index: ${index} => (${messages[index].length}/3)` } })
    if (messages[index].length === 3) {
      log({ type: 'attestor', data: { text: `comparing encodings for index!` } })
      find_invalid_encoding(messages[index], (err, res) => {
        messages[index].forEach(item => {
          if (err) return item.reject(err)
          log({ type: 'attestor', data: { text: `All 3 encodings for index ${index} verified`, res } })
